...
function updateVehicle($id,$model_id,$vehicle_engine_no,$vehicle_chasis_no,$vehicle_color_id,$model_year,$condition,$basic_price,$proof_type_id_array,$proof_no_array,$proof_img_array,$godown_id=NULL,$customer_id=NULL,$vehicle_reg_no=NULL,$vehicle_reg_date=NULL,$cng_cylinder_no=NULL,$cng_kit_no=NULL,$is_purchased=NULL,$is_sold_customer=NULL,$battery_make_id=NULL,$battery_no=NULL,$key_no=NULL,$service_book=NULL,$fitness_exp_date=NULL,$permit_exp_date=NULL,$battery_service_no=NULL,$service_no=NULL,$ledger_id=NULL,$opening_balance=NULL,$opening_cd=NULL,$extra_ledger_id=NULL,$opening_balance_extra=NULL,$opening_cd_extra=NULL){
	try
	{
		...
		if(!validateForNull($service_book))
		$service_book=$vehicle['service_book'];
		...
		if(!validateForNull($battery_make_id))
		{
		if(checkForNumeric($vehicle['battery_make_id']))	
		...
		if(!validateForNull($is_purchased))
		$is_purchased=$vehicle['is_purchased'];
		
		if(!validateForNull($is_sold_customer))
		$is_sold_customer=$vehicle['is_sold_by_customer'];
		
		if(!validateForNull($opening_balance))
		$opening_balance=$vehicle['opening_balance'];
		
		if(!validateForNull($opening_cd))
		$opening_cd=$vehicle['opening_cd'];
		
		if(!validateForNull($vehicle_reg_date))
		$vehicle_reg_date=date('d/m/Y',strtotime($vehicle['vehicle_reg_date']));
		
		if(!validateForNull($permit_exp_date))
		$permit_exp_date=date('d/m/Y',strtotime($vehicle['permit_exp_date']));
		
		if(!validateForNull($fitness_exp_date))
		$fitness_exp_date=date('d/m/Y',strtotime($vehicle['fitness_exp_date']));;
		
		if(!validateForNull($battery_service_no))
		$battery_service_no=$vehicle['battery_service_book_no'];
		
		if(!validateForNull($service_no))
		$service_no=$vehicle['service_no'];
		
		
		if(!checkForNumeric($ledger_id) || $ledger_id==-1)
		{
		if(!checkForNumeric($vehicle['ledger_id']))	
		$ledger_id="NULL";
		else
		$ledger_id = $vehicle['ledger_id'];
		}
		...
		if(!checkForNumeric($extra_ledger_id) || $extra_ledger_id==-1)
		{
		if(!checkForNumeric($vehicle['extra_ledger_id']))	
		$extra_ledger_id="NULL";
		$opening_balance_extra=$vehicle['opening_balance_extra'];
		$opening_cd_extra=$vehicle['opening_cd_extra'];
		}
		
		
		if(checkForNumeric($model_id,$model_year,$condition,$opening_balance,$opening_cd) && validateForNull($vehicle_engine_no,$vehicle_reg_date,$vehicle_chasis_no,$model_year,$vehicle_reg_no,$cng_cylinder_no,$cng_kit_no,$battery_no,$key_no,$service_book,$customer_id,$battery_make_id,$is_purchased,$is_sold_customer,$fitness_exp_date,$permit_exp_date,$service_no,$battery_service_no)  && !checkForDuplicateVehicle($vehicle_chasis_no,$vehicle_engine_no,$vehicle_reg_no,$id))
		{
			...
			$is_purchased = $old_vehicle['is_purchased'];
