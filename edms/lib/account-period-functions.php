...
function updateCurrentDateForUser($id,$curr_date)
{
	...
	setPeriodForUser($id,date('d/m/Y',strtotime($creater_period[1])),date('d/m/Y',strtotime($creater_period[2])));
			setCurrentDateForUser($id,date('d/m/Y',strtotime(getTodaysDate())));
			setCurrentCompanyForUser($id,$creater_period['curr_company'],$creater_period['company_type']);
	}
	else
	{
...
function getLatestPeriodSettings()
{
	$sql="SELECT period_id, from_period, to_period, last_updated, curr_date, curr_company,company_type FROM edms_ac_period_date ORDER BY last_updated DESC";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
