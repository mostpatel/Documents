			...
			if(checkForNumeric($model_id,$vehicle_color_id,$model_year,$basic_price,$godown_id,$condition) && validateForNull($vehicle_chasis_no,$vehicle_engine_no,$service_book)  && ($condition==1 || validateForNull($reg_no)))
			{
				...
				if(checkForNumeric($vehicle_id))
				{
					...
					if(getDeliveryChallanByVehicleId($vehicle_id))
					{
						...
						if(checkForNumeric($vehicle['vehicle_color_id']))
						...
						$reg_date = date('d/m/Y',strtotime($vehicle['vehicle_reg_date']));
						$fitness_date = date('d/m/Y',strtotime($vehicle['fitness_exp_date']));
						$permit_date = date('d/m/Y',strtotime($vehicle['permit_exp_date']));
						$ledger_id = $vehicle['ledger_id'];
						if(!validateForNull($battery_make_id))
						$battery_make_id=NULL;
					}
					
					updateVehicle($vehicle_id,$model_id,$vehicle_engine_no,$vehicle_chasis_no,$vehicle_color_id,$model_year,$condition,$basic_price,array(),array(),array(),$godown_id,NULL,$reg_no,$reg_date,$cng_cylinder_no,$cng_kit_no,$vehicle['is_purchased'],$vehicle['is_sold_by_customer'],$battery_make_id,$vehicle['battery_no'],$vehicle['key_no'],$service_book,$fitness_date,$permit_date);
	...
	// Loan jv 
		...
		SELECT IF(to_ledger_id = $from_ledger_id OR auto_rasid_type=5,edms_ac_jv_cd.amount,-edms_ac_jv_cd.amount) as amount
			  ...
			  WHERE auto_id=$vehicle_id AND edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id
			   AND (auto_rasid_type=2 OR auto_rasid_type=5) AND edms_ac_jv_cd.type=0";
	// vehicle recipts
		$sql=$sql." UNION ALL
		SELECT  -amount
		...
		if(checkForNumeric($purchase_id))
		{  
		$sql1="SELECT vehicle_id FROM edms_ac_purchase_vehicle WHERE purchase_id=$purchase_id"; 
		$result1=dbQuery($sql1);
		$result1Array = dbResultToArray($result1);
		$no=0;
		foreach($result1Array as $no_vehicle_id)
		{
			
			if($vehicle_id==$no_vehicle_id[0])
			break;
			else
			$no++;
		}	  
		
		$sql1="SELECT purchase_item_id FROM edms_ac_purchase_item WHERE purchase_id=$purchase_id"; 
		$result1=dbQuery($sql1);
		$result1Array = dbResultToArray($result1);
		
		$purchase_item_id = $result1Array[$no][0];
		
		$sql=$sql." UNION ALL SELECT -net_amount FROM edms_ac_purchase_item WHERE  purchase_item_id = $purchase_item_id ";
		$sql=$sql." UNION ALL SELECT -SUM(tax_amount) as tax_amount FROM edms_ac_purchase_tax WHERE  edms_ac_purchase_tax.purchase_id = $purchase_id AND edms_ac_purchase_tax.vehicle_id = $vehicle_id GROUP BY edms_ac_purchase_tax.vehicle_id ";	 
		}
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		if(dbNumRows($result)>0)
