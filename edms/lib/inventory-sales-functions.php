...
function getTaxwiseAmountPercentForSaleIdStockItems($sales_id)
{
	if(checkForNumeric($sales_id))
	{
		$return_array = array();
		$sql="SELECT  edms_ac_sales_item.sales_item_id, net_amount as amount,SUM(net_amount) as total_amount,edms_tax_grp.tax_group_id,display_name, edms_ac_sales_item.ledger_id, edms_ac_sales_item.tax_class_id FROM edms_ac_sales_item LEFT JOIN edms_ac_sales_tax ON edms_ac_sales_item.sales_item_id = edms_ac_sales_tax.sales_item_id
		LEFT JOIN edms_tax_grp ON edms_tax_grp.tax_group_id = edms_ac_sales_tax.tax_group_id WHERE edms_ac_sales_item.sales_id = $sales_id GROUP BY  edms_tax_grp.tax_group_id
		 ";	
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		...
		if(dbNumRows($result)>0)
		{
			...
			foreach($resultArray as $re)
			{
				...
				{
				$return_array[$re['tax_group_id']][1] = $return_array[$re['tax_group_id']][1] + $re['amount'];
				$total = $total + $re['amount'];
				}
				else
				{
				$return_array[$re['tax_group_id']][0] = $re['display_name'];
				$return_array[$re['tax_group_id']][1] = $re['amount'];
				$return_array[$re['tax_group_id']][3] = $re['ledger_id'];
				$return_array[$re['tax_group_id']][4] = $re['tax_class_id'];
				$total = $total + $re['amount'];
				}
			}
			...
			{
				$return_array[$key][2] = $return_array[$key][1] / $total;
...
function getNonStockItemForDisplayForSaleId($sales_id)
{
	if(checkForNumeric($sales_id))
	{
		$return_array = array();
		$sql="SELECT edms_ac_sales_nonstock.sales_non_stock_id,GROUP_CONCAT(edms_ac_sales_nonstock.sales_non_stock_id) as non_stock_ids,item_id,discount,SUM(amount) as amount,SUM(net_amount) as net_amount,edms_ac_sales_nonstock.sales_id,created_by,last_updated_by,date_added,date_modified FROM edms_ac_sales_nonstock 
		WHERE edms_ac_sales_nonstock.sales_id = $sales_id GROUP BY item_id";	
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		if(dbNumRows($result)>0)
		{
			...
			{
				$sales_item_id = $re['sales_non_stock_id'];
				$sales_item_id_string = $re['non_stock_ids'];
				$tax = getTaxForSaleItemId($sales_item_id_string,true);
...
function getTaxForSaleItemId($sales_item_id,$non_stock=false)
{
	...
	if(checkForNumeric($sales_item_id))
	{
		...
		$sql=$sql." edms_ac_sales_tax.sales_non_stock_id IN ($sales_item_id) GROUP BY tax_id";
		}
...
function getSaleIdsByItemDesc($item_desc)
{
	if(validateForNull($item_desc))
	{
		$sql="SELECT DISTINCT sales_id FROM edms_ac_sales_item WHERE item_desc = '$item_desc'";
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $resultArray;
...
function getSaleIdStringByItemDesc($item_desc)
{
	if(validateForNull($item_desc))
	{
		$sql="SELECT GROUP_CONCAT(DISTINCT sales_id) FROM edms_ac_sales_item WHERE item_desc = '$item_desc'";
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $resultArray;
...
function getCustomerIdsByItemDesc($item_desc)
{
	if(validateForNull($item_desc))
	{
		$sql="SELECT DISTINCT to_customer_id FROM edms_ac_sales_item INNER JOIN edms_ac_sales ON edms_ac_sales_item.sales_id = edms_ac_sales.sales_id WHERE item_desc = '$item_desc' AND to_customer_id IS NOT NULL";
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
