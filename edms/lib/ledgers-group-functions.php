...
function insertLedgerGroupName($name)
{
	...
	if(validateForNull($name) && strlen($name)<255 && !checkForDuplicateLedgerGroup($name))
	{
		...
		$sql="INSERT INTO edms_ac_ledgers_group (group_name,created_by,last_updated_by,date_added,date_modified)
...
function editLedgerGroupName($id,$name)
{
	...
	if(validateForNull($name) && strlen($name)<255 && !checkForDuplicateLedgerGroup($name,$id) && checkForNumeric($id))
	{
		$name=trim($name);
		$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
		$sql="UPDATE  edms_ac_ledgers_group
...
function deleteLedgerGroup($id)
{
	...
	{
		$sql="DELETE FROM edms_ac_ledgers_group WHERE group_id=$id";
		dbQuery($sql);
		return "success";
		}
	return "error";	
...
function insertLedgersToGroup($ledgers_array,$group_id)
{
	...
	{	
	$sql="INSERT INTO edms_ac_rel_ledgers_group (group_id,ledger_id)
...
function deleteRelLedgerGroupByGroupID($group_id)
{
	if(checkForNumeric($group_id))
	{
		$sql="DELETE FROM edms_ac_rel_ledgers_group WHERE group_id=$group_id";
		dbQuery($sql);
		return "success";
		}
	return "error";	
...
function deleteRelLedgerGroupByLedgerID($group_id)
{
	if(checkForNumeric($group_id))
	{
		$sql="DELETE FROM edms_ac_rel_ledgers_group WHERE ledger_id=$group_id";
		dbQuery($sql);
		return "success";
		}
	return "error";	
...
function mergeGroup($merge_id_array,$name,$city_id)
{
	...
	$sql="UPDATE edms_customer SET group_id=$group_id
	       WHERE group_id IN ($merge_id_string)";
	dbQuery($sql);
	$sql="UPDATE edms_guarantor SET group_id=$group_id
	       WHERE group_id IN ($merge_id_string)";
	dbQuery($sql);
	
	$sql="UPDATE edms_ac_rel_ledgers_group SET group_id=$group_id
	       WHERE group_id IN ($merge_id_string)";
	dbQuery($sql);
...
{
	$sql="SELECT edms_ac_ledgers_group.group_id,group_name,GROUP_CONCAT(ledger_id) as ledgers_id
	      FROM edms_ac_ledgers_group
		  LEFT JOIN edms_ac_rel_ledgers_group
		  ON edms_ac_ledgers_group.group_id=edms_ac_rel_ledgers_group.group_id
		  GROUP BY edms_ac_ledgers_group.group_id";
	$result=dbQuery($sql);	 
	if(dbNumRows($result)>0)
...
function getLedgerGroupByID($id)
{
	$sql="SELECT edms_ac_ledgers_group.group_id,group_name,GROUP_CONCAT(ledger_id) as ledgers_id
		  ...
		  LEFT JOIN edms_ac_rel_ledgers_group
		  ON edms_ac_ledgers_group.group_id=edms_ac_rel_ledgers_group.group_id
		  WHERE edms_ac_ledgers_group.group_id=$id
		  GROUP BY edms_ac_ledgers_group.group_id";
	$result=dbQuery($sql);	
	$resultArray=dbResultToArray($result); 
	if(dbNumRows($result)>0)
...
{
	$sql="SELECT ledger_id FROM edms_ac_rel_ledgers_group";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
	{
	...
	{
	$sql="SELECT ledger_id,ledger_name  FROM edms_ac_ledgers WHERE ledger_id NOT IN ($groups_id) ORDER BY ledger_name";
	$result2=dbQuery($sql);
