...
if(isset($_GET['action']))
{
	...
	if($_GET['action']=='search')
	{
		...
		if(validateForNull($chasis_number) || validateForNull($engine_number) || validateForNull($reg_number) || validateForNull($mobile_number) || validateForNull($name) || validateForNull($financer_name) || validateForNull($item_name) || validateForNull($item_desc)) 
			{
				...
				else if(validateForNull($chasis_number))
				{
					...
					else if(is_array($file_id))
					{
						...
						unset($_SESSION['search']);
						$_SESSION['ack']['msg']="Invalid Chasis Number!";
						$_SESSION['ack']['type']=5; // 5 for access
						header("Location: ".$_SERVER['PHP_SELF']);
						exit;
					...
					else if(validateForNull($item_desc))
				{
					
					$file_id=getCustomerIdsByItemDesc($item_desc);
					if(checkForNumeric($file_id))
					{
						unset($_SESSION['search']);
						header("Location: ".WEB_ROOT."admin/customer/index.php?view=details&id=".$file_id);
					...
					else if(is_array($file_id))
					{
						$file_id_array=$file_id;
						$_SESSION['search']['file_id_array']=$file_id_array;
						$_SESSION['search']['parameter']="Item Desc";
						$_SESSION['search']['value']=$chasis_number;
						header("Location: ".$_SERVER['PHP_SELF']);
						exit;
					}
						...
						unset($_SESSION['search']);
						$_SESSION['ack']['msg']="Invalid Item Desc!";
						$_SESSION['ack']['type']=5; // 5 for access
						header("Location: ".$_SERVER['PHP_SELF']);
						exit;
