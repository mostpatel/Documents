<?php 
        $id=$_GET['id'];
		if(validateForNull($id)&& substr($id, 0, 1) == 'C') // if the payment is done to a general account ledger
		{
...
<h4 class="headingAlignment">Transactions For <?php if(checkForNumeric($main_customer_id)) echo $main_customer['customer_name']; else if(checkForNumeric($main_ledger_id)) echo $main_ledger_name; echo "[".date("F", mktime(0, 0, 0, $month, 10))." ".$year."]"; echo ' ['.$from.'-'.$to."]";  ?></h4>
<?php 
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    ...
    <tbody>
		...
		if($payments!="error" && is_array($payments))
		{
				...
				$from_customer_id=$payment['from_customer_id'];
				 ...
				 <tr class="resultRow">
					...
					<td><?php echo ++$i; ?></td>
					 ...
					 <td><?php if($payment['auto_rasid_type']==0 || $payment['auto_rasid_type']>100) echo "Manual"; else if($payment['auto_rasid_type']==4) echo "Vehicle"; else if($payment['auto_rasid_type']==5) echo "Kasar";  ?>
					</td>
         ...
         <?php  }
		  else if(array_key_exists('purchase_tax_id',$payment))
				{
				$from_ledger_id=$payment['from_ledger_id'];
				$from_customer_id=$payment['from_customer_id'];
				 ?>
				...
				$from_customer_id=$payment['from_customer_id'];
				 ...
				 <tr class="resultRow">
				   ...
				   <td><?php if($head_type==1 || $head_type==0) $openingBalance=$openingBalance-$payment['amount']; else if($head_type==3) $openingBalance=$openingBalance+$payment['amount']; if($openingBalance>=0) echo number_format( $openingBalance,2)." Dr"; else echo number_format( -$openingBalance,2)." Cr";  ?>
					...
					<td class="no_print"> <a href="<?php if($payment['auto_rasid_type']==2) { echo WEB_ROOT.'admin/accounts/transactions/purchase_inventory/index.php?view=details&id='.$payment['purchase_id']; } else if($payment['auto_rasid_type']==1) echo WEB_ROOT.'admin/purchase/vehicle/index.php?view=details&id='.($payment['purchase_id']); ?>"><button title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
         ...
         <?php  }
				...
				$from_customer_id=$payment['from_customer_id'];
				 ...
				 <tr class="resultRow">
					...
					<td><?php 
					if($head_type==3 || $head_type==2) echo number_format( $payment['amount'],2);
					 ?>
					</td>
				   <td><?php if($head_type==1 || $head_type==0) $openingBalance=$openingBalance+$payment['amount']; else if($head_type==3 || $head_type==2) $openingBalance=$openingBalance-$payment['amount']; if($openingBalance>=0) echo number_format( $openingBalance,2)." Dr"; else echo number_format( -$openingBalance,2)." Cr";  ?>
					</td>
         ...
         <?php  }
				...
				{	
					...
					$to_customer_id=$payment['to_customer_id'];
					 ...
					 <tr class="resultRow">
						...
						<td><?php echo ++$i; ?></td>
						 ...
						 <td><?php if($payment['auto_rasid_type']==0 || $payment['auto_rasid_type']>100)echo "Manual"; else if($payment['auto_rasid_type']==2 ) echo "Financer"; else if($payment['auto_rasid_type']==3 ) echo "Jobcard"; else if($payment['auto_rasid_type']==4 ) echo "Vehicle";  else if($payment['auto_rasid_type']==5 ) echo "Sales"; else echo "Auto"; ?>
						</td>
					...
					<td class="no_print"> <a href="<?php
						...
						{
							$job_card_id_id=($payment['auto_id']);
						}
						
						
						 if($payment['auto_rasid_type']==0) { echo WEB_ROOT.'admin/accounts/transactions/receipt/index.php?view=details&id='.$payment['receipt_id']; } else if($payment['auto_rasid_type']==2) echo WEB_ROOT.'admin/customer/financer_receipt/index.php?view=details&id='.$payment['receipt_id']; else if($payment['auto_rasid_type']==3) echo WEB_ROOT.'admin/customer/vehicle/jobCard/receipt/index.php?view=details&id='.$payment['receipt_id']; else if($payment['auto_rasid_type']==4) echo WEB_ROOT.'admin/accounts/transactions/receipt/index.php?view=details&id='.$payment['receipt_id']; else echo WEB_ROOT.'admin/accounts/transactions/receipt/index.php?view=details&id='.$payment['receipt_id']; ?>"><button title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
         ...
         <tr class="resultRow">
            ...
            <td><?php if($jv_type==0) // if it is a accounts ledger and a debit jv
			 		{ if(is_numeric($from_ledger_id))  echo getLedgerNameFromLedgerId($from_ledger_id);
					  else if(is_numeric($from_customer_id)){ $customer=getCustomerDetailsByCustomerId($from_customer_id); echo $customer[1]." ";  }
					}
					else if($jv_type==1) // if it is a accounts ledger and a credit jv
			 		{ if(is_numeric($to_ledger_id))  echo getLedgerNameFromLedgerId($to_ledger_id);
					  else if(is_numeric($to_customer_id)) {  $customer=getCustomerDetailsByCustomerId($to_customer_id); echo $customer[1]; }
					}
					 ?>
            </td>
             <td><?php
			
			  if($payment['auto_rasid_type']==0)echo "Manual"; else if($payment['auto_rasid_type']==3) echo "Out Labour"; else if($payment['auto_rasid_type']==1) echo "Financer"; else if($payment['auto_rasid_type']==2) echo "Vehicle Purchase"; else if($payment['auto_rasid_type']==4) echo "Vehicle Sales"; else if($payment['auto_rasid_type']==7) echo "Kasar"; else if($payment['auto_rasid_type']==6) echo "Payment For Customer"; else if($payment['auto_rasid_type']==5) echo "Loan"; ?>
            </td>
			 ...
			 if($payment['auto_rasid_type']==0) { echo WEB_ROOT.'admin/accounts/transactions/jv/index.php?view=details&id='.$payment['jv_id']; } else echo WEB_ROOT.'admin/accounts/transactions/jv/index.php?view=details&id='.$payment['jv_id'];   ?>"><button title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
		...
		$from_ledger_id=$payment['from_ledger_id'];
         ...
         <tr class="resultRow">
           ...
           <td><?php if($to_ledger_id==$ledger_id) $openingBalance=$openingBalance+$payment['amount']; if($from_ledger_id==$ledger_id) $openingBalance=$openingBalance-$payment['amount']; if($openingBalance>=0) echo number_format( $openingBalance,2)." Dr"; else echo number_format( -$openingBalance,2)." Cr";  ?>
            </td>
