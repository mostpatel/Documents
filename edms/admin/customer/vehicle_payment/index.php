	...
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{			
				...
				$result=addPayment($_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks'],4,$_POST['vehicle_id']); // $cheque_return is 0 when inserting a payment			
				...
				header("Location: ".WEB_ROOT."admin/customer/vehicle_payment/index.php");
				exit;
				}
				else
				{
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				...
				header("Location: ".WEB_ROOT."admin/customer/vehicle_payment/index.php?id=".$_GET['id']);
				exit;
				}
			}
			else
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updatePaymentForVehicleId($_POST['vehicle_id'],$_POST["lid"],$_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks']);
				if($result=="success")
				{	
				$_SESSION['ack']['msg']="Payment updated Successfuly!";
				$_SESSION['ack']['type']=2; // 2 for update
				header("Location: ".WEB_ROOT."admin/customer/vehicle_payment/index.php?id=".$_POST['customer_id']);
				exit;
				}
				else{
			...
			{	
					$_SESSION['ack']['msg']="Authentication Failed! Not enough access rights!";
					$_SESSION['ack']['type']=5; // 5 for access
					header("Location: ".WEB_ROOT."admin/customer/vehicle_payment/index.php?view=details&id=".$_POST['lid']);
				exit;
			}
