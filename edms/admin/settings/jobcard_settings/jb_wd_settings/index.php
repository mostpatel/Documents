...
require_once "../../../../lib/job-card-work-done-functions.php";
...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertJobCardWorkDone($_POST["name"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Job Card Work Done successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteJobCardWorkDone($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Job Card Work Done deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Job Card Work Done! Job Card Work Done already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateJobCardWorkDone($_POST["lid"],$_POST["name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Job Card Work Done updated Successfuly!";
