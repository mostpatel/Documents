...
require_once "../../../../lib/job-card-description-functions.php";
...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertJobCardDescription($_POST["name"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Job Card Description / Customer Complaint successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteJobCardDescription($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Job Card Description / Customer Complaint deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Job Card Description / Customer Complaint! Job Card Description / Customer Complaint already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateJobCardDescription($_POST["lid"],$_POST["name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Job Card Description / Customer Complaint updated Successfuly!";
