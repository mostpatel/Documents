...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertItemManufacturer($_POST["name"],$_POST["address"],$_POST["contact_no"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Item Manufacturer successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteItemManufacturer($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Item Manufacturer deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Item Manufacturer! Item Manufacturer already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateItemManufacturer($_POST["lid"],$_POST["name"],$_POST["address"],$_POST["contact_no"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Item Manufacturer updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/itemManufacturer.js");
