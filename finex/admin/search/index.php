...
if(isset($_GET['action']))
{
	...
	if($_GET['action']=='search')
	{
		...
		if(validateForNull($agreement_number) || validateForNull($file_number) || validateForNull($reg_number) || validateForNull($mobile_number) || validateForNull($name) || validateForNull($gua_name))
			{
					...
					else if(validateForNull($reg_number))
				{
					...
					if(!checkForNumeric($reg_number[0],$reg_number[1]) && ($reg_number[2]=='0'))
			{
				$reg_number=substr($reg_number,0,2).substr($reg_number,3);
			}
					...
					else if(validateForNull($gua_name))
					{
					$file_id=getFileIdFromGuarantorName($gua_name);
					if(checkForNumeric($file_id))
					{
						unset($_SESSION['search']);
						header("Location: ".WEB_ROOT."admin/customer/index.php?view=details&id=".$file_id);
					...
					else if(is_array($file_id))
					{
						...
						{
						$_SESSION['search']['file_id_array']=$file_id_array;
						$_SESSION['search']['parameter']="Guarantor Name Like";
						$_SESSION['search']['value']=$name;
						header("Location: ".$_SERVER['PHP_SELF']);
						exit;	
							}
						...
						$_SESSION['search']['parameter']="Guarantor Name";
						$_SESSION['search']['value']=$name;
						header("Location: ".$_SERVER['PHP_SELF']);
						exit;
					}
						...
						unset($_SESSION['search']);
						$_SESSION['ack']['msg']="Invalid Guarantor Name!";
						$_SESSION['ack']['type']=5; // 5 for access
						header("Location: ".$_SERVER['PHP_SELF']);
						exit;
