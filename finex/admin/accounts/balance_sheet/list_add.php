<?php
		...
		$comp_type = $current_company[1];
		$comp_id = $current_company[0];
		if($comp_type==0) // oc
		{
			$error_files=checkForProperLoanEntriesOC($comp_id);
		}
		else if($comp_type==1) // agency
		{
			$error_files=checkForProperLoanEntriesAgency($comp_id);
		}
		else if($comp_type==2) // combined
		{}
...
<?php if($error_files && is_array($error_files)) { ?>
<div class="alert alert-error">
<strong>Warning!</strong> Loan Entries Missing/Errored For <?php foreach($error_files as $error_file) echo $error_file['file_number']." <br> "; ?>
</div>
<?php } ?>
<?php 
