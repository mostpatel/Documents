 ...
 <?php if(isset($_SESSION['ledgerEntriess']['entries_array']))
{
	...
	$openingBalance=getOpeningBalanceForLedgerForDate($id,$from);
     ...
     <h4 class="headingAlignment">Transaction For <?php if(isset($_SESSION['ledgerEntriess']['ledger_id']) && validateForNull($_SESSION['ledgerEntriess']['ledger_id'])) {  $display_name = $display_name." (".$from; 
	 if(isset($to))
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    ...
    <thead>
        ...
        <th class="heading no_sort">Added By</th>
        <th class="heading no_print btnCol"></th>
        </tr>
    </thead>
    <tbody>
      <?php 
	  		if(in_array($head_id,array(20,21,22,23,29,30)))
	  		$openingBalance=0;
	   ...
	   ?>
		...
		if($payments!="error" && is_array($payments))
		{
			foreach($payments as $payment)
			{
				...
				if($head_type==1) $openingBalance=$openingBalance+$payment['amount']; else if($head_type==0) $openingBalance=$openingBalance-$payment['amount'];
				 ?>
				 <tr class="resultRow <?php if($openingBalance<0 && $head_type==0) echo "dangerRow"; ?>">
					<td class="no_print"><input type="checkbox" class="selectTR" name="selectTR"  /></td>
					<td><?php echo ++$i; ?></td>
				   ...
				   <td><?php  if($openingBalance>=0) echo round($openingBalance)." Dr"; else echo round(-$openingBalance,2)." Cr";  ?>
					</td>
                     <td><?php echo $payment['remarks']; if($payment['auto_rasid_type']==1) {
						 
						 $loan_cheque_array=getLoanChequeByLoanId($payment['auto_id']);
						 if(is_array($loan_cheque_array))
						 {
						echo " Nos : <br>";	 
						  foreach($loan_cheque_array as $loan_cheque)
						  { 
						  echo $loan_cheque['loan_cheque_no']."<br>";  
						  }
						 }
						 } ?></td>
                    <td><?php echo getAdminUserNameByID($payment['last_updated_by']); ?></td>
         ...
         <?php  }
				...
				{	
					...
					if($head_type==0) $openingBalance=$openingBalance+$payment['amount']; else if($head_type==1) $openingBalance=$openingBalance-$payment['amount'];
					 ?>
					 <tr class="resultRow <?php if($openingBalance<0 && $head_type==0) echo "dangerRow"; ?>">
						<td class="no_print"><input type="checkbox" class="selectTR" name="selectTR"  /></td>
						<td><?php echo ++$i; ?></td>
					   ...
					   <td><?php  if($openingBalance>=0) echo round($openingBalance,2)." Dr"; else echo round(-$openingBalance,2)." Cr";  ?>
						</td>
...
foreach($credit_details as $debit_detail)
{
	...
	$credit_string = $credit_string.$name." <br>";
		...
		if($jv_type==0) $openingBalance=$openingBalance+$payment['amount']; else if($jv_type==1) $openingBalance=$openingBalance-$payment['amount'];
		...
		if($to_ledger_id==$ledger_id) $openingBalance=$openingBalance+$payment['amount']; else if($from_ledger_id==$ledger_id) $openingBalance=$openingBalance-$payment['amount'];
		 ?>
         <tr class="resultRow <?php if($openingBalance<0 && $head_type==0) echo "dangerRow"; ?>">
         	<td class="no_print"><input type="checkbox" class="selectTR" name="selectTR"  /></td>
        	<td><?php echo ++$i; ?></td>
             ...
             <td class="date"> <?php  echo date('d/m/Y',strtotime($payment['trans_date'])); ?>
              ...
              <td><?php if($head_type==0){
				  		if($to_ledger_id!=$ledger_id)  echo getLedgerNameFromLedgerId($to_ledger_id);
					  	else  echo getLedgerNameFromLedgerId($payment['from_ledger_id']); } ?>
            </td>
             <td><?php if($payment['auto_rasid_type']==0)echo "Manual"; else echo "Auto"; ?>
            </td>
