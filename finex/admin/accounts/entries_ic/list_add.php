...
<td width="260px;">From Date : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['ledgerICEntriess']['from'])) echo $_SESSION['ledgerICEntriess']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To Date : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['ledgerICEntriess']['to'])) echo $_SESSION['ledgerICEntriess']['to']; ?>"/>	
                 </td>
</tr>
...
<td>Ledger<span class="requiredField">* </span> : </td>
				<td>
                   ...
                   <select id="to_ledger" name="ledger_id" >
					...
					{
					?>
                    <option value="<?php echo $ledger['id']; ?>" <?php if( $ledger['id']==$_SESSION['ledgerICEntriess']['ledger_id']) { ?>  selected="selected"<?php } ?>><?php echo $ledger['name']; ?></option>			
                    <?php	
						}
					 ?>
...
<td>Transactions : </td>
				<td>
					<select name="transaction_type[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="1" <?php if(in_array(1,$_SESSION['ledgerICEntriess']['transaction_array'])) { ?>  selected="selected"<?php } ?> >Payments</option>
                         <option value="2" <?php if(in_array(2,$_SESSION['ledgerICEntriess']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Receipts</option>
...
<td width="260px;">Interest In(%): </td>
				<td>
				 <input autocomplete="off" type="text"  name="debit_interest" id="debit_interest" placeholder="Debit Interest Rate!"  value="<?php if(isset($_SESSION['ledgerICEntriess']['debit_ic'])) echo $_SESSION['ledgerICEntriess']['debit_ic']; ?>" />	
                 </td>
</tr>
...
<td>Interest Out(%): </td>
				<td>
				 <input autocomplete="off" type="text"  name="credit_interest" id="credit_interest" placeholder="Credit  Interest Rate!" value="<?php if(isset($_SESSION['ledgerICEntriess']['credit_ic'])) echo $_SESSION['ledgerICEntriess']['credit_ic']; ?>"/>	
                 </td>
</tr>
...
<td width="260px;">TDS(%): </td>
				<td>
				 <input autocomplete="off" type="text"  name="tds" id="tds" placeholder="TDS Rate!"  value="<?php if(isset($_SESSION['ledgerICEntriess']['tds'])) echo $_SESSION['ledgerICEntriess']['tds']; ?>" />	
                 </td>
</tr>


<tr>
<td>TDS IF INTEREST (IN)>=: </td>
				<td>
				 <input autocomplete="off" type="text"  name="tds_on_amount" id="tds_on_amount" placeholder="TDS ON (IN) INTEREST AMOUNT!" value="<?php if(isset($_SESSION['ledgerICEntriess']['tds_on_amount'])) echo $_SESSION['ledgerICEntriess']['tds_on_amount']; ?>"/>	
                 </td>
</tr>
...
<td></td>
 ...
 <?php if(isset($_SESSION['ledgerICEntriess']['entries_array']))
{
	$id=$_SESSION['ledgerICEntriess']['ledger_id'];
	$transaction_array=$_SESSION['ledgerICEntriess']['entries_array'];
	
	$payments=$transaction_array[0];
	$head_type=$transaction_array[1];
	
	
	if(isset($_SESSION['ledgerICEntriess']['from']) && validateForNull($_SESSION['ledgerICEntriess']['from']))
	{	
	$from=$_SESSION['ledgerICEntriess']['from'];
	}
	else
	{
	$from=getBooksStartingDateForLedgerCustomer($id);
		...
		if(isset($_SESSION['ledgerEntriess']['to']) && validateForNull($_SESSION['ledgerEntriess']['to']))
	{	
	$to=$_SESSION['ledgerEntriess']['to'];
	...
	$openingBalance=getOpeningBalanceForLedgerForDate($id,$from);
	 ...
	 ?>    
      <h4 class="headingAlignment">Transaction For <?php if(isset($_SESSION['ledgerICEntriess']['ledger_id']) && validateForNull($_SESSION['ledgerICEntriess']['ledger_id'])) {  $display_name = $display_name." (".$from; 
	 if(isset($to))
	 $display_name = $display_name." - ".date('d/m/Y',strtotime($to));
	 else
$display_name = $display_name." - ALL";	  
	 $display_name = $display_name.")";
	 echo $display_name; }else{ ?>Ledger<?php } ?></h4>
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    ...
    <tbody>
      <?php 
	  $debit_interest_rate=$_SESSION['ledgerICEntriess']['debit_ic'];
			$credit_interest_rate=$_SESSION['ledgerICEntriess']['credit_ic'];
			$tds_rate = $_SESSION['ledgerICEntriess']['tds'];
		    $tds_on_amount = $_SESSION['ledgerICEntriess']['tds_on_amount'];
			...
			$to = $_SESSION['ledgerICEntriess']['to'];
...
foreach($credit_details as $debit_detail)
{
   ...
   <table id="to_print" style="width:100%;" class="to_print adminContentTable"></table> 
    ...
    top:0;left:0;display:block;margin-top:50px;">Net Interest : <?php  if($total_debit_interest>=$total_credit_interest){ $net_interest= $total_debit_interest - $total_credit_interest; echo $net_interest." (IN)"; if($net_interest>=$tds_on_amount){ echo "TDS Payeable : "; echo $net_interest*$tds_rate/100; }} else { echo  $total_credit_interest - $total_debit_interest; echo "(OUT)"; }  ?></span>
<?php  ?>      
</div>
      ...
      _createAutocomplete: function() {
        ...
        this._on( this.input, {
          autocompleteselect: function( event, ui ) {
			...
			setPercentForLedger(ui.item.option);
          },
...
function setPercentForLedger(ledger_option_el)
{
	var ledger_id=ledger_option_el.value;
	var xmlhttp;
if (window.XMLHttpRequest)
  ...
  if (xmlhttp.readyState==4 && xmlhttp.status==200)
	...
	document.getElementById('debit_interest').value = myarray[0];
	document.getElementById('credit_interest').value = myarray[1];

    }
  }
  var url = '<?php echo WEB_ROOT ?>'+'json/ledger_percent.php?id='+ledger_id;
