...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		...
		$result=insertRtoAgent($_POST["name"], $_POST["address"], $_POST["contactNo"]);
		if($result=="success")
				{
				$_SESSION['ack']['msg']="RtoAgent successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteRtoAgent($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="RtoAgent deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete RtoAgent! RtoAgent already in use!";
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updateRtoAgent($_POST["lid"],$_POST["name"], $_POST["address"],  $_POST["contactNo"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="RtoAgent updated Successfuly!";
