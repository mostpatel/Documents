...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertCasePetetionar($_POST["name"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Case Petetionar successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteCasePetetionar($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Case Petetionar deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Case Petetionar! Case Petetionar already in use!";
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateCasePetetionar($_POST["lid"],$_POST["name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Case Petetionar updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/CasePetetionars.js");
