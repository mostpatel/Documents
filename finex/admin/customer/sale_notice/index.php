...
require_once "../../../lib/sale-notice-functions.php";
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=insertSaleNotice($_POST['file_id'],$_POST["notice_date"],$_POST['customer_name'],$_POST['customer_address'],$_POST['guarantor_name'],$_POST['guarantor_address'],$_POST['paid_emis'],$_POST['paid_amount'],$_POST['remarks']);
				
				if($result!="error" && is_numeric($result))
				{
				$_SESSION['ack']['msg']="notice successfully added!";
				$_SESSION['ack']['type']=1; // 1 for insert
				header("Location: ".WEB_ROOT."admin/customer/sale_notice/index.php?view=notice&id=".$result);
				exit;
				}
				else{
				...
				header("Location: ".WEB_ROOT."admin/customer/sale_notice/index.php?id=".$_POST['file_id']);
				exit;
				}
			}
			else
