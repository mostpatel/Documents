...
<?php  } ?> 
<div class="rasid">
<b> <i> For Vehicle No : </i>  </b><?php echo $reg_no; ?> &nbsp; &nbsp;<?php if(isset($vehicle['fitness_exp_date']) && $vehicle['fitness_exp_date']!="1970-01-01") echo "<b> Passing Exp : </b>".date('d/m/Y',strtotime($vehicle["fitness_exp_date"])); ?>
&nbsp; &nbsp;<?php if(isset($vehicle['fitness_exp_date']) && $vehicle['permit_exp_date']!="1970-01-01") echo "<b> Permit Exp : </b>".date('d/m/Y',strtotime($vehicle["permit_exp_date"])); ?>
</div>  

<div class="rasid smallerfont">
ખાસ નોંધ : વીમો, ટેક્ષ, પરમીટ તથા પાસીંગ ની જવાબદારી લોન લેનાર પાર્ટીની છે.&nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <span style="font-size:24px;"><?php  if(is_array($insurance)) {?> <?php echo "<b> Insurance Exp : </b>".date('d/m/Y',strtotime($insurance['insurance_issue_date'])); } ?></span>
</div>
...
<div class="lowerLeftDiv">
        ...
        <div class="rasid">
તમારા આજ સુધીના ચઢેલ હપ્તા : <?php 

$upto_date = date('Y-m-d',strtotime($payment['payment_date']));

$next_date = getNextDate(date('d/m/Y',strtotime($upto_date)));

echo getBucketForLoan($loan_id,$next_date); ?>
</div>
