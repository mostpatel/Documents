...
<h4 class="headingAlignment no_print">Custom Cash Reports</h4>
<?php 
 ...
 <?php if(isset($_SESSION['cInterestReport']['from'])) { ?>
  <span class="Total" style="margin-right:20px;font-weight:bold;"> From : <?php if(isset($_SESSION['cInterestReport']['from'])) echo $_SESSION['cInterestReport']['from']; ?></span>
     <span class="Total" style="margin-right:20px;font-weight:bold;"> To : <?php if(isset($_SESSION['cInterestReport']['to'])) echo $_SESSION['cInterestReport']['to']; ?></span>
      <span class="Total" style="margin-right:20px;font-weight:bold;">Type : Cash</span>
 ...
 <?php if(isset($_SESSION['cInterestReport']['remainder_array']))
{
	...
	$emi_array=$_SESSION['cInterestReport']['remainder_array'];
		$to = $_SESSION['cInterestReport']['to'];
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    ...
    <tbody>
		...
		foreach($emi_array as $emi)
		{
			...
			if(strtotime($emi['loan']['loan_approval_date'])<=strtotime($to))
			{
			...
			$total_interest_left = $total_interest_left + ($emi['emis_left']*$emi['interest_per_emi']);
			$file_status = NULL;
			}
         ...
         <tr class="resultRow">
            ...
            <td><?php echo date('d/m/Y',strtotime($emi['loan']['loan_approval_date'])); ?></td>
             ...
             <td><b><?php if(isset($file_status) && $file_status==4) echo number_format($amount_paid,2); else  echo number_format($emi['emi_paid'],2); ?></b>
            </td>
            
          
            <td><b><?php echo number_format($emi['interest'],2); if(isset($file_status) && $file_status==4) echo "<br>".number_format($refund,2)." (R)"; ?></b>
            </td>
            <td>
            ...
            <td><?php if(isset($file_status) && $file_status==4) echo "0.00"; else echo number_format($emi['emis_left'],2); ?>
            </td>
            <td><?php if(isset($file_status) && $file_status==4) echo "0.00"; else echo number_format(($emi['emis_left']*$emi['interest_per_emi']),2); ?>
            </td>
   ...
   <table id="to_print" style="width:100%;" class="to_print adminContentTable"></table> 
    <div class="Total" style="margin-right:20px;font-weight:bold;">Total Interest : <?php if(isset($total)) echo number_format($total)." "; ?> </div> 
  
     <div class="Total" style="margin-right:20px;"> Total Refund : <?php if(isset($total_refund)) echo number_format($total_refund); ?></div>
     <div class="Total" style="margin-right:20px;"> Total Interest Left : <?php if(isset($total_interest_left)) echo number_format($total_interest_left); ?></div>
<?php  ?>      
</div>
