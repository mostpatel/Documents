...
function generalEMIReports($from=NULL,$to=NULL,$win_gt=NULL,$win_lt=NULL,$emi_gt=NULL,$emi_lt=NULL,$balance_gt=NULL,$balance_lt=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL,$upto=NULL,$vehicle_type=NULL,$group_array = NULL,$roi_gt=NULL,$roi_lt=NULL,$from_loan_amount=NULL,$to_loan_amount=NULL,$include_legal_case=1,$from_loan_approval_date=NULL,$to_approval_date=NULL)
{
...
if(isset($from_loan_approval_date) && validateForNull($from_loan_approval_date))
{
	    $from_loan_approval_date = str_replace('/', '-', $from_loan_approval_date);
		$from_loan_approval_date=date('Y-m-d',strtotime($from_loan_approval_date));
	}
if(isset($to_approval_date) && validateForNull($to_approval_date))
{
	$to_approval_date = str_replace('/', '-', $to_approval_date);
		$to_approval_date=date('Y-m-d',strtotime($to_approval_date));
	}	
	...
	$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_number, emi, file_status, oc_id, agency_id,
		  ...
		  SUM(emi_amount) as total_emi_amount, broker_name
	 	  FROM 
	      fin_file 
...
if(!validateForNull($agency_id))
{
	...
	if(isset($from_loan_approval_date) && validateForNull($from_loan_approval_date))
	$sql=$sql."loan_approval_date>='$from_loan_approval_date' 
		  AND ";
	if(isset($to_approval_date) && validateForNull($to_approval_date))  
	$sql=$sql."loan_approval_date<='$to_approval_date'
		  AND ";  
	...
	$sql=$sql."emi<=$emi_lt
		  AND ";	 
	if(isset($roi_gt) && validateForNull($roi_gt))  
	$sql=$sql."roi>=$roi_gt
		  AND ";
	if(isset($roi_lt) && validateForNull($roi_lt))  
	$sql=$sql."roi<=$roi_lt
		  AND ";	 
		  if(isset($from_loan_amount) && validateForNull($from_loan_amount))  
	$sql=$sql."loan_amount>=$from_loan_amount
		  AND ";
	  if(isset($to_loan_amount) && validateForNull($to_loan_amount))  
	$sql=$sql."loan_amount>=$to_loan_amount
		  AND "; 	 	  
	...
	$sql=$sql." group_id IN ($group_string)
		  AND ";	  	
		  if(isset($include_legal_case) && checkForNumeric($include_legal_case) && $include_legal_case==0)  
	$sql=$sql." fin_file.file_id NOT IN (SELECT file_id FROM fin_legal_notice)
		  AND ";
	if(isset($include_legal_case) && checkForNumeric($include_legal_case) && $include_legal_case==2)  
	$sql=$sql." fin_file.file_id IN (SELECT file_id FROM fin_legal_notice)
		  AND ";	    	  	  	  
	$sql=$sql."	 
...
$today=date('Y-m-d');
	...
	if(dbNumRows($result)>0)
	{
		for($xy=0;$xy<count($resultArray);$xy++)
		{
			...
			$returnArray[$j]['total_emi_amount']=$reportRow['total_emi_amount'];
			...
			$loan_id_array[] = $loan_id;
			}
			$j++;
			}
		
		uasort($returnArray,'EMIDatesComparatorForEmiReports');
		if(validateForNull($from_loan_approval_date,$to_approval_date))
	    $_SESSION['cCustomReport']['loan_id_array'] = $loan_id_array;
		return $returnArray;	
...
if(!validateForNull($agency_id))
{
	...
	$sql=$sql."	 fin_loan.file_id=fin_file.file_id
		  ...
		  AND fin_file.file_id NOT IN (SELECT file_id FROM fin_vehicle_seize)
		  GROUP BY fin_file.file_id
		  ORDER BY fin_file.file_id";
...
function generalEMIReportsPaymentDate($from=NULL,$to=NULL,$win_gt=NULL,$win_lt=NULL,$emi_gt=NULL,$emi_lt=NULL,$balance_gt=NULL,$balance_lt=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL,$date_type=NULL,$vehicle_type=NULL,$from_emi_date=NULL,$to_emi_date=NULL,$group_array=NULL,$sort_by_area_name=1,$file_id_array=NULL,$include_legal_case=1,$emi_date_type=NULL) // $date_type=NULL mean by payment_date , 1= loan_approval_date, 2= loan_percentage ** $from_emi_date and $to_emi_date are only day part of the date AND emi_date_type = NULL, means normal emi_date , emi_date_type =1 treat it as loan_approval_date, emi_date_type = 2 treat it as loan_percentage
{
	...
	{
		$file_id_string = implode(",",$file_id_array);
	}
...
if(validateForNull($emi_date_type) && $emi_date_type==1)
{
if(isset($from_emi_date) && validateForNull($from_emi_date))
{
	    $from_emi_date = str_replace('/', '-', $from_emi_date);
		$from_emi_date=date('Y-m-d',strtotime($from_emi_date));
	}
if(isset($to_emi_date) && validateForNull($to_emi_date))
{
	$to_emi_date = str_replace('/', '-', $to_emi_date);
		$to_emi_date=date('Y-m-d',strtotime($to_emi_date));
}
}
	$today = getTodaysDate();	
	...
	$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_number, file_agreement_no, emi, file_status, oc_id, agency_id,
		  ...
		  GROUP_CONCAT(actual_emi_date) as loan_emi_date_array,
		  MAX(actual_emi_date) as last_emi_date,
		  loan_approval_date,area_id, broker_name, vehicle_reg_no, vehicle_engine_no, vehicle_chasis_no, model_id, vehicle_company_id, vehicle_model, (SELECT count(loan_emi_id) FROM fin_loan_emi as inner_emi WHERE inner_emi.loan_id = fin_loan.loan_id AND actual_emi_date <= '$today') as emi_to_be_paid, loan_duration, (SELECT MAX(legal_notice_id) FROM fin_legal_notice WHERE fin_legal_notice.file_id = fin_file.file_id GROUP BY fin_legal_notice.file_id ) as legal_notice_id, ((SELECT SUM(payment_amount) FROM fin_loan_emi_payment INNER JOIN fin_loan_emi ON fin_loan_emi.loan_emi_id = fin_loan_emi_payment.loan_emi_id WHERE loan_id = fin_loan.loan_id) / (SELECT SUM(emi_amount) FROM fin_loan_emi WHERE loan_id = fin_loan.loan_id)) as loan_paid_percent 
	 	  FROM 
	      fin_file 
		  INNER JOIN  fin_loan ON fin_loan.file_id=fin_file.file_id
		  INNER JOIN  fin_loan_emi ON fin_loan.loan_id=fin_loan_emi.loan_id
...
if(!validateForNull($agency_id))
{
	...
	{	  	
	if(isset($from_emi_date) && validateForNull($from_emi_date) && !validateForNull($emi_date_type))
	$sql=$sql." DAY(actual_emi_date)>=$from_emi_date 
		  AND ";
			  
	if(isset($to_emi_date) && validateForNull($to_emi_date) && !validateForNull($emi_date_type))  
	$sql=$sql." DAY(actual_emi_date)<=$to_emi_date
		  AND ";	
	}
	
	if(isset($from_emi_date) && validateForNull($from_emi_date) && $emi_date_type==1)
	$sql=$sql." loan_approval_date>='$from_emi_date' 
		  AND ";
	if(isset($to_emi_date) && validateForNull($to_emi_date) && $emi_date_type==1)  
	$sql=$sql." loan_approval_date<='$to_emi_date'
		  AND "; 
	
	
	  
		  
	if(isset($file_id_array) && is_array($file_id_array) && count($file_id_array)>0 && validateForNull($file_id_string)) 
	$sql=$sql."  fin_file.file_id IN ($file_id_string) AND ";  
	$sql=$sql."	  our_company_id=$oc_id 
		  GROUP BY fin_file.file_id HAVING 1=1 ";
	if(isset($from_emi_date) && validateForNull($from_emi_date) && $emi_date_type==2)
	$sql=$sql." AND ((emi_to_be_paid/loan_duration)*100)>='$from_emi_date' 
		   ";
	if(isset($to_emi_date) && validateForNull($to_emi_date) && $emi_date_type==2)  
	$sql=$sql." AND ((emi_to_be_paid/loan_duration)*100)<='$to_emi_date'
		   "; 		 
	if(isset($from_emi_date) && checkForNumeric($from_emi_date) && $emi_date_type==3)
	{
	$sql=$sql." AND ( ((loan_paid_percent*100)>=$from_emi_date) 
		   ";
	if($from_emi_date==0)
	$sql=$sql." OR  loan_paid_percent IS NULL )";
	else
	$sql=$sql.")";	   
	}
	if(isset($to_emi_date) && checkForNumeric($to_emi_date) && $emi_date_type==3)  
	{
	$sql=$sql." AND (((loan_paid_percent*100)<=$to_emi_date)
		   "; 	
	if($from_emi_date==0)
	$sql=$sql." OR  loan_paid_percent IS NULL )";
	else
	$sql=$sql.")";	 	   	
		  ...
		  $sql=$sql."ORDER BY fin_file.file_id";
...
$today=date('Y-m-d');
	...
	if(dbNumRows($result)>0)
	{
		for($xy=0;$xy<count($resultArray);$xy++)
		{
			...
			if($latest_payment_date=='1970-01-01')
			{
				
			$latest_payment_date = $reportRow['loan_approval_date'];
			$latest_payment_date_type = 1; // 1 means $latest_payment_date is the $last_emi_date
			}
			else
			$latest_payment_date_type = 0;
			...
			if(
				(
					($setWin_gt==-1 && $setWin_lt==-1) || 
					(  ($setWin_gt==-1 || $window>=$setWin_gt) && 
					   ($setWin_lt==-1 || $window<=$setWin_lt) 
					 )
				) && 
				( 
					($setbal_gt==-1 && $setbal_lt==-1) || 
					( 
						($setbal_gt==-1 || $balance>=$setbal_gt) && 
						($setbal_lt==-1 || $balance<=$setbal_lt) 
					)
				) && 
				validateForNull($emi_date) && 
				$emi_date!="NA" && 
				(
					(
						!validateForNull($from) || 
						(strtotime($latest_payment_date)<=strtotime($from)) && 
						(
							!validateForNull($to) || 
							(strtotime($latest_payment_date)>strtotime($to))
						) && 
						!validateForNull($date_type)
					) || 
					$date_type==1
				)
			)
			...
			$returnArray[$j]['model_id']=$reportRow['model_id'];
			$returnArray[$j]['vehicle_company_id']=$reportRow['vehicle_company_id'];
			...
			$returnArray[$j]['bucket_details']=$bucket_details;
			$returnArray[$j]['broker_name']=$reportRow['broker_name'];
			$returnArray[$j]['vehicle_model']=$reportRow['vehicle_model'];
			$customer=getCustomerDetailsByFileId($file_id);
			...
			$returnArray[$j]['loan_paid_percent']=$reportRow['loan_paid_percent']*100;
			$returnArray[$j]['legal_notice_id']=$reportRow['legal_notice_id'];
			}
			$j++;
			}
		if($sort_by_area_name==1)
		uasort($returnArray,'EMIPaymentDatesComparatorForEmiReports');
		else if($sort_by_area_name==2)
		{
		uasort($returnArray,'EMIPaymentDatesComparatorForEmiReportsDesc');
		}
		else
		uasort($returnArray,'EMIAreaNameComparatorForEmiReports');
...
function kankriyaBrokerReports($from=NULL,$to=NULL,$from_approval_date=NULL,$to_approval_date=NULL,$from_noc_date=NULL,$to_noc_date=NULL,$roi_gt=NULL,$roi_lt=NULL,$vehicle_condition=NULL,$win_gt=NULL,$win_lt=NULL,$emi_gt=NULL,$emi_lt=NULL,$balance_gt=NULL,$balance_lt=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL,$date_type=NULL,$vehicle_type=NULL,$from_emi_date=NULL,$to_emi_date=NULL,$group_array=NULL,$sort_by_area_name=1,$file_id_array=NULL,$include_legal_case=1,$emi_date_type=NULL,$noc_status=NULL) // $date_type=NULL mean by payment_date , 1= loan_approval_date, 2= loan_percentage ** $from_emi_date and $to_emi_date are only day part of the date AND emi_date_type = NULL, means normal emi_date , emi_date_type =1 treat it as loan_approval_date, emi_date_type = 2 treat it as loan_percentage
{
...
if(isset($from_approval_date) && validateForNull($from_approval_date))
{
	    $from_approval_date = str_replace('/', '-', $from_approval_date);
		$from_approval_date=date('Y-m-d',strtotime($from_approval_date));
	}
if(isset($to_approval_date) && validateForNull($to_approval_date))
{
	$to_approval_date = str_replace('/', '-', $to_approval_date);
...
if(isset($from_noc_date) && validateForNull($from_noc_date))
{
	    $from_noc_date = str_replace('/', '-', $from_noc_date);
		$from_noc_date=date('Y-m-d',strtotime($from_noc_date));
	}
if(isset($to_noc_date) && validateForNull($to_noc_date))
{
	$to_noc_date = str_replace('/', '-', $to_noc_date);
		$to_noc_date=date('Y-m-d',strtotime($to_noc_date));
}
if(validateForNull($emi_date_type) && $emi_date_type==1)
{
if(isset($from_emi_date) && validateForNull($from_emi_date))
...
if(isset($to_emi_date) && validateForNull($to_emi_date))
{
	...
	$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_number, file_agreement_no, emi, file_status, oc_id, agency_id, file_status,
		  GROUP_CONCAT(loan_emi_id) as loan_emi_array,
		  GROUP_CONCAT(actual_emi_date) as loan_emi_date_array,
		  MAX(actual_emi_date) as last_emi_date,roi, loan_amount, loan_ending_date,
		  loan_approval_date,area_id, broker_name, vehicle_reg_no, vehicle_engine_no, vehicle_chasis_no, model_id, vehicle_company_id, vehicle_model, vehicle_condition, vehicle_type_id, (SELECT count(loan_emi_id) FROM fin_loan_emi as inner_emi WHERE inner_emi.loan_id = fin_loan.loan_id AND actual_emi_date <= '$today') as emi_to_be_paid,(SELECT MAX(actual_emi_date) FROM fin_loan_emi as inner_emi WHERE inner_emi.loan_id = fin_loan.loan_id AND actual_emi_date <= '$today') as emi_date_to_be_paid, loan_duration, (SELECT MAX(legal_notice_id) FROM fin_legal_notice WHERE fin_legal_notice.file_id = fin_file.file_id GROUP BY fin_legal_notice.file_id ) as legal_notice_id, ((SELECT SUM(payment_amount) FROM fin_loan_emi_payment INNER JOIN fin_loan_emi ON fin_loan_emi.loan_emi_id = fin_loan_emi_payment.loan_emi_id WHERE loan_id = fin_loan.loan_id) / (SELECT SUM(emi_amount) FROM fin_loan_emi WHERE loan_id = fin_loan.loan_id)) as loan_paid_percent 
	 	  FROM 
	      fin_file 
		  INNER JOIN  fin_loan ON fin_loan.file_id=fin_file.file_id
		  INNER JOIN  fin_loan_emi ON fin_loan.loan_id=fin_loan_emi.loan_id
		  ...
		  LEFT JOIN fin_vehicle ON fin_file.file_id = fin_vehicle.file_id
		  LEFT JOIN fin_rel_groups_file ON fin_file.file_id = fin_rel_groups_file.file_id
		  LEFT JOIN fin_file_noc ON fin_file_noc.file_id = fin_file.file_id
		  WHERE  file_status!=3
		  AND "; 
if($our_company_id=="NULL" && is_numeric($agency_id))
{
...
if(!validateForNull($agency_id))
{
	...
	if(isset($from_approval_date) && validateForNull($from_approval_date) )
	$sql=$sql."loan_approval_date>='$from_approval_date' 
		  AND ";
	if(isset($to_approval_date) && validateForNull($to_approval_date))  
	$sql=$sql."loan_approval_date<='$to_approval_date'
		  AND "; 
	...
	if(isset($from_noc_date) && validateForNull($from_noc_date) )
	$sql=$sql."noc_date>='$from_noc_date' 
		  AND ";
	if(isset($to_noc_date) && validateForNull($to_noc_date))  
	$sql=$sql."noc_date<='$to_noc_date'
		  AND "; 
	
	if(isset($noc_status) && checkForNumeric($noc_status) && $noc_status==1)
	$sql=$sql." fin_file.file_id IN (SELECT file_id FROM fin_file_noc) AND ";
	
	if(isset($noc_status) && checkForNumeric($noc_status) && $noc_status==0)
	$sql=$sql." fin_file.file_id NOT IN (SELECT file_id FROM fin_file_noc) AND ";
	...
	$sql=$sql." vehicle_type_id IN ($vehicle_type)
		  AND ";
	if(isset($vehicle_condition) && validateForNull($vehicle_condition) && $vehicle_condition<2)  
	$sql=$sql." vehicle_condition IN ($vehicle_condition)
		  AND ";	  
	if(isset($include_legal_case) && checkForNumeric($include_legal_case) && $include_legal_case==0)  
	$sql=$sql." fin_file.file_id NOT IN (SELECT file_id FROM fin_legal_notice)
		  AND ";
...
$today=date('Y-m-d');
	...
	if(dbNumRows($result)>0)
	{
		for($xy=0;$xy<count($resultArray);$xy++)
		{
			...
			$returnArray[$j]['area_id']=$reportRow['area_id'];
			$returnArray[$j]['model_id']=$reportRow['model_id'];
			$returnArray[$j]['vehicle_type_id']=$reportRow['vehicle_type_id'];
			$returnArray[$j]['vehicle_condition']=$reportRow['vehicle_condition'];
			$returnArray[$j]['vehicle_company_id']=$reportRow['vehicle_company_id'];
			$returnArray[$j]['file_agreement_number']=$reportRow['file_agreement_no'];
			$returnArray[$j]['file_no']=$file_number;
			$returnArray[$j]['file_status']=$file_status;
			...
			$returnArray[$j]['emi_date_array']=$reportRow['loan_emi_date_array'];
			$returnArray[$j]['emi_to_be_paid']=$reportRow['emi_to_be_paid'];
			$returnArray[$j]['emi_date_to_be_paid']=$reportRow['emi_date_to_be_paid'];
			$returnArray[$j]['file_status']=$reportRow['file_status'];
			
			if(!validateForNull($from,$to) && $reportRow['file_status']!=4)
			$returnArray[$j]['emi_paid']=getTotalEmiPaidForLoan($loan_id);
			else if($reportRow['file_status']!=4)
			{
			$yesterday_to=getPreviousDate($to);	
			$deduct_emi_paid = getTotalEmiPaidForLoanUptoDate($loan_id,$yesterday_to);
			$total_emi_paid = getTotalEmiPaidForLoanUptoDate($loan_id,$from);
			$returnArray[$j]['emi_paid']= $total_emi_paid - $deduct_emi_paid;
			}
			else 
			{
				
				$loan_ending_date = $reportRow['loan_ending_date'];
				$closureDetails=getPrematureClosureDetails($file_id);
				$closure_date = $closureDetails['file_close_date'];
				
				$yesterday_to=getPreviousDate($to);	
				
				$deduct_emi_paid = getTotalEmiPaidForLoanUptoDate($loan_id,$yesterday_to);
				
				$sql="SELECT count(*), datediff('$closure_date',MAX(actual_emi_date)) / day(last_day(MAX(actual_emi_date))) FROM fin_loan_emi WHERE loan_id = $loan_id AND actual_emi_date<='$closure_date' GROUP BY loan_id";
				$resul = dbQuery($sql);
				$resulArray = dbResultToArray($resul);
				$emi_to_be_paid_upto_closure = $resulArray[0][0];
				$extra_emi = $resulArray[0][1];
				$returnArray[$j]['emi_paid']= $emi_to_be_paid_upto_closure + $extra_emi - $deduct_emi_paid;
			}
			$returnArray[$j]['emi']=$emi;
			$returnArray[$j]['loan_scheme']=getLoanScheme($loan_id);
			$returnArray[$j]['balance']=$balance;
		  ...
		  if(!validateForNull($agency_id))
{
	...
	{
		$our_companies_string = implode(",",$our_companies);
		$sql=$sql." AND oc_id IN ($our_companies_string) AND agency_id IS NULL  ";
	}
	if(is_array($agencies) && count($agencies)>0)
	{
		$agencies_string = implode(',',$agencies);
		$sql=$sql." AND agency_id IN ($agencies_string) AND oc_id IS NULL ";
	}
	}
}
...
function generalRasidReports($from=NULL,$to=NULL,$city_id=NULL,$area_id=NULL,$agency_id_array=NULL,$payment_mode=NULL,$type_array=NULL,$include_deleted=0) // type_array is comma separated string
{
...
if(isset($to) && validateForNull($to))
	...
	$sql="SELECT emi_payment_id, file_number, file_agreement_no, fin_file.agency_id,fin_file.oc_id, SUM(payment_amount) as payment_amt,fin_loan_emi_payment.paid_by, payment_mode, payment_date, rasid_no, GROUP_CONCAT(fin_loan_emi.loan_emi_id) as loan_emi_array, fin_loan_emi.loan_id, fin_file.file_id, 1 as type, remarks, fin_loan_emi_payment.created_by, fin_loan_emi_payment.last_updated_by, broker_id
...
if(!validateForNull($agency_id_array) || empty($agency_id_array))
{
	...
	{
		$sql=$sql." UNION ALL SELECT emi_payment_id, file_number, file_agreement_no, fin_file.agency_id,fin_file.oc_id, SUM(payment_amount) as payment_amt,fin_loan_emi_payment_deleted.paid_by, payment_mode, payment_date, rasid_no, GROUP_CONCAT(fin_loan_emi.loan_emi_id) as loan_emi_array, fin_loan_emi.loan_id, fin_file.file_id, 4 as type, remarks, fin_loan_emi_payment_deleted.created_by, fin_loan_emi_payment_deleted.last_updated_by, broker_id
	      FROM fin_loan_emi_payment_deleted, fin_loan_emi, fin_loan, fin_file, fin_customer
		  WHERE ";
	if($agency_id_string!="" && validateForNull($agency_id_string) && $oc_id_string=="")
	{
	$sql=$sql." agency_id IN ($agency_id_string) AND ";
...
if(!validateForNull($agency_id_array) || empty($agency_id_array))
{
	...
	$sql=$sql."	 
		  file_status!=3
	 	  AND fin_loan.file_id=fin_file.file_id		
		  AND fin_customer.file_id=fin_file.file_id
		  AND fin_loan_emi_payment_deleted.loan_emi_id=fin_loan_emi.loan_emi_id
		  AND fin_loan_emi.loan_id=fin_loan.loan_id
		  AND our_company_id=$oc_id 
		  GROUP BY rasid_no,loan_id,fin_loan_emi_payment_deleted.date_added,fin_customer.customer_id";	
	}	   
		   
	$sql=$sql." UNION ALL SELECT penalty_id, file_number, file_agreement_no, fin_file.agency_id,fin_file.oc_id, total_amount as payment_amt,fin_loan_penalty.paid_by, payment_mode, paid_date, rasid_no, rasid_type_id as loan_emi_array, fin_loan_penalty.loan_id, fin_file.file_id, 2 as type, 'NA' as remarks, fin_loan_penalty.created_by,  fin_loan_penalty.last_updated_by, broker_id
	...
	if(!validateForNull($agency_id_array) || empty($agency_id_array))
{
	...
	$sql=$sql." HAVING type IN ($type_array)
		   ";		 	  
	$sql=$sql." UNION ALL SELECT file_closed_id, file_number, file_agreement_no, fin_file.agency_id,fin_file.oc_id, amount_paid as payment_amt ,-1 as paid_by, mode, file_close_date, rasid_no, -1  as loan_emi_array,  fin_file_closed.loan_id, fin_file.file_id, 3 as type, remarks, fin_file_closed.closed_by,  fin_file_closed.last_updated_by, broker_id
...
function generalChequeReturnReports($from=NULL,$to=NULL,$agency_id_array=NULL) // type_array is comma separated string
{
...
if(isset($to) && validateForNull($to))
	...
	$sql="SELECT cheque_return_id, bank_id, branch_id, cheque_no,cheque_date, fin_file.file_id,loan_emi_id, cheque_amount, rasid_no, fin_loan_emi_payment_cheque_return.date_added, payment_date, file_number, customer_name, customer_address, (SELECT GROUP_CONCAT(customer_contact_no) FROM fin_customer_contact_no WHERE customer_id = fin_customer.customer_id) as contact_no, broker_name,  fin_loan_emi_payment_cheque_return.created_by, admin_name
	      FROM fin_loan_emi_payment_cheque_return, fin_file, fin_customer, fin_broker, fin_admin
		  WHERE ";
	if($agency_id_string!="" && validateForNull($agency_id_string) && $oc_id_string=="")
	{
	$sql=$sql." agency_id IN ($agency_id_string) AND ";
...
if(!validateForNull($agency_id_array) || empty($agency_id_array))
{
	...
	$sql=$sql."	 
		  file_status!=3
	 	  AND fin_loan_emi_payment_cheque_return.file_id=fin_file.file_id	
		  AND fin_file.file_id=fin_customer.file_id		
		  AND fin_broker.broker_id = fin_file.broker_id
		  AND our_company_id=$oc_id 
		  AND fin_loan_emi_payment_cheque_return.created_by = fin_admin.admin_id
		 ";
...
function generalPaymentRemianderReports($from=NULL,$to=NULL,$remainder_status=0,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$dated=NULL)
{
	if(validateForNull($area_id))
	$area_id_array=explode(",",$area_id);
	else
...
$sql="SELECT remainder_id as id, date as date, remarks, fin_file.file_id, file_number, file_agreement_no, IF(1 = 1, 'payment', ' ') AS remainder_type, remainder_status
      FROM fin_file_remainder
	  INNER JOIN fin_file
	  ON fin_file.file_id=fin_file_remainder.file_id
	  INNER JOIN fin_customer
...
function EMIPaymentDatesComparatorForEmiReportsDesc($a,$b){
	$aEMIDate=$a['payment_date'];
	$bEMIDate=$b['payment_date'];
	$aEMIDate = str_replace('/', '-', $aEMIDate);
	$aEMIDate=date('Y-m-d',strtotime($aEMIDate));
...
function EMIAreaNameComparatorForEmiReports($a,$b){
	$aEMIDate=$a['customer']['area_name'];
	$bEMIDate=$b['customer']['area_name'];
	return strnatcmp($aEMIDate,$bEMIDate);
}
...
function generalVehicleReports($from_given=NULL,$to_given=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL,$from_received=NULL,$to_receievd=NULL,$rto_agent_id=NULL,$docs_type_array=NULL,$docs_with=NULL,$date_filter_type=0,$vehicle_docs_added=2) // 0 = by entry date, 1 = agent given_date, 2 =customer_given_date
{
	if(validateForNull($area_id))
	$area_id_array=explode(",",$area_id);
	else
...
if(isset($from_given) && validateForNull($from_given))
{
	$from_given = str_replace('/', '-', $from_given);
		$from_given=date('Y-m-d',strtotime($from_given));
	}
if(isset($to_given) && validateForNull($to_given))
{
	$to_given = str_replace('/', '-', $to_given);
		$to_given=date('Y-m-d',strtotime($to_given));
	}	
	
$sql="SELECT fin_file.file_id , file_number, fin_file.date_added, fin_file.created_by, admin_username,city_id, fin_vehicle.vehicle_id, vehicle_reg_no, vehicle_chasis_no, vehicle_engine_no, rto_papers, passing, permit, insurance,hp,bill,vehicle_key,remarks,broker_name, fin_customer.customer_id, GROUP_CONCAT(fin_vehicle_rto_work.rto_work_id) as rto_work_ids, fin_rto_agent.rto_agent_id, rto_agent_name , fin_vehicle_docs.vehicle_docs_id
      FROM fin_file
	  INNER JOIN fin_customer
	  ON fin_customer.file_id=fin_file.file_id
	  INNER JOIN fin_broker
	  ...
	  LEFT JOIN fin_vehicle_docs
	  ON fin_file.file_id = fin_vehicle_docs.file_id
	  LEFT JOIN fin_admin
	  ON fin_vehicle_docs.last_updated_by=admin_id
	  LEFT JOIN fin_vehicle_rto_work
	  ON fin_vehicle_rto_work.vehicle_id = fin_vehicle_docs.vehicle_id
	  LEFT JOIN fin_rto_agent
	  ON fin_vehicle_docs.rto_agent_id = fin_rto_agent.rto_agent_id
	  WHERE 1 AND file_status!=3
		  AND 
	  ";	  	
if($our_company_id=="NULL" && is_numeric($agency_id))
...
if(!validateForNull($agency_id))
{
	...
	if(isset($from_given) && validateForNull($from_given))
	$sql=$sql."fin_file.date_added>='$from_given' 
		  AND ";
	if(isset($to_given) && validateForNull($to_given))  
	$sql=$sql."fin_file.date_added<='$to_given'
		  AND ";
		  
	}
	else if($date_filter_type==1)
	{
		
		if(isset($from_given) && validateForNull($frfrom_givenom))
	$sql=$sql."fin_vehicle_docs.work_given_date>='$from_given' 
		  AND ";
	if(isset($to_given) && validateForNull($to_given))  
	$sql=$sql."fin_vehicle_docs.work_given_date<='$to_given'
		  AND ";
	}
	else if($date_filter_type==2)
	{
	if(isset($from_given) && validateForNull($from_given))
	$sql=$sql."fin_vehicle_docs.customer_given_date>='$from_given' 
		  AND ";
	if(isset($to_given) && validateForNull($to_given))  
	$sql=$sql."fin_vehicle_docs.customer_given_date<='$to_given'
		  AND ";	
		
	}
	switch($vehicle_docs_added)
	{
		case 0: $sql=$sql." fin_vehicle_docs.vehicle_docs_id IS NULL AND ";
				break;
		case 1: $sql=$sql." fin_vehicle_docs.vehicle_docs_id IS NOT NULL AND ";
				break;
		case 2:
				break;		
		default: break;
	...
	{
	$sql=$sql." (";	
	if((in_array(1,$docs_type_array) && $docs_with>0) || empty($docs_type_array))
	{
		$sql=$sql." rto_papers IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	}
	if((in_array(2,$docs_type_array) && $docs_with>0)  || empty($docs_type_array))
	{
		$sql=$sql." passing IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	}
	if((in_array(3,$docs_type_array) && $docs_with>0)  || empty($docs_type_array))
	{
		$sql=$sql." permit IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	}
	if((in_array(4,$docs_type_array) && $docs_with>0)  || empty($docs_type_array))
	{
		$sql=$sql." insurance IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	}
	if((in_array(5,$docs_type_array) && $docs_with>0)  || empty($docs_type_array))
	{
		$sql=$sql." hp IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	}
	if((in_array(6,$docs_type_array) && $docs_with>0)  || empty($docs_type_array))
	{
		$sql=$sql." bill IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	}
	if((in_array(7,$docs_type_array) && $docs_with>0)  || empty($docs_type_array))
	{
		$sql=$sql." vehicle_key IN ($docs_with)  ";
		if(empty($docs_type_array))
		$sql=$sql." OR ";
		else
		$sql=$sql." AND ";
	...
	$sql=$sql." fin_file.broker_id IN ($broker_string)
		  AND ";	
	if(isset($rto_agent_id) && checkForNumeric($rto_agent_id) && $rto_agent_id>0)  
	$sql=$sql." fin_vehicle_docs.rto_agent_id IN ($rto_agent_id)
		  AND ";		    
	if(isset($city_id) && validateForNull($city_id) && !in_array('0',$area_id_array))  
	$sql=$sql."city_id=$city_id
		  AND ";
		if(isset($area_id) && validateForNull($area_id))  
			$sql=$sql." area_id IN ($area_id)
				  AND ";		  	  	  
	$sql=$sql."	 our_company_id=$oc_id GROUP BY fin_file.file_id
	ORDER BY fin_file.date_added DESC";	 
...
$returnArray=array();
	...
	if(dbNumRows($result)>0)
	{
		for($xy=0;$xy<count($resultArray);$xy++)
		{
			...
			$returnArray[$j]['file_no']=$file_number;
			$returnArray[$j]['file_id']=$reportRow['file_id'];
			$returnArray[$j]['vehicle_id']=$reportRow['vehicle_id'];
			$returnArray[$j]['vehicle_docs_id']=$reportRow['vehicle_docs_id'];
			$returnArray[$j]['customer']=$customer;
			$returnArray[$j]['vehicle_reg_no']=$reportRow['vehicle_reg_no'];
			$returnArray[$j]['vehicle_engine_no']=$reportRow['vehicle_engine_no'];
			$returnArray[$j]['vehicle_chasis_no']=$reportRow['vehicle_chasis_no'];
			...
			$returnArray[$j]['hp']=$reportRow['hp'];
			$returnArray[$j]['bill']=$reportRow['bill'];
			$returnArray[$j]['vehicle_key']=$reportRow['vehicle_key'];
			$returnArray[$j]['remarks']=$reportRow['remarks'];
			$returnArray[$j]['broker_name']=$reportRow['broker_name'];
			$returnArray[$j]['rto_agent_name']=$reportRow['rto_agent_name'];
			$j++;
			}
		return $returnArray;	
...
function generalChequeReceivedReports($from=NULL,$to=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL)
{
	if(validateForNull($area_id))
	$area_id_array=explode(",",$area_id);
	else
...
$sql="SELECT fin_file.file_id , file_number, fin_file.date_added, fin_file.created_by, admin_username,city_id, fin_vehicle.vehicle_id, vehicle_reg_no, vehicle_chasis_no, vehicle_engine_no, rto_papers, passing, permit, insurance,hp,bill,vehicle_key,fin_customer_cheque_details.remarks,broker_name, fin_customer.customer_id, cheque_details_id,required_cheques, cheques_received, ac_no, bank_name, branch_name, loan_starting_date
      FROM fin_file
	  INNER JOIN fin_customer
	  ON fin_customer.file_id=fin_file.file_id
	  INNER JOIN fin_loan
	  ...
	  ON fin_file.file_id = fin_vehicle.file_id
	  LEFT JOIN fin_vehicle_docs
	  ON fin_vehicle_docs.vehicle_id = fin_vehicle.vehicle_id
	  LEFT JOIN fin_customer_cheque_details
	  ON fin_file.file_id = fin_customer_cheque_details.file_id
	  LEFT JOIN fin_bank
	  ON fin_customer_cheque_details.bank_id = fin_bank.bank_id
	  LEFT JOIN fin_bank_branch
	  ON fin_customer_cheque_details.branch_id = fin_bank_branch.branch_id
	  WHERE 1 AND file_status!=3
		  AND 
	  ";	  	
if($our_company_id=="NULL" && is_numeric($agency_id))
...
$returnArray=array();
	...
	if(dbNumRows($result)>0)
	{
		for($xy=0;$xy<count($resultArray);$xy++)
		{
			...
			$returnArray[$j]['remarks']=$reportRow['remarks'];
			$returnArray[$j]['broker_name']=$reportRow['broker_name'];
			$returnArray[$j]['cheque_details_id']=$reportRow['cheque_details_id'];
			$returnArray[$j]['required_cheques']=$reportRow['required_cheques'];
			$returnArray[$j]['cheques_received']=$reportRow['cheques_received'];
			$returnArray[$j]['ac_no']=$reportRow['ac_no'];
			$returnArray[$j]['bank_name']=$reportRow['bank_name'];
			$returnArray[$j]['branch_name']=$reportRow['branch_name'];
...
$sql="SELECT fin_file.file_id,fin_loan.loan_id,file_number,loan_amount,loan_scheme, roi, emi, loan_approval_date,loan_starting_date,loan_duration,broker_name
      FROM fin_file
	  INNER JOIN fin_customer
	  ON fin_customer.file_id=fin_file.file_id
	  INNER JOIN fin_loan
...
function generalSummaryLoanReports($from=NULL,$to=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL)
{
	if(validateForNull($area_id))
	$area_id_array=explode(",",$area_id);
	else
...
$finc_years_array = getFinancialYearsBetweenLoanApprovalDates($from,$to);
...
$sql="SELECT SUM(loan_amount) as total_loan_amount,broker_name, GROUP_CONCAT(fin_file.file_id) as file_id_string, GROUP_CONCAT(fin_loan.loan_id) as loan_id_string, count(DISTINCT fin_loan.loan_id) as total_loans,(SELECT SUM(file_charges) FROM fin_file_charges 
	  WHERE fin_file_charges.file_id = fin_file.file_id GROUP BY fin_file.file_id)
 as total_file_charges, ( SELECT  SUM(total_amount) FROM fin_loan_penalty
WHERE fin_loan_penalty.file_id = fin_file.file_id GROUP BY fin_file.file_id) as total_penalty
      FROM fin_file
	  INNER JOIN fin_loan
	  ON fin_file.file_id=fin_loan.file_id
	  INNER JOIN fin_customer
	  ON fin_file.file_id=fin_customer.file_id
	  INNER JOIN fin_broker
	  ON fin_file.broker_id = fin_broker.broker_id
	  WHERE 1 AND file_status!=3
		  AND 
...
$returnArray=array();
	...
	if(dbNumRows($result)>0)
	{
		...
		{
			$reportRow=$resultArray[$xy];
			$loan_id_string = $reportRow['loan_id_string'];
			$income_array = getIncomeForLoanIdsForFincYear($loan_id_string,$finc_years_array);
			$returnArray[$j]['total_loan_amount']=$reportRow['total_loan_amount'];
			$returnArray[$j]['total_file_charges']=getTotalFileChargesForLoanIdString($loan_id_string);
			$returnArray[$j]['total_penalty']=getTotalPenaltyForLoanIdString($loan_id_string);
			$returnArray[$j]['total_loans']=$reportRow['total_loans'];
			$returnArray[$j]['broker_name']=$reportRow['broker_name'];
			$returnArray[$j]['income_array']=$income_array;
			$j++;
			}
...
function getTotalFileChargesForLoanIdString($loan_id_string)
{
	$sql="SELECT SUM(file_charges) FROM fin_file_charges INNER JOIN fin_loan ON fin_loan.file_id = fin_file_charges.file_id WHERE fin_loan.loan_id IN ($loan_id_string)";
	$result =dbQuery($sql);
	$resultArray = dbResultToArray($result);
	return $resultArray[0][0];
}

function getTotalPenaltyForLoanIdString($loan_id_string)
{
	$sql="SELECT SUM(total_amount) FROM fin_loan_penalty WHERE fin_loan_penalty.loan_id IN ($loan_id_string)";
	$result =dbQuery($sql);
	$resultArray = dbResultToArray($result);
	return $resultArray[0][0];
...
function getIncomeForLoanIdsForFincYear($loan_id_string,$finc_year_array)
{
	
	$loan_id_array=explode(",",$loan_id_string);
	$income_array = array();
	foreach($finc_year_array as $finc_year)
	{
		$income_array[$finc_year[2]] = 0;	
	}
	foreach($loan_id_array as $loan_id)
	{
		...
		{
		$no_of_emis = 0;	
		$no_of_emis=getNoOfEmiBetweenDatesForLoanId($loan_id,$finc_year[0],$finc_year[1]);
		
		$income_array[$finc_year[2]] = round($income_array[$finc_year[2]] + ($no_of_emis*$interest_per_emi),0);
		}
	}
...
function getIncomeForLoanIdForFincYear($loan_id,$finc_year_array)
{
...
function getFinancialYearsBetweenDates($from,$to) // Y-m-d
{
	...
	if($to_month<4)
	$to_year = $to_year-1;
	$finc_dates_array =array();
	for($from_year;$from_year<=$to_year;$from_year++)
	{
		$finc_from_date = $from_year."-04-01";
		$fin_to_date = ($from_year+1)."-03-31";
		$year_format = $from_year." - ".($from_year+1);
		$finc_dates_array[] = array($finc_from_date,$fin_to_date,$year_format);
	}
	
	return $finc_dates_array;
	
}

function getFinancialYearsBetweenLoanApprovalDates($from,$to,$loan_id_array=NULL) // Y-m-d
{
	if(validateForNUll($from,$to))
	{
		if(isset($from) && validateForNull($from))
{
	$from = str_replace('/', '-', $from);
...
if(is_array($loan_id_array) && checkForNumeric($loan_id_array[0]))
{
	$loan_id_string = implode(",",$loan_id_array);
}		
else
$loan_id_string=NULL;
		$sql="SELECT MAX(loan_ending_date) as max_ending_date FROM fin_loan WHERE loan_approval_date >= '$from' AND loan_approval_date <='$to'";
		if(validateForNull($loan_id_string))
		$sql=$sql." AND loan_id IN ($loan_id_string)";
	$result = dbQuery($sql);
	$resultArray = dbResultToArray($result);
	$max_ending_date = $resultArray[0][0];
...
function CapitalAndInterestReports($agency_id,$vehicle_company_id=NULL,$vehicle_model_year=NULL,$customer_group_id=NULL,$vehicle_type=NULL)
{
	...
	if(validateForNull($agency_id))
	{
		...
		$agency_id=$agency_id;
		$our_company_id="NULL";
		$fileAndLoanIDs=getFileIdsAndLoanIdsForAgencyId($agency_id,false,$vehicle_company_id,$vehicle_model_year,$customer_group_id,$vehicle_type);
		}
		else if($type=="oc")
		{
		...
		$our_company_id=$agency_id;
		$agency_id="NULL";	
		$fileAndLoanIDs=getFileIdsAndLoanIdsForOCId($our_company_id,false,$vehicle_company_id,$vehicle_model_year,$customer_group_id,$vehicle_type);
		}
		...
		if(count($fileAndLoanIDs)>0)
			...
			foreach($fileAndLoanIDs as $fileAndLoanID)
			{
				...
				if($balance<0 && ($file_status==1 || $file_status==5))
				{
					$live_files++;
	...
	return array($live_files,$total_capital_left,$tatal_interest_left,$total_company_left,$returnArray);
...
function getVehicleReleasedReports($from=NULL,$to=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$sold=NULL)
{
	$sold=NULL;
	if(validateForNull($area_id))
	$area_id_array=explode(",",$area_id);
...
if(isset($to) && validateForNull($to))
{
	...
	$sql="SELECT seize_id,  seize_date, sold, remarks, fin_vehicle_seize_released.date_added, fin_vehicle_seize_released.date_modified, fin_vehicle_seize_released.created_by, fin_vehicle_seize_released.last_updated_by, fin_vehicle_seize_released.file_id, fin_vehicle_seize_released.vehicle_id , godown_id, seize_remarks, release_date
	      FROM fin_vehicle_seize_released
,fin_file,fin_customer
		  WHERE file_status!=3
		  AND fin_vehicle_seize_released.file_id=fin_customer.file_id
		  AND fin_vehicle_seize_released.file_id=fin_file.file_id
		  AND
		  ";
if($our_company_id=="NULL" && is_numeric($agency_id))
{
...
if(validateForNull($agency_id))
	{
		...
		$agency_id=$agency_id;
		$our_company_id="NULL";
		$fileAndLoanIDs=getFileIdsAndLoanIdsForAgencyId($agency_id,$to);
		$preClosedFileAndLoanIDs = getPreclosedFileIdsAndLoanIdsForAgencyId($agency_id,$from,$to);
		}
		else if($type=="oc")
		{
		...
		$our_company_id=$agency_id;
		$agency_id="NULL";	
		$fileAndLoanIDs=getFileIdsAndLoanIdsForOCId($our_company_id,$to);
			...
			foreach($fileAndLoanIDs as $fileAndLoanID)
			{
				...
				$loan_id=$fileAndLoanID['loan_id'];
					...
					if($loan_id!=0)
					{
						...
						$payment_received = getPaymentForLoanUptoDate($loan_id,$to);
						...
						if(is_array($all_payments) && count($all_payments)>0 && $all_payments!=0)
						{
							...
							$returnArray[$i]['payments_left']=$collection-$payment_received;
...
function interestbetweenDateReportsCash($from=NULL,$to=NULL,$agency_id=NULL)
{
    $oc_id=$_SESSION['adminSession']['oc_id'];
	$original_agency_id=$agency_id;
	if(!validateForNull($from,$to))
...
if(validateForNull($agency_id))
	{
		...
		$agency_id=$agency_id;
		$our_company_id="NULL";
		$fileAndLoanIDs=getFileIdsAndLoanIdsForAgencyIdCash($agency_id,$from,$to);
		$preClosedFileAndLoanIDs = getPreclosedFileIdsAndLoanIdsForAgencyId($agency_id,$from,$to);
		}
		else if($type=="oc")
		{
		$our_company_id=$agency_id;
		$agency_id="NULL";	
		$fileAndLoanIDs=getFileIdsAndLoanIdsForOCIdCash($our_company_id,$from,$to);
		$preClosedFileAndLoanIDs = getPreclosedFileIdsAndLoanIdsForOCId($our_company_id,$from,$to);
		}
				...
				$loan_id=$fileAndLoanID['loan_id'];
					...
					if($loan_id!=0)
					{
						...
						if(is_array($all_payments) && count($all_payments)>0 && $all_payments!=0)
						{
							...
							$returnArray[$i]['interest']=$interest_for_loan;
							$returnArray[$i]['payments_left']=$collection-$payment_received;
							$returnArray[$i]['loan_interest']=$collection-$loan_amount;
							$returnArray[$i]['emis_left']=$loan['loan_duration']-getTotalEmiPaidForLoanUptoDate($loan['loan_id'],$to);
...
function interestbetweenDateReportsMercantile($from=NULL,$to=NULL,$agency_id=NULL)
{
    $oc_id=$_SESSION['adminSession']['oc_id'];
	$original_agency_id=$agency_id;
	if(!validateForNull($from,$to))
...
if(isset($to) && validateForNull($to))
{
	$to = str_replace('/', '-', $to);
	$to=date('Y-m-d',strtotime($to));
	$next_to = getNextDate($to);
	}
else
{
	$to=date('Y-m-d');
	$next_to = getNextDate(strtotime('d/m/Y',strtotime($to)));
	}
...
if(validateForNull($agency_id))
	{
		...
		$agency_id=$agency_id;
		$our_company_id="NULL";
		$fileAndLoanIDs=getFileIdsAndLoanIdsForAgencyIdMercantile($agency_id,$from,$to);
		$nonInterestFileAndLoanIds = getNewNonInterestFileIdsAndLoanIdsForAgencyIdMercantile($agency_id,$from,$to);
		$preClosedFileAndLoanIDs = getPreclosedFileIdsAndLoanIdsForAgencyId($agency_id,$from,$to);
		}
		else if($type=="oc")
		{
		$our_company_id=$agency_id;
		$agency_id="NULL";	
		$fileAndLoanIDs=getFileIdsAndLoanIdsForOCIdMercantile($our_company_id,$from,$to);
		$nonInterestFileAndLoanIds = getNewNonInterestFileIdsAndLoanIdsForOCIdMercantile($our_company_id,$from,$to);
		$preClosedFileAndLoanIDs = getPreclosedFileIdsAndLoanIdsForOCId($our_company_id,$from,$to);
		}
		...
		if(count($fileAndLoanIDs)>0)
		{
			...
			foreach($fileAndLoanIDs as $fileAndLoanID)
			{
				...
				$loan_id=$fileAndLoanID['loan_id'];
					...
					if($loan_id!=0)
					{
						$total_no_of_emis=getNoOfEmiBetweenDatesForLoanId($loan_id,$from,$to);
						$all_payments=getAllPaymentsWithinDates($loan_id,$from,$to);
						$no_of_emis_left=getNoOfEmiAfterDateForLoanId($loan_id,$next_to);
						if(is_array($all_payments) && count($all_payments)>0 && $all_payments!=0)
						{
							foreach($all_payments as $payment)
							{
							...
							$returnArray[$i]['total_emis']=$total_no_of_emis;
							$returnArray[$i]['emi_paid']=$total_emis_paid;
							$returnArray[$i]['emi_left']=$no_of_emis_left;
							$returnArray[$i]['interest']=$interest_for_loan;
			...
			if(count($nonInterestFileAndLoanIds)>0)
		{
			if(!is_numeric($i))
			$i=0;
			foreach($nonInterestFileAndLoanIds as $fileAndLoanID)
			{
				$file_id=0;
				$loan_id=0;
				$total_emis_paid=0;
...
function generalNoticeReports($from=NULL,$to=NULL,$agency_id=NULL,$reg_ad=NULL,$received=NULL)
{
	$oc_id=$_SESSION['adminSession']['oc_id'];
...
$sql="SELECT file_number as file_no,file_agreement_no,notice_date,customer_name,customer_address,guarantor_name,guarantor_address,bucket,bucket_amount,fin_loan_notice.file_id,reg_ad,received,not_received_type_id, notice_type, broker_id FROM fin_loan_notice INNER JOIN fin_file ON fin_loan_notice.file_id = fin_file.file_id WHERE 1=1 AND ";
    if(isset($from) && validateForNull($from))
...
function generalNoticeReportsRegAd($from=NULL,$to=NULL,$agency_id=NULL,$reg_ad=NULL,$received=NULL,$notice_stage=NULL)
{
	$oc_id=$_SESSION['adminSession']['oc_id'];
	$original_agency_id=$agency_id;
	$our_company_id=NULL;
...
$sql="SELECT notice_id,file_number as file_no,file_agreement_no,notice_date,fin_loan_notice.customer_name,fin_loan_notice.customer_address,fin_customer.city_id as customer_city_id,fin_customer.area_id as customer_area_id,fin_loan_notice.guarantor_name,fin_loan_notice.guarantor_address,fin_guarantor.city_id as guarantor_city_id,fin_guarantor.area_id as guarantor_area_id,fin_loan_notice.file_id,reg_ad,received,not_received_type_id,notice_type, notice_stage, broker_id FROM fin_loan_notice INNER JOIN fin_file ON fin_loan_notice.file_id = fin_file.file_id INNER JOIN fin_customer ON fin_customer.file_id = fin_loan_notice.file_id LEFT JOIN fin_guarantor ON fin_guarantor.file_id = fin_file.file_id WHERE 1=1 AND ";
if(isset($from) && validateForNull($from))
	$sql=$sql." notice_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
	...
	if(isset($notice_stage) && checkForNumeric($notice_stage) && $notice_stage>=0)  
	$sql=$sql." notice_stage=$notice_stage
		  AND ";	  
	if(isset($reg_ad) && checkForNumeric($reg_ad) && $reg_ad==1)  
	$sql=$sql." reg_ad!='' AND ";
	else if(isset($reg_ad) && checkForNumeric($reg_ad) && $reg_ad==0)  
...
function generalEMIReportsCompanyDate($from=NULL,$to=NULL,$win_gt=NULL,$win_lt=NULL,$emi_gt=NULL,$emi_lt=NULL,$balance_gt=NULL,$balance_lt=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL,$date_type=NULL,$vehicle_type=NULL,$from_emi_date=NULL,$to_emi_date=NULL,$group_array=NULL,$sort_by_area_name=1,$file_id_array=NULL,$include_legal_case=1,$emi_date_type=NULL) // $date_type=NULL mean by payment_date , 1= loan_approval_date ** $from_emi_date and $to_emi_date are only day part of the date AND emi_date_type = NULL, means normal emi_date , emi_date_type =1 treat it as loan_approval_date
{
...
if(isset($to_emi_date) && validateForNull($to_emi_date))
{
	...
	$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_number, file_agreement_no, emi, file_status, oc_id, agency_id,
		 (SELECT COUNT(fin_loan_emi.loan_emi_id) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date<='$today') as no_of_emis_to_pay,
		 (SELECT COUNT(fin_loan_emi.loan_emi_id) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND company_paid_date IS NOT NULL) as paid_emis,
		  (SELECT COUNT(fin_loan_emi.loan_emi_id) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND company_paid_date IS NULL AND actual_emi_date<='$today') as due_emis,
		   (SELECT SUM(emi_amount) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND company_paid_date IS NOT NULL) as paid_emis_amount,
		  (SELECT SUM(emi_amount) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND company_paid_date IS NULL AND actual_emi_date<='$today') as due_emis_amount,
		   (SELECT MAX(company_paid_date) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND company_paid_date IS NULL) as payment_date,
		  loan_approval_date,area_id, broker_name, vehicle_reg_no, vehicle_engine_no, vehicle_chasis_no, model_id, vehicle_company_id, vehicle_model, (SELECT MAX(legal_notice_id) FROM fin_legal_notice WHERE fin_legal_notice.file_id = fin_file.file_id GROUP BY fin_legal_notice.file_id) as legal_notice_id
	 	  FROM 
	      fin_file 
		  INNER JOIN  fin_loan ON fin_loan.file_id=fin_file.file_id
		  INNER JOIN  fin_loan_emi ON fin_loan.loan_id=fin_loan_emi.loan_id
...
if(!validateForNull($agency_id))
{
	...
	$sql=$sql."	  our_company_id=$oc_id 
		  GROUP BY fin_file.file_id HAVING 1=1 ";
	if(checkForNumeric($win_gt) && $win_gt>0)
	{
		$sql=$sql." AND due_emis>=$win_gt ";
		
	}
	if(checkForNumeric($win_lt) && $win_lt>0)
	{
		$sql=$sql." AND due_emis<=$win_lt ";
