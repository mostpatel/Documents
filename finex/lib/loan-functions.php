...
function insertLoan($amount,$loan_amount_type,$duration,$loan_type,$loan_scheme,$roi,$emi,$loan_approval_date,$loan_starting_date,$file_id,$customer_id,$agency_amount=0,$agency_emi=0,$agency_duration=0,$duration_unit=1)
{
	...
	try
	{
		...
		if(checkForNumeric($amount,$file_id,$customer_id,$loan_type,$loan_amount_type,$loan_scheme,$agency_amount) && $loan_approval_date!=null && $loan_approval_date!="" && $loan_starting_date!=null && $loan_starting_date!="" && $loan_ending_date!=null && $loan_ending_date!="" && !checkForDuplicateLoan($file_id)) // impratant validation && checks for duplicate loan for same file_id
		{
			...
			setCorrectFlatRateOfInterest($loan_id);
			...
			if(getAccountsStatus())
			{
				...
				if($account_settings && $account_settings['include_loan']>0 && strtotime($books_starting_date)<=strtotime($loan_approval_date)) // account_settings is true if accounts_status for agency or oc is set to 1 in database
				{
					...
					{
						if($account_settings['mercantile']==0 || $account_settings['mercantile']==3)
						{
...
function getLatestPaymentDateForLoan($id)
{
	...
	{
		$sql="SELECT max(Payment_date) FROM fin_loan_emi_payment,fin_loan_emi Where fin_loan_emi_payment.loan_emi_id=fin_loan_emi.loan_emi_id AND fin_loan_emi.loan_id=$id GROUP BY fin_loan_emi.loan_id";
...
function getSecondLatestPaymentDateForLoan($id)
{
	if(checkForNumeric($id))
	{
		$sql="SELECT max(Payment_date) FROM fin_loan_emi_payment,fin_loan_emi Where fin_loan_emi_payment.loan_emi_id=fin_loan_emi.loan_emi_id AND fin_loan_emi.loan_id=$id AND payment_date NOT IN (SELECT max(Payment_date) FROM fin_loan_emi_payment,fin_loan_emi Where fin_loan_emi_payment.loan_emi_id=fin_loan_emi.loan_emi_id AND fin_loan_emi.loan_id=$id GROUP BY fin_loan_emi.loan_id) GROUP BY fin_loan_emi.loan_id";
...
function updateLoan($id,$amt,$loan_amount_type,$duration,$loan_type,$loan_scheme,$roi,$emi,$loan_approval_date,$loan_starting_date,$bank_name_array=false,$branch_name_array=false,$cheque_amount_array=false,$cheque_date_array=false,$cheque_no_array=false,$axin_no_array=false,$ledger_id_array=0,$cash_amount=0,$agency_amount=0,$agency_emi=0,$agency_duration=0,$duration_unit=1){
	...
	try
	{
		...
		else if($loan_amount_type==1) 
		{
				...
				setCorrectFlatRateOfInterest($id);
...
function getFullPenaltyTableForLoanId($loan_id,$today=false) // returns a full penalty table responding to each and every emi
{
	...
	if($today==false && $file['file_status']!=4)
	{
		$today=date('Y-m-d');
		}
	else if($file['file_status']==4)
	{
		$closure_details=getPrematureClosureDetails($file_id);
		$today = $closure_details['file_close_date'];
	}	
	else
	{
...
function getFractionEmiPaidAmountForLoan($loan_id)
{
	if(checkForNumeric($loan_id))
	{
		$loan=getLoanById($loan_id);
		if($loan['loan_scheme']==1)
		{
			...
			$totalEMIsPaid=number_format($totalPayment/$loan['emi'],3);
			$whole_emis_paid_amount = (floor($totalEMIsPaid))*$loan['emi'];
			
			return $totalPayment - $whole_emis_paid_amount;
		}
		else if($loan['loan_scheme']==2)
		{
			...
			{
			$extra_payment_remaining_after_slab = $totalPayment;
			}
			
			$slab_emi = $loan_scheme[$i]['emi'];
			$emi_paid_latest_slab = $extra_payment_remaining_after_slab / $slab_emi;
			$whole_emis_paid_amount = (floor($emi_paid_latest_slab))*$slab_emi;
			
			
			return $extra_payment_remaining_after_slab-$whole_emis_paid_amount;
		}
	}
	
}

function getTotalEmiPaidForLoan($loan_id,$upto=false)
{
	if(checkForNumeric($loan_id))
	{
		$loan=getLoanById($loan_id);
...
function getTotalEmiPaidForLoanUptoDate($loan_id,$upto)
{
	if(checkForNumeric($loan_id) && validateForNull($upto))
	{
		$loan=getLoanById($loan_id);
		if($loan['loan_scheme']==1)
		{
			$totalPayment=getTotalPaymentForLoanUptoDate($loan['loan_id'],$upto);
			$totalEMIsPaid=number_format($totalPayment/$loan['emi'],3);
			return $totalEMIsPaid;
		}
		else if($loan['loan_scheme']==2)
...
function getClosureAmountForPercent($loan_id,$percent)
{
	...
	$emis_gone=getNoOfEmiBeforeDateForLoanId($loan_id,$date);
...
function getBucketForLoan($loan_id,$upto=0)
{
		...
		if($bucket>0)  return number_format($bucket,3); 
		else return  0.0;
...
$eirr=(($strt)*12)*10000;
$eirr=round($eirr,2); // reducing rate
$eirr1=round($eirr1,2); // IRR
$eirr/=100;
$eirr1/=100;
$emi=$eirr;
$irr=$eirr1;
...
function getIntPrincBalanceTableForLoan($id)
{
	...
	if(checkForNumeric($amount,$dutation,$roi))
	{
		...
		for($i=0;$i<count($emi_date_array);$i++){
			...
			if($i==0 && $loan['loan_approval_date']==$loan['loan_starting_date'])
			{
			$intr=0;
			$princ_paid=$emi;
			}
			...
			if($i==(count($emi_date_array)-1))
			{
				
				if($amount>0)
				{
					$princ_paid = $princ_paid + $amount;
					$intr = $intr - $amount;
				}
				else
				{
...
function getIntPrincBalanceTableForLoanAndEmiDate($id,$emi_date)
{
	...
	if(checkForNumeric($amount,$dutation,$roi))
	{
		...
		for($i=0;$i<count($emi_date_array);$i++){
			...
			if($i==0 && strtotime($loan['loan_approval_date'])==strtotime($loan['loan_starting_date']))
			{
			$intr=0;
			$princ_paid=$emi;
			}
...
function getIntPrincBalanceTable($amount,$dutation,$roi,$emi)
{
	...
	$returnArray=array();	
		for($i=0;$i<$dutation;$i++){
			...
			$intr=round(((($amount*$dutation*$roi)/(1200*($dutation/12)))/12),2); //interest per month
...
function getFileIdsAndLoanIdsForAgencyId($agency_id,$to=false,$vehicle_company_id=NULL,$vehicle_model_year=NULL,$customer_group_id=NULL,$vehicle_type=NULL,$from_approval_date=NULL)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id , loan_amount
				FROM fin_file 
				...
				LEFT JOIN fin_rel_groups_file ON fin_file.file_id = fin_rel_groups_file.file_id
				 WHERE  agency_id=$agency_id AND file_status!=3  ";
		if($to && validateForNull($to))
		$sql=$sql." AND (file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL )";
		

		if(isset($from_approval_date) && validateForNull($from_approval_date))
		{
			$sql=$sql." AND loan_approval_date>= '$from_approval_date' ";
		}
		if(isset($vehicle_type) && checkForNumeric($vehicle_type))  
	$sql=$sql." AND vehicle_type_id IN ($vehicle_type)
		   ";	
		 if(isset($vehicle_model_year) && checkForNumeric($vehicle_model_year))  
	$sql=$sql." AND vehicle_model IN ($vehicle_model_year)
		   ";	    
		   if(isset($vehicle_company_id) && checkForNumeric($vehicle_company_id))  
	$sql=$sql." AND vehicle_company_id IN ($vehicle_company_id)
		   ";	
		if(isset($customer_group_id) && checkForNumeric($customer_group_id))  
	$sql=$sql." AND group_id IN ($customer_group_id)
		   ";	
...
function getFileIdsAndLoanIdsForAgencyIdMercantile($agency_id,$from,$to)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id, (SELECT COUNT(*) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date>='$from' AND actual_emi_date<='$to' GROUP BY fin_loan.loan_id) as no_of_emis FROM fin_file, fin_loan WHERE fin_file.file_id= fin_loan.file_id AND agency_id=$agency_id AND file_status!=3 AND (file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL ) HAVING no_of_emis>0 ORDER BY loan_approval_date ASC ";
...
function getNewNonInterestFileIdsAndLoanIdsForAgencyIdMercantile($agency_id,$from,$to)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id, (SELECT COUNT(*) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date>='$from' AND actual_emi_date<='$to' GROUP BY fin_loan.loan_id) as no_of_emis FROM fin_file, fin_loan WHERE fin_file.file_id= fin_loan.file_id AND agency_id=$agency_id AND file_status!=3 AND  file_status!=4 AND loan_approval_date>='$from' AND loan_approval_date <='$to' HAVING no_of_emis IS NULL ORDER BY loan_approval_date ASC ";
...
function getNewNonInterestFileIdsAndLoanIdsForOCIdMercantile($oc_id,$from,$to)
{
	if(checkForNumeric($oc_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id, (SELECT COUNT(*) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date>='$from' AND actual_emi_date<='$to' GROUP BY fin_loan.loan_id) as no_of_emis FROM fin_file, fin_loan WHERE fin_file.file_id= fin_loan.file_id AND oc_id=$oc_id AND file_status!=3 AND file_status!=4 AND loan_approval_date>='$from' AND loan_approval_date <='$to' HAVING no_of_emis IS NULL ORDER BY loan_approval_date ASC ";
...
function getFileIdsAndLoanIdsForAgencyIdCash($agency_id,$from,$to)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_status as fil_status, 
		(SELECT SUM(emi_amount) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id GROUP BY fin_loan_emi.loan_id) as total_collection,
		 (SELECT SUM(payment_amount) 
		 FROM fin_loan_emi_payment, fin_loan_emi 
		 WHERE 
		 fin_loan_emi_payment.loan_emi_id = fin_loan_emi.loan_emi_id AND 
		 fin_loan_emi.loan_id = fin_loan.loan_id AND 
		 payment_date<='$to') as total_payments_received 
		 FROM fin_file, fin_loan 
		 WHERE fin_file.file_id= fin_loan.file_id AND 
		 agency_id=$agency_id AND file_status!=3 AND (file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL )
		 HAVING 
		 (SELECT COUNT(*) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date>='$from' AND actual_emi_date<='$to' GROUP BY fin_loan_emi.loan_id)>0 
		 OR (SELECT COUNT(*) FROM fin_loan_emi_payment, fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND fin_loan_emi.loan_emi_id = fin_loan_emi_payment.loan_emi_id AND payment_date>='$from' AND payment_date<='$to' GROUP BY fin_loan_emi.loan_id)>0 
		 OR (SELECT COUNT(*) FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND  file_close_date>='$from' AND file_close_date<='$to' GROUP BY fin_file_closed.file_id)>0   
		 OR fil_status IN (1,5)  
		 OR ( ((total_collection-total_payments_received)>0 OR total_payments_received IS NULL ) AND (fil_status!=4 OR (SELECT file_close_date FROM fin_file_closed WHERE fin_file_closed.file_id=fin_file.file_id AND file_close_date>'$to') IS NOT NULL)) ORDER BY loan_approval_date ASC";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
		if(dbNumRows($result)>0)
		{
...
function getFileIdsAndLoanIdsForOCId($agency_id,$to=false,$vehicle_company_id=NULL,$vehicle_model_year=NULL,$customer_group_id=NULL,$vehicle_type=NULL,$from_approval_date=NULL)
{
	
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id , loan_amount FROM fin_file INNER JOIN  fin_loan ON fin_file.file_id= fin_loan.file_id  LEFT JOIN fin_vehicle ON fin_file.file_id = fin_vehicle.file_id LEFT JOIN fin_rel_groups_file
 ON fin_file.file_id = fin_rel_groups_file.file_id WHERE  oc_id=$agency_id AND file_status!=3 ";
		if(validateForNull($to))
		$sql=$sql." AND (file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL )";
		
		if(validateForNull($from_approval_date))
		$sql=$sql." AND loan_approval_date>='$from_approval_date' ";
		if(isset($vehicle_type) && checkForNumeric($vehicle_type))  
	$sql=$sql." AND vehicle_type_id IN ($vehicle_type)
		   ";	
...
function getFileIdsAndLoanIdsForOCIdMercantile($agency_id,$from,$to)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id, (SELECT COUNT(*) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date>='$from' AND actual_emi_date<='$to' GROUP BY fin_loan.loan_id) as no_of_emis FROM fin_file, fin_loan WHERE fin_file.file_id= fin_loan.file_id AND oc_id=$agency_id AND file_status!=3 AND (file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL ) HAVING no_of_emis>0  ORDER BY loan_approval_date ASC";
...
function getFileIdsAndLoanIdsForOCIdCash($agency_id,$from,$to)
{
	if(checkForNumeric($agency_id))
	{
	$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_status as fil_status, 
		(SELECT SUM(emi_amount) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id GROUP BY fin_loan_emi.loan_id) as total_collection,
		 ...
		 FROM fin_file, fin_loan 
		 WHERE fin_file.file_id= fin_loan.file_id AND 
		 oc_id=$agency_id AND file_status!=3 AND (file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL )
		 HAVING 
		 (SELECT COUNT(*) FROM fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND actual_emi_date>='$from' AND actual_emi_date<='$to' GROUP BY fin_loan_emi.loan_id)>0 
		 OR (SELECT COUNT(*) FROM fin_loan_emi_payment, fin_loan_emi WHERE fin_loan_emi.loan_id = fin_loan.loan_id AND fin_loan_emi.loan_emi_id = fin_loan_emi_payment.loan_emi_id AND payment_date>='$from' AND payment_date<='$to' GROUP BY fin_loan_emi.loan_id)>0 
		 OR (SELECT COUNT(*) FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND  file_close_date>='$from' AND file_close_date<='$to' GROUP BY fin_file_closed.file_id)>0   
...
function getPreclosedFileIdsAndLoanIdsForAgencyId($agency_id,$from=false,$to=false)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id FROM fin_file, fin_loan,fin_file_closed WHERE fin_file.file_id= fin_loan.file_id AND agency_id=$agency_id AND file_status=4 AND fin_file.file_id = fin_file_closed.file_id ";
...
function getTotalPaymentForLoanBetweenDate($loan_id,$from,$to)
{
...
else
{
	...
	if(checkForNumeric($loan_id) && validateForNull($from,$to))
	{
		...
		$sql="SELECT sum(payment_amount)
		      FROM fin_loan_emi_payment
			  INNER JOIN fin_loan_emi
			  WHERE  fin_loan_emi.loan_emi_id = fin_loan_emi_payment.loan_emi_id AND payment_date >= '$from' AND payment_date <='$to' AND loan_id = $loan_id
			  GROUP BY loan_id";
...
function getPenaltyDaysFroEmiId($id,$today=false,$month_mode=0)
{
	if(defined('PENALTY_IN_DAYS'))
	$month_mode = PENALTY_IN_DAYS;
...
$sql="SELECT actual_emi_date
	...
	if(dbNumRows($result)>0)
	{
		...
		else if($month_mode==1)
		{
			...
			if($penalty_days<DAYS_OF_PENALTY_EQUAL_TO_MONTH)
			return 0;
			else if($penalty_days==DAYS_OF_PENALTY_EQUAL_TO_MONTH)
			return 1;
			else if($penalty_days>DAYS_OF_PENALTY_EQUAL_TO_MONTH)
			{
				...
				$penalty_months = $interval->format('%m');	
				
				$penalty_months_additional_days = $interval->format('%d');
				$penalty_years = $interval->format('%y');
				
				
				if($penalty_years>0)
				$penalty_months = $penalty_months + ($penalty_years*12);
				if($penalty_months_additional_days<DAYS_OF_PENALTY_EQUAL_TO_MONTH)
				return $penalty_months;
				else if($penalty_months_additional_days>=DAYS_OF_PENALTY_EQUAL_TO_MONTH)
				return $penalty_months+1;
	...
	if(dbNumRows($result)>0)
	{
		...
		foreach($resultArray as $re)
		{
			...
			if($notExpired)
			{
				...
				{
					$penalty_days = (strtotime($today)-strtotime($penaltyDetails['payment_date']))/$daylen;
					if($month_mode==0)
					$returnArray['UnPaidDetails']['days']= $penalty_days;
					else if($month_mode==1)
					{
						if($penalty_days<DAYS_OF_PENALTY_EQUAL_TO_MONTH)
						$returnArray['UnPaidDetails']['days']= 0;
						else if($penalty_days==DAYS_OF_PENALTY_EQUAL_TO_MONTH)
						$returnArray['UnPaidDetails']['days']= 1;
						else if($penalty_days>DAYS_OF_PENALTY_EQUAL_TO_MONTH)
						{
							
							$datetime1 = date_create($penaltyDetails['payment_date']);
							$datetime2 = date_create($today);
							$interval = date_diff($datetime1,$datetime2);
							$penalty_months = $interval->format('%m');	
							...
							if($penalty_months_additional_days<DAYS_OF_PENALTY_EQUAL_TO_MONTH)
							$returnArray['UnPaidDetails']['days']= $penalty_months;
							else if($penalty_months_additional_days>=DAYS_OF_PENALTY_EQUAL_TO_MONTH)
							$returnArray['UnPaidDetails']['days']= $penalty_months+1;
						}
					...
					if(checkForNumeric($cheque_no,$emi_payment_id) && $payment_mode==2 && $bank_name!=false && $bank_name!="" && $bank_name!=null && $branch!=false && $branch!="" && $branch!=null)
					{
						...
						if(getAccountsStatus())
						{
							...
							if(strtotime($payment_date)>=strtotime($books_starting_date))
							{
								...
								else if($account_settings['mercantile']==3) // cash reducing interest
								{
									$principal_amount=getPrincipalAmountToBeCreditedForEMIPaymentIdReducing($emi_payment_id);
									$interest_amount=$amount-$principal_amount;			
				...
				/* accounts start */
						if($payment_mode==2)
						{
							if(getAccountsStatus())
							{
								...
								{
									$principal_amount=getPrincipalAmountToBeCreditedForEMIPaymentIdReducing($rasid_identifier);
									$interest_amount=$amount-$principal_amount;			
...
function deletePayment($id)
{
	...
	if($payment['payment_mode']==2)
	{
		...
		insertChequeReturn($cheque_details['bank_id'],$cheque_details['branch_id'],$cheque_details['cheque_no'],$cheque_details['cheque_date'],$file_id,$loan_emi_id,$cheque_amount,$id,$cheque_details['payment_cheque_id'],$payment['rasid_no'],$payment['payment_date']);
		}
	}
	$rasid_identifier=getRasidIdentifierForPaymentId($id);	
	$principal_amount=getPrincipalAmountToBeCreditedForEMIPaymentId($id);
	AddDeletedEmiPaymentId($id);
	deletePenaltyForEmiPaymentId($id);
			...
			if($difference<=0)
			{
				...
				if($amount!=$totalPayment) // if amount of rasid is changed
				{
					...
					$emi_payment_id=insertMultiplePayment($loan_emi_id,$amount,$payment_mode,$payment_date,$or_rasid_no,$remarks,$remainder_date,$bank_name,$branch,$cheque_no,$cheque_date,$cheque_return,$ledger_id);
						...
						/* accounts stop */
					$rasidIdentifier=getRasidIdentifierForPaymentId($emi_payment_id);	
					 if($rasidIdentifier==0)
					 $rasidIdentifier=$emi_payment_id;
					 
					return $rasidIdentifier;
...
function getAmountPercentWiseForPaymentId($id)
{
	if(checkForNumeric($id))
	{
		$rasid_identifier=getRasidIdentifierForPaymentId($id);
		$sql="SELECT SUM(payment_amount), actual_emi_date, SUM(payment_amount)/emi_amount as amount_percent FROM fin_loan_emi_payment INNER JOIN fin_loan_emi ON fin_loan_emi.loan_emi_id = fin_loan_emi_payment.loan_emi_id WHERE ";
		if($rasid_identifier==0)
		$sql=$sql."  emi_payment_id=$id OR rasid_identifier=$id";
		else if($rasid_identifier!=0)
		$sql=$sql."  emi_payment_id=$rasid_identifier OR rasid_identifier=$rasid_identifier";
		$sql=$sql." GROUP BY fin_loan_emi.loan_emi_id";
...
function insertChequeReturn($bank_id,$branh_id,$cheque_no,$cheque_date,$file_id,$loan_emi_id,$amount,$payment_id,$cheque_id,$rasid_no,$payment_date)
{
	if(checkForNumeric($bank_id,$branh_id,$cheque_no,$file_id,$loan_emi_id,$amount) && validateForNull($cheque_date,$rasid_no,$payment_date))
	{	
	
	$admin_id=$_SESSION['adminSession']['admin_id'];	
	$sql="INSERT INTO fin_loan_emi_payment_cheque_return (bank_id,branch_id,cheque_no,cheque_date,file_id,loan_emi_id,date_added,created_by,cheque_amount, rasid_no, payment_date)
		  VALUES($bank_id,$branh_id,'$cheque_no','$cheque_date',$file_id,$loan_emi_id,NOW(),$admin_id,$amount,'$rasid_no','$payment_date')";
	dbQuery($sql);
	return "success";	  
	}
	return "error";
...
function getPenaltyDaysForPaymentId($id)
{
	...
	$result=dbQuery($sql);	
		...
		if(isset($payment_date) && $payment_date!="" && $payment_date!=null && $payment_date!=false && (strtotime($last_payment_date)-strtotime($actual_date))/$daylen>0 && (strtotime($payment_date)-strtotime($actual_date))/$daylen>0)
		{
		...
		else if($month_mode==1)
		{
			...
			else if($penalty_days>DAYS_OF_PENALTY_EQUAL_TO_MONTH)
			{
				...
				if($penalty_months_additional_days<DAYS_OF_PENALTY_EQUAL_TO_MONTH)
				$returnArray['days']= $penalty_months;
				else if($penalty_months_additional_days>=DAYS_OF_PENALTY_EQUAL_TO_MONTH)
				$returnArray['days']= $penalty_months+1;
			}
		}
...
function addPenaltyToLoan($days_paid, $paid_date, $payment_mode , $amount_per_day, $or_rasid_no , $paid_by,  $loan_id,$file_id,$emi_payment_id,$total_amount,$rasid_type=1,$paid=1,$bank_name=false,$branch=false,$cheque_no=false,$cheque_date=false,$ledger_id=0,$auto_rasid_no=0)
{
	...
	if(checkForNumeric($loan_id,$file_id,$amount_per_day) && $amount_per_day>=0)
	{
		...
		if(getAccountsStatus())
			{
				if($agency_company_type=="agency")
				{
					...
					$auto_interest_ledger=getAdvanceInterestLedgerIdForAgency($agency_company_type_id);
					$income_ledger=getIncomeLedgerIdForAgency($agency_company_type_id);
					$penalty_income_ledger = getPenaltyLedgerIdForAgency($agency_company_type_id);
				}
				else
				{
					$account_settings=getAccountsSettingsForOC($agency_company_type_id);
					...
					$auto_interest_ledger=getAdvanceInterestLedgerIdForOC($agency_company_type_id);
					$income_ledger=getIncomeLedgerIdForOC($agency_company_type_id);
					$penalty_income_ledger = getPenaltyLedgerIdForOC($agency_company_type_id);
				}
			}
		/* Accounts Stop */
		...
		if($auto_rasid_no==$or_rasid_no && is_numeric($or_rasid_no) && !checkForNumeric($emi_payment_id))
		{
	...
	if(checkForNumeric($amount_per_day) && $paid_date!=null && $paid_date!="")
		{
		  ...
		  /* accounts start */
						if(getAccountsStatus() && (($rasid_type==1 && $account_settings['include_penalty']==1) || ($rasid_type>1 && $account_settings['include_ac']==1)) && $paid==1 && $payment_mode==1)
						{
							$customer_id=getCustomerIdFromLoanId($loan_id);
							if(strtotime($paid_date)>=strtotime($books_starting_date))
							{
								...
								else if($account_settings['mercantile']==0 || $account_settings['mercantile']==3)
								{
									insertReceipt($total_amount,$paid_date,'C'.$customer_id,$cash_ledger_id,$rasid_no.' Entry',3,$penalty_id);	
								}
								
								if(PENALTY_INCOME_JV==1)
								{
									
									addJV($total_amount,$paid_date,'C'.$customer_id,'L'.$penalty_income_ledger,$rasid_no.' Entry-Penalty',5,$penalty_id);
								}
							}
						}
						/* accounts stop */	
						...
						if(getAccountsStatus() && (($rasid_type==1 && $account_settings['include_penalty']==1) || ($rasid_type>1 && $account_settings['include_ac']==1)) && $paid==1)
						{
							...
							if(strtotime($paid_date)>=strtotime($books_starting_date))
							{
								...
								if(PENALTY_INCOME_JV==1)
								{
									...
									addJV($total_amount,$paid_date,'C'.$customer_id,'L'.$penalty_income_ledger,$rasid_no.' Entry-Penalty',5,$id);
								}
							}
						}
						/* accounts stop */	
		...
		if(getAccountsStatus())
			{
				...
				$account_receipts=getReceiptForPenaltyId($id);
				$account_jv = getJVsForPenaltyId($id);
				if($account_receipts!="error")
				{
					foreach($account_receipts as $account_receipt)
					{
...
function calculatePenaltyForLoanByInterest($interest_rate,$loan_id,$uptoDate=false)
{
	...
	if($uptoDate==false && $file['file_status']!=4)
	{
		$uptoDate=date('Y-m-d');
		}
	else if($file['file_status']==4)
	{
		$closure_details=getPrematureClosureDetails($file_id);
		$uptoDate = $closure_details['file_close_date'];
	}	
	else
	{
...
function getAllRasidForLoanId($loan_id)
{
	...
	if(checkForNumeric($loan_id))
	{
	$sql="SELECT SUM(payment_amount) as payment_amount,fin_loan_emi_payment.payment_mode,payment_date,fin_loan_emi_payment.rasid_no,fin_loan_emi_payment.paid_by,fin_loan_emi_payment.remarks,remainder_date,remainder_status,fin_loan_emi_payment.created_by,fin_loan_emi_payment.last_updated_by,fin_loan_emi_payment.date_added,fin_loan_emi_payment.date_modified,IF(rasid_identifier=0,fin_loan_emi_payment.emi_payment_id,rasid_identifier) as payment_identifier, bank_id , branch_id,cheque_no, cheque_date,ledger_id, cheque_return, days_paid, amount_per_day, fin_loan_penalty.total_amount as total_penalty_amount FROM fin_loan_emi_payment LEFT JOIN fin_loan_penalty ON fin_loan_emi_payment.emi_payment_id = fin_loan_penalty.emi_payment_id  LEFT JOIN fin_loan_emi_payment_cheque ON fin_loan_emi_payment.emi_payment_id = fin_loan_emi_payment_cheque.emi_payment_id INNER JOIN  fin_loan_emi ON fin_loan_emi_payment.loan_emi_id  = fin_loan_emi.loan_emi_id WHERE fin_loan_emi.loan_id=$loan_id   GROUP BY payment_identifier ORDER BY payment_date";
...
function reorderPayments($file_id)
{
	...
	$loan_emi_id = getOldestUnPaidEmi($loan_id);	
		...
		$emi_payment_id=insertPayment($loan_emi_id,$payment['payment_amount'],$payment['payment_mode'],date('d/m/Y',strtotime($payment['payment_date'])),str_replace($rasid_prefix,'',$payment['rasid_no']),$payment['remarks'],$payment['remainder_date'],$bank_name,$branch_name,$payment['cheque_no'],$payment['cheque_date'],$payment['cheque_return'],$payment['ledger_id'],$payment['paid_by'],0);
		
		if($payment['total_penalty_amount']>0)
		{
			$loan_id = getLoanIdFromEmiId($loan_emi_id);
			$file_id = getFileIdFromLoanId($loan_id);
			addPenaltyToLoan($payment['days_paid'],date('d/m/Y',strtotime($payment['payment_date'])),$payment['payment_mode'],$payment['amount_per_day'],str_replace($rasid_prefix,'',$payment['rasid_no']),$payment['paid_by'],$loan_id,$file_id,$emi_payment_id,$payment['total_penalty_amount'],1,1,$bank_name,$branch_name,$payment['cheque_no'],$payment['cheque_date'],$payment['ledger_id'],0);
		...
		{
		$emi_payment_id=insertPayment($loan_emi_id,$payment['payment_amount'],$payment['payment_mode'],date('d/m/Y',strtotime($payment['payment_date'])),str_replace($rasid_prefix,'',$payment['rasid_no']),$payment['remarks'],$payment['remainder_date'],NULL,NULL,NULL,NULL,0,0,$payment['paid_by'],0);	
...
function setCorrectFlatRateOfInterest($loan_id)
{
	if(checkForNumeric($loan_id))
	{
		$loan = getLoanById($loan_id);
		...
		$sql="SELECT SUM(emi_amount) FROM fin_loan_emi WHERE loan_id = $loan_id GROUP BY loan_id";
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		
		$total_collection = $resultArray[0][0];
		
		$interst_amount_per_emi = ($total_collection - $loan_amount) / $duration;
		
		$interst_amount_per_year = $interst_amount_per_emi * 12;
		
		$roi = ($interst_amount_per_year / $loan_amount) * 100;
		if(is_numeric($roi) && $roi>=0)
		{
		$sql="UPDATE fin_loan SET roi = $roi WHERE loan_id = $loan_id";
		dbQuery($sql); 
		}
	}
	else return false;
...
function returnLoanErrorPayments($from_date,$agency_id)
{
...
function getAgencyEndingDateForLoan($loan_id)
{
	if(checkForNumeric($loan_id))
	{
		$sql="SELECT SUM(agency_duration) FROM fin_loan_scheme_agency WHERE  fin_loan_scheme_agency.loan_id = $loan_id GROUP BY loan_id";
		$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
	    $agency_duration = $resultArray[0][0];
		
		if(checkForNumeric($agency_duration) && $agency_duration>0)
		{
			$sql=" SELECT DATE_ADD(loan_approval_date,INTERVAL $agency_duration MONTH) AS loan_ending_date FROM fin_loan WHERE loan_id = $loan_id";
			$result = dbQuery($sql);
		$resultArray = dbResultToArray($result);
		return $resultArray[0][0];
