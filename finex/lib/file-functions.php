...
function listNOCFilesForAgency($id){
	...
	try
	{
		...
		$sql="SELECT fin_file.file_id, file_number, file_agreement_no, customer_id, customer_name, customer_address, city_id, area_id, opening_balance, opening_cd
		      FROM fin_file, fin_customer , fin_file_noc
			  WHERE agency_id = $id  AND our_company_id = $oc_id AND file_status!=3 AND fin_file.file_id = fin_customer.file_id AND fin_file.file_id = fin_file_noc.file_id  ";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
		for($i=0;$i<count($resultArray);$i++)
		{
...
function listNOCFilesForOurCompany($id){
	...
	try
	{
		...
		$sql="SELECT fin_file.file_id, file_number, file_agreement_no, customer_id, customer_name, customer_address, city_id, area_id,opening_balance, opening_cd
		      FROM fin_file, fin_customer , fin_file_noc
			  WHERE oc_id = $id  AND our_company_id = $oc_id AND file_status!=3 AND fin_file.file_id = fin_customer.file_id AND fin_file.file_id = fin_file_noc.file_id ";
...
function insertFile($agency_id, $agreement_no, $file_number, $OC_id, $broker_id, $remarks=false, $remainder_date=false,$mem_no=false,$ho_opening_date="1970-01-01"){
...
$current_rasid_counter=getRasidCounterForOCID($our_company_id);
		...
		if(!validateForNull($mem_no))	
		$mem_no="";
		if(!validateForNull($ho_opening_date))
		$ho_opening_date = "1970-01-01";
		else
			{
				
			$ho_opening_date = str_replace('/', '-', $ho_opening_date);
			$ho_opening_date=date('Y-m-d',strtotime($ho_opening_date));
		...
		if(checkForNumeric($OC_id,$broker_id) && $agreement_no_validate && $file_number!=NULL && $file_number!="" && !checkForDuplicateFile($original_agency_id, $agreement_no, $file_number, $OC_id))
		{
			...
			$sql="INSERT INTO fin_file
				  (agency_id, oc_id, file_agreement_no, file_number, our_company_id, broker_id,created_by, last_updated_by, file_status, date_added, date_modified,mem_no, ho_opening_date)
				  VALUES
				  ($agency_id, $our_company_id,  '$agreement_no', '$file_number', $OC_id, $broker_id, $admin_id, $admin_id, $file_status, NOW(), NOW(),'$mem_no','$ho_opening_date')";
			dbQuery($sql);
			$file_id=dbInsertId();	 
...
function updateFile($id,$agency_id, $agreement_no, $file_number,$broker_id,$mem_no=false,$ho_opening_date = "1970-01-01"){
...
$prefix=getPrefixFromOCId($our_company_id);
		...
		$admin_id=$_SESSION['adminSession']['admin_id'];		
		$sql="UPDATE fin_file
		     SET agency_id = $agency_id, oc_id=$our_company_id, file_agreement_no = '$agreement_no', file_number = '$file_number',  last_updated_by = $admin_id, date_modified = NOW(), broker_id=$broker_id, mem_no = '$mem_no', ho_opening_date ='$ho_opening_date' 
			 WHERE file_id=$id";
			dbQuery($sql);
...
function getFileDetailsByFileId($file_id)
{
	...
	{
		$oc_id=$_SESSION['adminSession']['oc_id'];
		$sql="SELECT agency_id, oc_id, file_agreement_no, file_number, our_company_id, broker_id, created_by, file_status, date_added, mem_no, ho_opening_date
		      FROM fin_file
...
function addRemainder($file_id,$date,$remarks,$payment_reminder=0)
{
	if(!checkForNumeric($payment_reminder))
	$payment_reminder=0;
	...
	$sql="INSERT INTO fin_file_remainder(file_id,date,remarks,payment_reminder) VALUE ($file_id,'$date','$remarks',$payment_reminder)";
	$result=dbQuery($sql);
	return "success";
...
function listRemainderForFile($file_id)
{
	...
	if(checkForNumeric($file_id))
	{
		$sql="SELECT remainder_id,date,remarks,remainder_status, payment_reminder FROM fin_file_remainder WHERE file_id=$file_id";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
...
function listRemarksForFile($file_id)
{
	...
	if(checkForNumeric($file_id))
	{
		$sql="SELECT remainder_id,date,remarks,remainder_status,payment_reminder FROM fin_file_remainder WHERE file_id=$file_id AND remainder_status=0 AND payment_reminder=0 ORDER BY date";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
...
function listPaymentRemindersForFile($file_id)
{
	
	if(checkForNumeric($file_id))
	{
		$sql="SELECT remainder_id,date,remarks,remainder_status,payment_reminder FROM fin_file_remainder WHERE file_id=$file_id AND remainder_status=0 AND payment_reminder=1 ORDER BY date";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
...
function listOnlyRemaindersForFile($file_id)
{
	...
	if(checkForNumeric($file_id))
	{
		$sql="SELECT remainder_id,date,remarks,remainder_status,payment_reminder FROM fin_file_remainder WHERE file_id=$file_id AND (date!='1970-01-01' AND date!='0000-00-00')";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
...
function getRemainderById($id)
{
	if(checkForNumeric($id))
	{
		$sql="SELECT remainder_id,date,remarks,file_id,payment_reminder FROM fin_file_remainder WHERE remainder_id=$id";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
...
function editRemainDer($id,$date,$remarks,$payment_reminder=0)
{
	$remarks=clean_data($remarks);
	if(checkForNumeric($id))
	{
		$date = str_replace('/', '-', $date);
			$date=date('Y-m-d',strtotime($date));
		$sql="UPDATE fin_file_remainder SET date='$date', remarks='$remarks', payment_reminder = $payment_reminder WHERE remainder_id=$id";
		$result=dbQuery($sql);
		return "success";
	}		
	else
		...
		if(getAccountsStatus())
			{
				...
				if($mode==2)
				{
					$customer_id=getCustomerIdFromLoanId($loan_id);
							...
							{
								if(($account_settings['mercantile']==2 || $account_settings['mercantile']==3) && $account_settings['include_fc']==1)
								{
								...
								else if($account_settings['mercantile']==0 && $account_settings['include_fc']==1)
								{
									...
									if($interest_earned>0 && $account_settings['include_fc_interest']==1)
