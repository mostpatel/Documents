...
function insertVehicleTypeIfNotDuplicate($vehicle_type){
	
	try
	{
		$vehicle_type=clean_data($vehicle_type);
		$vehicle_type = ucwords(strtolower($vehicle_type));
		$duplicate = checkForDuplicateVehicleType($vehicle_type);
		if(validateForNull($vehicle_type) && !$duplicate)
		{
		$admin_id=$_SESSION['adminSession']['admin_id'];
		$sql="INSERT INTO fin_vehicle_type
		      (vehicle_type, created_by, last_updated_by, date_added, date_modified)
...
function getOthersVehicleTypeId(){
