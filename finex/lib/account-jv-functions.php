...
function getAllJVs()
{
	$sql="SELECT fin_ac_jv.jv_id,fin_ac_jv.amount as total_amount, fin_ac_jv_cd.amount as amount,from_ledger_id,to_ledger_id,from_customer_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified, qty
...
function getJVsForPenaltyId($id)
{
	...
	if(checkForNumeric($id))
	{
		$sql="SELECT fin_ac_jv.jv_id,fin_ac_jv.amount as total_amount, fin_ac_jv_cd.amount as amount,from_ledger_id,to_ledger_id,from_customer_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified
			  FROM fin_ac_jv,fin_ac_jv_cd
			  WHERE fin_ac_jv.jv_id = fin_ac_jv_cd.jv_id AND auto_id=$id AND auto_rasid_type=5";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $resultArray;
...
function addJV($amount,$trans_date,$to_ledger,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0,$qty=0) //$from_ledger should start with C for customer or L for ledger to_ledger = by(debit) and from_ledger = to(credit)  
{
	$admin_id=$_SESSION['adminSession']['admin_id'];
	if(!checkForNumeric($qty))
	$qty=0;
	...
	if(strtotime($trans_date)<strtotime($ac_starting_date) || (PERIOD_RESTRICTION==1 && $auto_rasid_type==0 && (strtotime($trans_date)<strtotime($from_period_date) || strtotime($trans_date)>strtotime($to_period_date) ))) // payment date should be greater than books starting date
	{
		return "date_error";
	}	
	
	if(checkForNumeric($amount,$admin_id,$qty) && validateForNull($trans_date))
	{
			
			$sql="INSERT INTO fin_ac_jv(amount,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,qty)
			VALUES ($amount,$oc_id,$auto_rasid_type,$auto_id,'$trans_date','$remarks',$admin_id,$admin_id,NOW(),NOW(),$qty)";
...
function addMultiJV($trans_date,$to_ledger_id_array,$to_ledger_amount_array,$from_ledger_id_array,$from_ledger_amount_array,$remarks,$auto_rasid_type=0,$auto_id=0,$qty=0) //$from_ledger should start with C for customer or L for ledger to_ledger = by(debit) and from_ledger = to(credit)  
{
...
function updateMultiJV($id,$trans_date,$to_ledger_id_array,$to_ledger_amount_array,$from_ledger_id_array,$from_ledger_amount_array,$remarks,$auto_rasid_type=0,$auto_id=0,$qty=0) //$from_ledger should start with C for customer or L for ledger to_ledger = by(debit) and from_ledger = to(credit)  
{
	...
	if(checkForNumeric($amount,$admin_id,$qty) && validateForNull($trans_date))
	{
			...
			$sql="UPDATE  fin_ac_jv SET amount = $amount,trans_date = '$trans_date',remarks = '$remarks',last_updated_by = $admin_id,date_modified = NOW(), qty = $qty
			WHERE jv_id = $id";
...
function updateJV($id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,$qty=0)
{
	$admin_id=$_SESSION['adminSession']['admin_id'];
	...
	$from_period_date = $period[0];
	$to_period_date = $period[1];
	if(strtotime($trans_date)<strtotime($ac_starting_date) || (PERIOD_RESTRICTION==1 && $old_payment['auto_rasid_type']==0 && (strtotime($trans_date)<strtotime($from_period_date) || strtotime($trans_date)>strtotime($to_period_date) ))) // payment date should be greater than books starting date
	{
		return "date_error";
	}	
	
		
	if(checkForNumeric($amount,$admin_id,$id,$qty) && validateForNull($trans_date))
	{
			if(isset($trans_date) && validateForNull($trans_date))
			{
			$trans_date = str_replace('/', '-', $trans_date);
...
if(isset($to) && validateForNull($to))
{
	...
	if($from_ledger==-1)
	$sql=$sql." GROUP BY fin_ac_jv.jv_id";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
	return $resultArray;
