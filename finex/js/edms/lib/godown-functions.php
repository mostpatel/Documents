...
function listGodowns(){
	...
	try
	{
		...
		$sql="SELECT godown_id, godown_name, godown_address, edms_city.city_id, city_name
		  FROM edms_godown,edms_city
		  WHERE edms_godown.city_id=edms_city.city_id AND our_company_id = $our_company_id
		  ORDER BY godown_id";
		$result=dbQuery($sql);	 
...
function insertGodown($godown_name,$godown_address,$city_id,$contact_no){
	...
	try
	{
		...
		$duplcate=checkForDuplicateGodown($godown_name,$city_id);
		if($godown_name!=null && $godown_name!=''  && checkForNumeric($city_id) && !checkForDuplicateGodown($godown_name,$city_id))
			{
...
function updateGodown($id,$godown_name,$godown_address,$city_id,$contact_no){
	...
	try
	{
		...
		$godown_name = ucwords(strtolower($godown_name));
		$godown_address=clean_data($godown_address);
		if($godown_name!=null && $godown_name!=''  && checkForNumeric($city_id,$id) && !checkForDuplicateGodown($godown_name,$city_id,$id))
			{
...
function getGodownById($id){
	...
	try
	{
		$sql="SELECT godown_id, godown_name, godown_address, edms_city.city_id, our_company_id, city_name
		  FROM edms_godown,edms_city
		  WHERE edms_godown.city_id=edms_city.city_id
		  AND godown_id=$id";
		$result=dbQuery($sql);	 
		$resultArray=dbResultToArray($result);
		if(dbNumRows($result)>0)
...
function checkForDuplicateGodown($name,$city_id,$id=false)
{
	$our_company_id=$_SESSION['edmsAdminSession']['oc_id'];
