...
function checkForNSItemsInArray($item_id_array,$amount_array,$discount_array,$tax_group_id_array)
{
	$total_amount=0;
	$has_items=false;
...
function insertNonStockToPurchase($item_id,$amount,$discount,$purchase_id,$tax_group_id)
{
	...
	if(checkForNumeric($item_id,$amount,$discount,$tax_group_id) &&  $item_id>0  && $discount>=0  && $tax_group_id>=0)
			{
				...
				$sql="INSERT INTO edms_ac_purchase_nonstock (item_id,amount,discount,net_amount,purchase_id,created_by,last_updated_by,date_added,date_modified) VALUES ($item_id,$amount,$discount,$nett_amount,$purchase_id,$admin_id,$admin_id,NOW(),NOW())";
				dbQuery($sql);
				return dbInsertId();
			}	
		return false;		
...
function insertNonStocksToPurchase($item_id_array,$amount_array,$discount_array,$purchase_id,$tax_group_array)
{
	...
	if(is_array($item_id_array) && count($item_id_array)>0)
	{
		for($i=0;$i<count($item_id_array);$i++)
		{
			...
			if(checkForNumeric($item_id,$amount,$discount,$purchase_id,$tax_group_id) && $item_id>0 && $amount>=0 && $discount>=0 &&  $tax_group_id>=0)
			{
				$net_amount = $amount - $amount*($discount/100);
				
				$purchase_item_id=insertNonStockToPurchase($item_id,$amount,$discount,$purchase_id,$tax_group_id);
