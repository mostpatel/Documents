...
{
	$sql="SELECT credit_note_id,credit_note_ref_type,credit_note_ref,amount,from_ledger_id,to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,delivery_date,created_by,last_updated_by,date_added,date_modified, retail_tax
...
function insertCreditNote($amount,$trans_date,$delivery_date,$to_ledger,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0,$credit_note_ref="NA",$credit_note_ref_type=0,$retail_tax=0) // $to_ledger should start with C for customer or L for ledger to_ledger: credit, from_ledger: debit
{
	...
	if(checkForNumeric($amount,$from_ledger,$admin_id) && $from_ledger>0 && validateForNull($trans_date,$delivery_date))
	{
			...
			$sql="INSERT INTO edms_ac_credit_note (credit_note_ref_type,credit_note_ref,amount,from_ledger_id,to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,delivery_date,remarks,created_by,last_updated_by,date_added,date_modified, retail_tax)
			VALUES ($credit_note_ref_type,'$credit_note_ref',$amount,$from_ledger,$to_ledger,$to_customer,$oc_id,$auto_rasid_type,$auto_id,'$trans_date','$delivery_date','$remarks',$admin_id,$admin_id,NOW(),NOW(), $retail_tax)";
...
function updateCreditNote($id,$amount,$trans_date,$delivery_date,$to_ledger,$from_ledger,$remarks,$credit_note_ref="NA",$credit_note_ref_type=0, $retail_tax=0) // $to_ledger should start with C for customer or L for ledger
{
	$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT sale_id,SUM(amount),from_ledger_id,to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date, trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year,created_by,last_updated_by,date_added,date_modified
...
if(isset($to) && validateForNull($to))
{
	...
	if(checkForNumeric($to_customer) || (isset($head_type) && checkForNumeric($to_ledger)) && $head_type!=0 && $head_type!=2 && $head_type!=3)
	{
	$sql="SELECT credit_note_id,SUM(amount),from_ledger_id,to_ledger_id,to_customer_id
			  FROM edms_ac_credit_note WHERE ";	  
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT edms_ac_credit_note.credit_note_id, amount ";
	if($head_type==1 || !isset($head_type))
...
if(isset($to) && validateForNull($to))
{
	...
	if((checkForNumeric($to_customer) || (isset($head_type) && checkForNumeric($to_ledger) && $head_type!=0 && $head_type!=2 && $head_type!=3)) && checkForNumeric($month_id,$year))
	{
	$sql="SELECT edms_ac_credit_note.credit_note_id,SUM(amount) as total_amount,from_ledger_id,to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified
			  FROM edms_ac_credit_note WHERE ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
...
if(isset($to) && validateForNull($to))
{
	...
	$sql="SELECT SUM(net_amount) as total_amount, SUM(net_amount)/SUM(quantity) as avg_rate, SUM(quantity) as quantity, CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year
			  FROM edms_ac_credit_note, edms_ac_credit_note_item WHERE edms_ac_credit_note.credit_note_id = edms_ac_credit_note_item.credit_note_id AND ";	  
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
...
if(isset($to) && validateForNull($to))
{
	...
	if(checkForNumeric($item_id) && checkForNumeric($month_id,$year))
	{
	$sql="SELECT edms_ac_sales_item.credit_note_item_id, edms_ac_credit_note.credit_note_id, edms_ac_credit_note_item.amount , net_amount, rate, quantity, discount, from_ledger_id,to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year
			  FROM edms_ac_credit_note_item INNER JOIN  edms_ac_credit_note ON edms_ac_credit_note.credit_note_id = edms_ac_credit_note_item.credit_note_id
WHERE ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
