...
function insertAccessRightsForAdmin($admin_id,$access)
{
	...
	if(is_array($access) && !empty($access))
	{
		foreach($access as $right)
		{
					...
					$sql="INSERT INTO
						  trl_rel_admin_right(admin_id,admin_right_id)
						  VALUES
						  ($admin_id,$right)";
					$result=dbQuery($sql);	  
		}	
...
function deleteAccessRightsForAdminWithoutRead($admin_id)
{
	if(checkForNumeric($admin_id))
	{
	$sql="DELETE FROM trl_rel_admin_right
...
{
	$sql="SELECT count(admin_id) FROM trl_admin WHERE is_active=1";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
...
{
	$sql="SELECT trl_rel_admin_right.admin_right_id,admin_right
		      FROM trl_rel_admin_right,trl_admin_right
			  WHERE admin_id=$id
			  AND trl_rel_admin_right.admin_right_id=trl_admin_right.admin_right_id";
		$result=dbQuery($sql);	  
		$resultArray=dbResultToArray($result);
		return $resultArray;	  
	}
...
function getAllAdminRights()
{
	try{
		$sql="SELECT admin_right_id, admin_right
		      FROM trl_admin_right WHERE admin_right_id<100";
		$result=dbQuery($sql);	  
		$resultArray=dbResultToArray($result);
...
function getAdminUserByID($id)
{
	...
	{
	$sql="SELECT trl_admin.admin_id,admin_name,admin_username,admin_email,last_login,date_added
	      FROM trl_admin,trl_rel_admin_right
		  WHERE trl_admin.admin_id=trl_rel_admin_right.admin_id
		  AND trl_admin.admin_id=$id
		  AND is_active=1";
	$result=dbQuery($sql);	
	$resultArray=dbResultToArray($result);
...
function changePassword($id,$oldpassword,$newPassword)
{
	if(checkForNumeric($id) && validateForNull($oldpassword,$newPassword))
	{
		...
		$sql="UPDATE  trl_admin SET admin_password='$safePassword' WHERE admin_id=$id";
		dbQuery($sql);
		return "success";
		}
	}
