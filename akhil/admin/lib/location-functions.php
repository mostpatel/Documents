...
function listLocations()
    {
        try
        {
            $sql="SELECT location_id,location_name, super_location_id, created_by, last_updated_by, date_added, date_modified
...
function getSuperLocationById($id)
{
	...
	if(checkForNumeric($id))
	{
		 ...
		 $sql="SELECT super_location_name
...
// -- Params : $name
// -- Purpose : 
function insertLocation($name,$super_location_id,$about,$carosal_images)
    {
        try
        {
            ...
            if(validateForNull($name,$about) && checkForNumeric($super_location_id) && !$duplicate && $super_location_id>0)
            {
				
                $admin_id=$_SESSION['adminSession']['admin_id'];
                $sql="INSERT INTO
		      trl_locations (location_name, super_location_id, about, created_by, last_updated_by, date_added, date_modified)
			  VALUES
			  ('$name', $super_location_id, '$about' , $admin_id, $admin_id, NOW(), NOW())";
			  
			  
                $result=dbQuery($sql);
				$location_id=dbInsertId();
				$location_name = str_replace(" ","_",$name);
				 addCarosalImagesToLocation($location_id,$carosal_images,$location_name);
...
// -- Params : $id,$name
// -- Purpose : 
    function updateLocation($id,$name,$super_location_id,$about,$carosal_images)
    {
        try
        {
            $name=clean_data($name);
            ...
            if(validateForNull($name,$about) && checkForNumeric($id,$super_location_id) && !$duplicate)
            {
                $admin_id=$_SESSION['adminSession']['admin_id'];
                $sql="UPDATE trl_locations
			  SET location_name='$name', super_location_id = $super_location_id,about = '$about', last_updated_by=$admin_id, date_modified=NOW()
			  WHERE location_id=$id";
                dbQuery($sql);
				
				$location_name = str_replace(" ","_",$name);
				 addCarosalImagesToLocation($id,$carosal_images,$location_name);
...
// -- Purpose : 
    ...
    {
        $sql="SELECT location_id, location_name, super_location_id, about, created_by, last_updated_by, date_added, date_modified
			  FROM 
			  trl_locations 
			  WHERE location_id=$id";
        $result=dbQuery($sql);
...
function addCarosalImagesToLocation($location_id,$img_array,$location_name){
								...
								if(is_array($img_array['name'])) // if proof has more than one image
								  {
									  ...
									  for($j=0;$j<$images_for_a_proof;$j++)
									  {
										  if($img_array['name'][$j]!="" &&  $img_array['name'][$j]!=null)
										  {
										   ...
										   $imageName=addLocationCarosalImage($location_name."_".$location_id,$imagee);
