...
if(isset($_GET['action']))
{
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=insertHotelPackage($_POST["name"],$_POST['location_id'],$_POST['stars'],$_FILES['thumb_img']);
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateHotelPackage($_POST['lid'],$_POST["name"],$_POST['location_id'],$_POST['stars'],$_FILES['thumb_img']);
				if($result=="success")
				{
		...
		if(isset($_SESSION['adminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=deleteImageForHotel($_GET["lid"],$_GET['id']);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Image Deleted Successfuly!";
				$_SESSION['ack']['type']=2; // 2 for update
				}
				else if($result=="number_error")
				{
					$_SESSION['ack']['msg']="Can not Delete Image! Min One Image Required";
					$_SESSION['ack']['type']=4; // 4 for error
				...
				header("Location: ".WEB_ROOT."admin/hotel_package/index.php?view=details&id=".$_GET['lid']);
				exit;
			}
			else
			{	
