	...
	
	{
		...
		if(is_array($contact_no))
		{
			foreach($contact_no as $no)
			{
				...
				if(checkForNumeric($no[0]) && (preg_match('/^\d{10}$/', $no[0])))
				{
				  sendNewLeadSMS($customer_prefix, $customer_name, $no[0], $admin_name, $admin_number, $admin_email, $type=5);
				}
			}
		}
...
function sendanSMSTemp($mobile_no,$message)
{
	...
	$params = array('APIKEY' => 'Ciqyif78Kbi', 'MobileNo' => '91'.$mobile_no, 'SenderID' => 'ABTTPL', 'Message' => $message , 'ServiceName' => 'PROMOTIONAL_HIGH');
...
function sendNewLeadSMS($customer_prefix, $customer_name, $customer_mobile_no, $admin_name, $admin_number, $admin_email, $type)
{
