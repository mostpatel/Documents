...
function updateEnquiry($id, $discussion, $customer_type_id=-1, $follow_up_date, $budget=0)
{
	...
	try
	{
		...
		$followUpDetails = getFollowUpDetailsByEnquiryId($id);
		$follow_up_id = $followUpDetails[0]['follow_up_id'];	
		$sql="UPDATE ems_enquiry_form
			  SET customer_type_id=$customer_type_id, enquiry_discussion='$discussion', budget=$budget
			  WHERE enquiry_form_id=$id";
		dbQuery($sql);
		
		$sql="UPDATE ems_follow_up SET next_follow_up_date = '$follow_up_date', discussion = '$discussion' WHERE follow_up_id = $follow_up_id";
		dbQuery($sql);
		setReminderDateForEnquiry($id);
		return "success";
		}
		else
		{
