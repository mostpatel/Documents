...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			...
			$followUps=viewFollowUps($_POST['from_date'],$_POST['to_date']);
				...
				$_SESSION['expiredReminderReport']['followUp_array']=$followUps;
				$_SESSION['expiredReminderReport']['from_date']=$_POST['from_date'];
				$_SESSION['expiredReminderReport']['to_date']=$_POST['to_date'];
	...
	else if($_GET['action']=='fromHomeExpiredFollowUps')
	{
		...
		$_SESSION['expiredReminderReport']['from_date']=$from;
		$_SESSION['expiredReminderReport']['to_date']=$to;
