...
require_once "../../lib/get-sms-buttons.php";
...

{
		...
		if($_GET['action']=='editCustomerNote')
	{
		...
		if($_GET['action']=='sendDatabaseSMS')
		{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				
				$customer_id = $_GET["id"];;
				$custom_sms_id = $_GET["state"];
				...
				if(checkForNumeric($contact_no[0]) && strlen($contact_no[0])==10)
					{
				 $result = sendCustomSMSFromDB($customer_name, $message, $contact_no[0]);
				...
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Message has been sent successfully!";
				$_SESSION['ack']['type']=2; // 2 for update
				}
				else
				{
				...
				header("Location: ".WEB_ROOT."admin/customer/index.php?view=details&id=".$_GET["state2"]);
				exit;
			}
			else
			{	
