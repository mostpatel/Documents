...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertEnquiryGroup($_POST["enq_group_name"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Enquiry Group successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteEnquiryGroup($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Enquiry Group deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
				$_SESSION['ack']['msg']="Cannot delete Enquiry Group! Enquiry group already in use!";
		...
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateEnquiryGroup($_POST["lid"], $_POST["enq_group_name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Enquiry Group Name updated Successfuly!";
