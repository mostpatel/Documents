...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertFollowUpType($_POST["follow_up_type"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Follow Up Type successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteFollowUpType($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Follow Up Type deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Follow Up Type! Follow Up Type already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateFollowUpType($_POST["lid"],$_POST["follow_up_type"]);
				if($result=="success")
				{
