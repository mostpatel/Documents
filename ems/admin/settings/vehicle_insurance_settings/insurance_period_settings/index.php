...
require_once "../../../../lib/insurance-period-functions.php";
...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertInsurancePeriod($_POST["period"], $_POST["min_range"], $_POST["max_range"], $_POST["vehicle_type_id"] );
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Period successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteInsurancePeriod($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Period deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete period! Period already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateInsurancePeriod($_POST["lid"],$_POST["period"], $_POST["min_range"], $_POST["max_range"], $_POST["vehicle_type_id"] );
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Period updated Successfuly!";
