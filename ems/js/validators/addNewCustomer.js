...
jQuery("#branch_name").validate({
	...
	jQuery("#branch_name").validate({
				...
				jQuery("#cheque_no").validate({
                    expression: "if (( ($('input[name=loan_amount_type]:checked').val()==2) && (!isNaN(VAL) && VAL.length==6)) || ($('input[name=loan_amount_type]:checked').val()==1)) return true; else return false;",
                    message: "Only 6 digits allowed in Cheque Number!"
                });
