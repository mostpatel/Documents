...
function insertStartJourneyInquiry($name,$email,$phone,$booking_date="01/01/1970",$loc_type=1,$package_type=1,$destination="NA",$no_nights=1,$inquiry="NA",$adult=1,$child=0)
{
	...
	if(validateForNull($name) && checkForNumeric($loc_type,$package_type,$adult,$child) && (validateForNull($email) || (checkForNumeric($phone) && strlen($phone)==10)))
	{
	...
	$package_type=clean_data($package_type);	
	...
	$sql="INSERT INTO trl_start_journey (name, mobile_no, email, message,  booking_date, destination, loc_type_id, no_of_nights, pck_type_id, date_added, adult, child) VALUES ('$name',$phone, '$email', '$inquiry', '$booking_date' , '$destination' , $loc_type, $no_nights, $package_type, NOW(), $adult, $child )";	
	
	dbQuery($sql);
	
	
	
	$message="Name: ".$name."\n Email: ".$email."\n Phone: ".$phone."\n Booking Date: ".$booking_date."\n No Of Nights: ".$no_nights."\n Destination Type: ".getInquiryLocTypeById($loc_type)."\n Destination: ".$destination."\n Adult: ".$adult."\n Child: ".$child."\n Package Type: ".getInquiryPackageTypeById($package_type)."\n Details: ".$inquiry;
	$headers="From:info@bhagwatiholidays.com";
	
	if($loc_type==2) // worldwide
	$to_mail = "bhagwati.holiday@gmail.com";
	else if($loc_type==1) // india
	$to_mail = "bhagwati.hotel@gmail.com";
	
	mail($to_mail,"Holidays Inquiry",$message,$headers);	
	return "success";	
	}
	return "error";
...
function insertHotelBookingInquiry($name,$email,$phone,$booking_date="01/01/1970",$loc_type=1,$hotel_type=1,$destination="NA",$no_nights=1,$inquiry="NA",$adult=1,$child=0)
{
	...
	if(validateForNull($name) && checkForNumeric($loc_type,$hotel_type,$adult,$child) && (validateForNull($email) || (checkForNumeric($phone) && strlen($phone)==10)))
	{
	...
	$sql="INSERT INTO trl_hotel_booking (name, mobile_no, email, message,  booking_date, destination, loc_type_id, no_of_nights, hotel_type, date_added , adult, child) VALUES ('$name',$phone, '$email', '$inquiry', '$booking_date' , '$destination' , $loc_type, $no_nights, $hotel_type, NOW(), $adult, $child )";	
	
	dbQuery($sql);
	
	
	
	$message="Name: ".$name."\n Email: ".$email."\n Phone: ".$phone."\n Booking Date: ".$booking_date."\n Destination Type: ".getInquiryLocTypeById($loc_type)."\n Destination: ".$destination."\n Adult: ".$adult."\n Child: ".$child."\n No Of Nights: ".$no_nights."\n Hotel Type: ".getInquiryHotelTypeById($hotel_type)."\n Details: ".$inquiry;
	$headers="From:info@bhagwatiholidays.com";
