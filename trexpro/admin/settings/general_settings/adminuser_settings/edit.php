...
<td>Access Rights : </td>
<td><?php $admin_rights=listBranches();

$branches=getBranchIdsForAdminId($admin_id);

foreach($admin_rights as $right)
{
	?>
  <input id="<?php echo $right['ledger_id'] ?>" type="checkbox" name="branch[]" value="<?php echo $right['ledger_id'] ?>" class="insertCheckBox" <?php if(in_array($right['ledger_id'],$branches)) { ?> checked="checked"  <?php } ?> /> <label class="insertCheckBoxLabel" for="<?php echo $right['ledger_id'] ?>"><?php echo $right['ledger_name'] ?></label> 
<?php    
}?></td>
</tr>
