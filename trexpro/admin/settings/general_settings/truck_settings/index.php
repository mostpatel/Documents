...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertTruck($_POST["name"],$_POST["truck_no"],$_POST["remarks"],$_POST["owner_ledger_id"]);
				
				if(is_numeric($result))
				{
				$_SESSION['ack']['msg']="Truck successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteTruck($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Truck deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Truck! Truck already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateTruck($_POST["lid"],$_POST["name"],$_POST["truck_no"],$_POST["remarks"],$_POST["owner_ledger_id"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Truck updated Successfuly!";
