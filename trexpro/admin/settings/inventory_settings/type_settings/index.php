...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertItemType($_POST["name"],$_POST['inc_inventory']);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Item Type successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteItemType($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Item Type deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Item Type! Item Type already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateItemType($_POST["lid"],$_POST["name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Item Type updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/itemTypes.js");
