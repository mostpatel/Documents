...
<td>By (Credit)<span class="requiredField">* </span> : </td>
				<td>
                   ...
                   <select id="to_ledger" name="to_ledger_id" >
					...
					{
					?>
                    <option value="<?php echo $ledger['id']; ?>" <?php if((is_numeric($sale['to_ledger_id']) && $ledger['id']=='L'.$sale['to_ledger_id']) || (is_numeric($sale['to_customer_id']) && $ledger['id']=='C'.$sale['to_customer_id'])) { ?> selected="selected" <?php } ?>><?php echo $ledger['name']; ?></option>			
                    <?php	
						}
					 ?>
