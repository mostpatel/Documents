...
$admin_branches = getBranchesForAdminId($_SESSION['edmsAdminSession']['admin_id']);
 ?>
<div class="jvp"><?php if(isset($_SESSION['cIncomingTripReport']['agency_id']) && $_SESSION['cIncomingTripReport']['agency_id']!="") { echo getAgecnyIdOrOCidNameFromAgnecySelectInput($_SESSION['cIncomingTripReport']['agency_id']);  } ?></div>
...
<td>From LR Date  : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cCommisonTripReport']['from'])) echo $_SESSION['cCommisonTripReport']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To LR Date  : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cCommisonTripReport']['to'])) echo $_SESSION['cCommisonTripReport']['to']; ?>"/>	
                 </td>
</tr>
<tr>
<td>From Branch<span class="requiredField">* </span> : </td>
				<td>
					...
					{
					?>
                    <option value="<?php echo $branch['branch_id']; ?>" <?php if($branch['ledger_id']==$_SESSION['cCommisonTripReport']['from_branch_ledger_id']) { ?> selected="selected" <?php } ?>><?php echo $branch['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
...
<td>To Branch<span class="requiredField">* </span> : </td>
				<td>
					...
					{
					?>
                    <option value="<?php echo $branch['ledger_id']; ?>" <?php if($branch['ledger_id']==$_SESSION['cCommisonTripReport']['to_branch_ledger_id']) { ?> selected="selected" <?php } ?>><?php echo $branch['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
                    </select><span class="DateError customError">Please select a Branch!</span>
...
<hr class="firstTableFinishing" />
	...
	<div class="no_print">
 <?php if(isset($_SESSION['cIncomingTripReport']['emi_array']))
{
	
	$emi_array=$_SESSION['cIncomingTripReport']['emi_array'];
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    <div class="showColumns">
        ...
        <input class="showCB" type="checkbox" id="6" checked="checked"  /><label class="showLabel" for="6">Truck No</label>  
        <input class="showCB" type="checkbox" id="7" checked="checked"  /><label class="showLabel" for="7">To Pay</label> 
         <input class="showCB" type="checkbox" id="8" checked="checked"  /><label class="showLabel" for="8">Paid</label> 
          <input class="showCB" type="checkbox" id="9" checked="checked"  /><label class="showLabel" for="9">To Be Billed</label> 
           <input class="showCB" type="checkbox" id="9" checked="checked"  /><label class="showLabel" for="9">Tax</label> 
		...
		foreach($untripped_lrs as $emi)
		{
         ...
         <td class="no_print"><input type="checkbox" class="selectTR" name="selectTR[]" value="<?php echo $emi['lr_id']; ?>" /></td>
           ...
           <td><?php echo $emi['truck_no']; ?></td>
            ...
            <td class="payment_amount"><?php   echo  $emi['tax_amount']; $tax_amount = $tax_amount + $emi['tax_amount']; ?>
            </td>
    ...
    <span class="Total" style="display:block;position:relative;width:15%;">To Pay : <?php if(isset($to_pay)) echo number_format($to_pay); ?></span>
     <span class="Total" style="display:block;position:relative;width:15%;">Paid : <?php if(isset($paid)) echo number_format($paid); ?></span>
      <span class="Total" style="display:block;position:relative;width:15%;">To Be Billed : <?php if(isset($to_be_billed)) echo number_format($to_be_billed); ?></span>
       <span class="Total" style="display:block;position:relative;width:15%;">Tax Amount : <?php if(isset($tax_amount)) echo number_format($tax_amount); ?></span>
<?php  ?>      
</div>
