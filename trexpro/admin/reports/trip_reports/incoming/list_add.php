...
<td>From LR Date  : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cIncomingTripReport']['from'])) echo $_SESSION['cIncomingTripReport']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To LR Date  : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cIncomingTripReport']['to'])) echo $_SESSION['cIncomingTripReport']['to']; ?>"/>	
                 </td>
</tr>
...
<td>From Branch<span class="requiredField">* </span> : </td>
				<td>
					...
					{
					?>
                    <option value="<?php echo $branch['ledger_id']; ?>" <?php if($branch['ledger_id']==$_SESSION['cIncomingTripReport']['from_branch_ledger_id']) { ?> selected="selected" <?php } ?>><?php echo $branch['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
...
<td>To Branch<span class="requiredField">* </span> : </td>
				<td>
					...
					{
					?>
                    <option value="<?php echo $branch['branch_id']; ?>" <?php if($branch['ledger_id']==$_SESSION['cIncomingTripReport']['to_branch_ledger_id']) { ?> selected="selected" <?php } ?>><?php echo $branch['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
                    </select><span class="DateError customError">Please select a Branch!</span>
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
		...
		foreach($untripped_lrs as $emi)
		{
         ...
         <td class="no_print"><input type="checkbox" class="selectTR" name="selectTR[]" value="<?php echo $emi['lr_id']; ?>" /></td>
            ...
            <td><?php echo $emi['trip_memo_no']; ?></td>
             ...
             <td><?php $total_qty = $total_qty + $emi['qty_nos']; echo $emi['qty_nos']; ?></td>
   ...
   <span class="Total" style="display:block;position:relative;width:100%;">Total Amount : <?php  echo number_format($to_pay+$paid+$to_be_billed); ?></span>
    <span class="Total" style="display:block;position:relative;width:100%;">Total Qty : <?php  echo number_format($total_qty); ?></span>
