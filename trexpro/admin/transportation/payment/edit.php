...
$receipts=getAllPaymentsForInvoiceandLedger($invoice_id,$ledger_id);
$jv_cd=getJvCDByInvoiceAndLedgerId($invoice_id,$ledger_id);
$total_receipt_amount = getTotalPaymentAmountForInvoiceLedger($invoice_id,$ledger_id);
...
<input type="hidden" name="ledger_id" value="<?php echo $ledger_customer_id; ?>" />
...
<td>Customer <span class="requiredField">* </span> : </td>
				<td>
					<input type="hidden" id="from_ledger" name="from_ledger_id" value="<?php echo $ledger_customer_id; ?>" /> 
                  <?php if(substr($ledger_customer_id, 0, 1) == 'C') { $customer_id = str_replace("C","",$ledger_customer_id); $customer = getCustomerDetailsByCustomerId($customer_id);  echo $customer['customer_name']; } else echo getLedgerNameFromLedgerId(str_replace("L","",$ledger_customer_id)); ?>
                            </td>
</tr>
