	...
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{			
				...
				$result=addPayment($_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks'],$_POST['auto_rasid_type'],$_POST['invoice_id'],$_POST['ref'],$_POST['ref_type']); // $cheque_return is 0 when inserting a payment			
				
				if(is_numeric($result))
				{
					$_SESSION['ack']['msg']="Payment successfully added!";
					$_SESSION['ack']['type']=1; // 1 for insert
					header("Location: ".WEB_ROOT."admin/transportation/payment/index.php?id=".$_POST["invoice_id"]."&state=".$_POST['ledger_id']);
					exit;
				}
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				...
				header("Location: ".WEB_ROOT."admin/transportation/payment/index.php?id=".$_GET["invoice_id"]."&state=".$_GET['ledger_id']);
				exit;
				}
			}
			else
