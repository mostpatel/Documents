...
if(isset($_GET['action']))
{
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=insertSaleCertificateForVehicle($_POST['delivery_challan_id'],$_POST['cert_date']);
				
				if(is_numeric($result))
				{
				$_SESSION['ack']['msg']="Sale Certificate successfully added!";
				$_SESSION['ack']['type']=1; // 1 for insert
				header("Location: ".WEB_ROOT."admin/customer/vehicle_form21/index.php?view=details&id=".$_POST['delivery_challan_id']);
				exit;
				}
				else{
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				deleteSaleCertificateById($_GET["lid"]);
				
				$_SESSION['ack']['msg']="Sale Certificate deleted Successfuly!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				updateSaleCert($_POST['delivery_challan_id'],$_POST['cert_date']);
				
				$_SESSION['ack']['msg']="Sales Certificate updated Successfuly!";
