...
function insertCustomer($name,$address,$city_id,$area,$pincode,$contact_no,$human_proof_type_id,$proofno,$proofImg,$pan_no="NA",$tin_no="NA",$notes="NA",$opening_balance=0,$opening_cd=0,$cst_no="NA",$service_tax_no="NA",$email=""){	
	try
	{
...
$our_company_id = $oc_id;
		...
		$name = ucwords(strtolower($name));
			...
			if(validateForNull($name,$address,$contact_no) && checkForNumeric($city_id,$pincode,$our_company_id,$area_id)  && !empty($contact_no) && ((!checkForDuplicateCustomer($name,$id) && DUPLICATE_CUSTOMER_NAME==1) || DUPLICATE_CUSTOMER_NAME==0))
			{
				...
				$sql="INSERT INTO 	edms_customer(customer_name,customer_address,customer_pincode,city_id,area_id,pan_no,tin_no,cst_no,service_tax_no,email,notes,oc_id, our_company_id,created_by,last_updated_by,date_added,date_modified)				VALUES 
						('$name', '$address', $pincode, $city_id, $area_id,'$pan_no', '$tin_no', '$cst_no', '$service_tax_no', '$email' , '$notes', $oc_id, $our_company_id, $admin_id, $admin_id, NOW(), NOW())";
				$result=dbQuery($sql);
...
function updateCustomer($id,$name,$address,$city_id,$area,$pincode,$contact_no,$human_proof_type_id,$proofno,$proofImg,$pan_no="NA",$tin_no="NA",$notes="NA",$opening_balance=0,$opening_cd=0,$cst_no="NA",$service_tax_no="NA",$email = ''){
		...
		$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
			...
			if(validateForNull($name,$address) && checkForNumeric($city_id,$area_id) && $contact_no!=null && !empty($contact_no) && ((!checkForDuplicateCustomer($name,$id) && DUPLICATE_CUSTOMER_NAME==1) || DUPLICATE_CUSTOMER_NAME==0) )
			{
				...
				$sql="UPDATE edms_customer
				     SET customer_name = '$name', customer_address = '$address', customer_pincode = $pincode , city_id = $city_id, area_id = $area_id,  pan_no = '$pan_no', tin_no = '$tin_no', cst_no = '$cst_no', service_tax_no = '$service_tax_no', email = '$email', notes='$notes', last_updated_by = $admin_id, date_modified = NOW()
					 WHERE customer_id=$id";
...
function setOpeningBalanceForCustomer($customer_id,$opening_balnce,$opening_balance_cd)
{
	...
	if(checkForNumeric($customer_id,$opening_balnce,$opening_balance_cd,$oc_id))
	{
		...
		if(getOpeningBalanceForCustomer($customer_id))
		{
...
function getCustomerNameByCustomerId($customer_id)
{
	...
	if(checkForNumeric($customer_id))
	{
		$sql="SELECT edms_customer.customer_id,customer_name,customer_address,customer_pincode,city_id, area_id, edms_customer.oc_id,  current_balance, current_balance_cd, pan_no, tin_no,cst_no,service_tax_no,email, notes, IF(opening_balance_id IS NOT NULL,edms_customer_opening_balance.opening_balance,0),IF(opening_balance_id IS NOT NULL,edms_customer_opening_balance.opening_cd,0) FROM edms_customer LEFT JOIN edms_customer_opening_balance ON edms_customer.customer_id = edms_customer_opening_balance.customer_id AND edms_customer_opening_balance.oc_id = $our_company_id
...
function getFullCustomerNameByCustomerID($customer_id)
{
	if(checkForNumeric($customer_id))
	{
		$sql="SELECT CONCAT('C',edms_customer.customer_id) as id, CONCAT(customer_name,' ',IFNULL(vehicle_reg_no,''), ' | [C',edms_customer.customer_id,']') as full_ledger_name FROM edms_customer LEFT JOIN edms_vehicle ON edms_vehicle.customer_id = edms_customer.customer_id WHERE edms_customer.customer_id = $customer_id";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
		$contactNoArray=getCustomerContactNo($customer_id);
