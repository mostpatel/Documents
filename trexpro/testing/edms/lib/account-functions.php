...
function getOpeningBalanceForLedgerArrayForDate($array_id,$date) // return opening balance on date for id, id should start with L or C
{
	...
	(SELECT IF(head_id = $cash_head_id OR head_id = $bank_head_id , -SUM(amount) , SUM(amount) ) FROM edms_ac_payment WHERE oc_id = $oc_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT IF(head_id != $cash_head_id AND head_id != $bank_head_id AND head_id != $tax_head_id AND head_id != $purchase_head_id AND head_id != $sales_head_id, -SUM(amount) , SUM(amount) ) FROM edms_ac_receipt WHERE oc_id = $oc_id AND  ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT IF(head_id != $cash_head_id AND head_id != $bank_head_id, -SUM(edms_ac_jv_cd.amount) , 0 ) FROM edms_ac_jv_cd, edms_ac_jv WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT IF(head_id = $cash_head_id OR head_id = $bank_head_id, -SUM(amount) , 0 ) FROM edms_ac_contra WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
