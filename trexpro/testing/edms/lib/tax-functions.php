...
function insertTaxGroup($name,$display_name,$tax_array)
{
	...
	$group_id=insertTaxGroupName($name,$display_name,$main_tax_in_out);
...
function insertTaxGroupName($name,$display_name,$in_out)
{
	$our_company_id=$_SESSION['edmsAdminSession']['oc_id'];
	$display_name = clean_data($display_name);
	$name = clean_data($name);
	...
	if(validateForNull($name) && strlen($name)<255 && checkForNumeric($in_out,$our_company_id) && !checkForDuplicateTaxGroup($name,$in_out) )
	{
		...
		$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
		$sql="INSERT INTO edms_tax_grp (tax_group_name,display_name, in_out,our_company_id)
		       VALUES('$name' , '$display_name', $in_out, $our_company_id)";
		dbQuery($sql);
		return dbInsertId();	   
...
function editTaxGroupName($id,$name,$display_name,$in_out)
{
	...
	if(validateForNull($name) && strlen($name)<255 && !checkForDuplicateTaxGroup($name,$in_out,$id) && checkForNumeric($id))
	{
		$name=trim($name);
		$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
		$sql="UPDATE  edms_tax_grp
		      SET tax_group_name='$name', display_name = '$display_name'
...
function editTaxGroup($id,$name,$display_name,$area_array)
{
	...
	$result=editTaxGroupName($id,$name,$display_name,$main_tax_in_out);
	if($result!="error")
	{
...
{
	$sql="SELECT edms_tax_grp.tax_group_id,GROUP_CONCAT(tax_id) as taxes_id, in_out,CONCAT(IF(in_out=2,'IN PURCHASE',IF(in_out=1,'OUT',IF(in_out=3,'IN SALE','IN'))), ' ', tax_group_name) as tax_group_name, tax_group_name as unprefixed_name
	      FROM edms_tax_grp
		  LEFT JOIN edms_rel_tax_grp_tax
		  ON edms_tax_grp.tax_group_id=edms_rel_tax_grp_tax.tax_group_id
