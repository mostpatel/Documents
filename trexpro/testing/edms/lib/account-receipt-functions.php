	...
	$oc_id = $sales['oc_id'];
		if($amount>0)
		{
			...
			if(checkForNumeric($result) && $kasar_amount>0) // finanacer rasid
			{
				...
				if($kasar_type==0)
				addJV($kasar_amount,$trans_date,"L".$kasar_ledger,$to_ledger,$remarks,7,$result,$sales['oc_id']);
				else if($kasar_type==1)
				addJV($kasar_amount,$trans_date,$to_ledger,"L".$kasar_ledger,$remarks,7,$result,$sales['oc_id']);
			}
...
function updateReceiptForSalesId($sales_id,$id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,$kasar_amount=0,$kasar_type=0,$oc_id=NULL,$payment_mode_id=-1,$chq_date="01/01/1970",$chq_no="000000",$bank_name="NA",$branch_name="NA") // $to_ledger should start with C for customer or L for ledger
{
	$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
	...
	if(checkForNumeric($id,$sales_id))
		{
			...
			$result = updateReceipt($id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,"NA",0,$oc_id,$payment_mode_id,$chq_date,$chq_no,$bank_name,$branch_name);
			...
			if(checkForNumeric($id) && $kasar_amount>0) // finanacer rasid
			{
				...
				if($kasar_type==0)
				addJV($kasar_amount,$trans_date,"L".$kasar_ledger,$to_ledger,$remarks,7,$id,$sales['oc_id']);
				else if($kasar_type==1)
				addJV($kasar_amount,$trans_date,$to_ledger,"L".$kasar_ledger,$remarks,7,$id,$sales['oc_id']);
			}
...
function insertReceipt($amount,$trans_date,$to_ledger,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0,$receipt_ref="NA",$receipt_ref_type=0,$oc_id=NULL,$payment_mode_id=-1,$chq_date="01/01/1970",$chq_no="000000",$bank_name="NA",$branch_name="NA",$parent_id=0) // $to_ledger should start with C for customer or L for ledger to_ledger: credit, from_ledger: debit
{
	$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
	...
	if(checkForNumeric($amount,$from_ledger,$admin_id) && $from_ledger>0 && validateForNull($trans_date) && (($receipt_ref_type==2 && $receipt_ref>0) || $receipt_ref_type!=2) )
	{
		
			$sql="INSERT INTO edms_ac_receipt (receipt_ref_type,receipt_ref,amount,from_ledger_id,to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,parent_id)
			VALUES ($receipt_ref_type,'$receipt_ref',$amount,$from_ledger,$to_ledger,$to_customer,$oc_id,$auto_rasid_type,$auto_id,'$trans_date','$remarks',$admin_id,$admin_id,NOW(),NOW(),$parent_id)";
...
function insertMultiReceipt($amount_array,$trans_date,$to_ledger_array,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0,$receipt_ref_array="NA",$receipt_ref_type_array=0,$oc_id=NULL,$payment_mode_id=-1,$chq_date="01/01/1970",$chq_no="000000",$bank_name="NA",$branch_name="NA") // $to_ledger should start with C for customer or L for ledger to_ledger: credit, from_ledger: debit
{
	...
	if(checkforInputsMultiReceipt($amount_array,$trans_date,$to_ledger_id_array,$from_ledger,$receipt_ref_array,$receipt_ref_type_array))
	{
		for($i=1;$i<count($to_ledger_id_array);$i++)
		{
			...
			,$remarks,0,0,$receipt_ref_array[$i],$receipt_ref_type_array[$i],$oc_id,$payment_mode_id,$chq_date,$chq_no,$bank_name,$branch_name,$parent_id);
...
function updateReceipt($id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,$receipt_ref="NA",$receipt_ref_type=0,$oc_id=NULL,$payment_mode_id=-1,$chq_date="01/01/1970",$chq_no="000000",$bank_name="NA",$branch_name="NA") // $to_ledger should start with C for customer or L for ledger
{
	$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
