...
function getAllPaymentsForInvoiceandLedger($invoice_id,$ledger_id) // type = 0 or type >100
{
	...
	$sql="SELECT payment_id,payment_ref,amount,to_ledger_id,from_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,created_by,last_updated_by,date_added,date_modified,remarks
			  FROM edms_ac_payment WHERE (auto_rasid_type=11) AND auto_id = $invoice_id";
...
function getTotalPaymentAmountForInvoiceLedger($invoice_id,$ledger_id) // type = 0 or type >100
{
	if(is_numeric($invoice_id))
	{
	if(substr($ledger_id, 0, 1) == 'L') // if the pament is done to a general account ledger
	...
	$sql="SELECT SUM(amount) FROM edms_ac_payment WHERE (auto_rasid_type=11) AND auto_id = $invoice_id ";
			    if(is_numeric($from_ledger))
			  $sql=$sql." AND from_ledger_id = $from_ledger ";
			   if(is_numeric($from_customer))
			  $sql=$sql." AND from_customer_id = $from_customer ";
...
function addPayment($amount,$trans_date,$to_ledger,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0,$payment_ref="NA",$payment_ref_type=0) //$from_ledger should start with C for customer or L for ledger, from_ledger: debit and to_ledger: credit 
...
function addMultiPayment($amount_array,$trans_date,$to_ledger,$from_ledger_array,$remarks,$auto_rasid_type=0,$auto_id=0,$payment_ref="NA",$payment_ref_type=0) //$from_ledger should start with C for customer or L for ledger, from_ledger: debit and to_ledger: credit 
// auto_rasid_type = 2 for financer payment
{
	$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
	$from_ledger = $from_ledger_array[0];
	if(substr($from_ledger, 0, 1) == 'L') // if the pament is done to a general account ledger
	{
		$from_ledger=str_replace('L','',$from_ledger);
		$from_ledger=intval($from_ledger);
	...
	$from_ledger = $from_ledger_array[$i];
		if(substr($from_ledger, 0, 1) == 'L') // if the pament is done to a general account ledger
	{
		$from_ledger=str_replace('L','',$from_ledger);
		$from_ledger=intval($from_ledger);
	...
	if(checkForNumeric($amount,$to_ledger,$admin_id) && $to_ledger>0 && validateForNull($trans_date))
	{
			...
			$sql="INSERT INTO edms_ac_payment (payment_ref_type,payment_ref,amount,from_ledger_id,from_customer_id,to_ledger_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,parent_id)
			VALUES ($payment_ref_type,'$payment_ref',$amount,$from_ledger,$from_customer,$to_ledger,$oc_id,$auto_rasid_type,$auto_id,'$trans_date','$remarks',$admin_id,$admin_id,NOW(),NOW())";
...
function updatePayment($id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,$payment_ref="NA",$payment_ref_type=0)
{
