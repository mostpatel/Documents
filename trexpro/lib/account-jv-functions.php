...
function getAllJVs()
{
	$sql="SELECT edms_ac_jv.jv_id,edms_ac_jv.amount as total_amount, edms_ac_jv_cd.amount as amount,from_ledger_id,to_ledger_id,from_customer_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified
...
function removeDuplicatesForJvIdAndAmountArray($id_array,$amount_array)
{
	
	if(count($id_array)!=count($amount_array))
	return array($id_array,$amount_array);
	else
	{
		$new_combo_array=array();
		$new_id_array=array();
		$new_amount_array = array();
		...
		for($i=0;$i<count($id_array);$i++)
		{
			...
			if(array_key_exists($id,$new_combo_array))
			{
				if(checkForNumeric($amount))
				$new_combo_array[$id] = $new_combo_array[$id] + $amount;
			}
			else if(checkForNumeric($amount))
			$new_combo_array[$id] = $amount;
		}
		
		foreach($new_combo_array as $id => $amount)
		{
			$new_id_array[]= $id;
			$new_amount_array[] = $amount;
		}
		return array($new_id_array,$new_amount_array);
	}
}
...
function getOutSideLabourJVForNonStockId($non_stock_id) // type = 3
{
	...
	{
		$sql="SELECT edms_ac_jv.jv_id,edms_ac_jv.amount as total_amount, edms_ac_jv_cd.amount as amount,MAX(from_ledger_id) as from_ledger_id,MAX(to_ledger_id) as to_ledger_id,MAX(from_customer_id) as from_customer_id,MAX(to_customer_id) as to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified
			  FROM edms_ac_jv,edms_ac_jv_cd
...
function addJV($amount,$trans_date,$to_ledger,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0) //$from_ledger should start with C for customer or L for ledger to_ledger = by(debit) and from_ledger = to(credit)  
{
	...
	if(checkForNumeric($amount,$admin_id) && validateForNull($trans_date))
	{
			...
			$sql="INSERT INTO edms_ac_jv(amount,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified)
...
function addMultiJV($amount,$trans_date,$to_ledger_id_array,$to_ledger_amount_array,$from_ledger_id_array,$from_ledger_amount_array,$remarks,$auto_rasid_type=0,$auto_id=0) //$from_ledger should start with C for customer or L for ledger to_ledger = by(debit) and from_ledger = to(credit)  
{
...
if(isset($to) && validateForNull($to))
{
	...
	$sql="SELECT edms_ac_jv.jv_id,SUM(edms_ac_jv_cd.amount),from_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified
			  FROM edms_ac_jv, edms_ac_jv_cd WHERE edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id AND type=1  ";
	if(isset($from) && validateForNull($from))
...
if(isset($to) && validateForNull($to))
{
	...
	$sql="SELECT edms_ac_jv.jv_id,SUM(edms_ac_jv_cd.amount),to_ledger_id,to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date, trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year,created_by,last_updated_by,date_added,date_modified
			  FROM edms_ac_jv  ,edms_ac_jv_cd WHERE edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id AND type=0 ";	  
	if(isset($from) && validateForNull($from))
	$sql=$sql." AND trans_date>='$from' 
		   ";
	if(isset($to) && validateForNull($to))  
...
if(isset($to) && validateForNull($to))
{
	...
	if((checkForNumeric($from_customer)  || checkForNumeric($from_ledger)) && checkForNumeric($month_id,$year))
	{
	$sql="SELECT edms_ac_jv.jv_id,edms_ac_jv_cd.amount,(SELECT from_ledger_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 1 LIMIT 0,1) as from_ledger_id, (SELECT from_customer_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 1 LIMIT 0,1) as from_customer_id,(SELECT to_ledger_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 0 LIMIT 0,1) as to_ledger_id  , (SELECT to_customer_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 0 LIMIT 0,1) as to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified
			  FROM edms_ac_jv , edms_ac_jv_cd  WHERE edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id  ";
	if(isset($from) && validateForNull($from))
	$sql=$sql." AND trans_date>='$from' 
		   ";
	if(isset($to) && validateForNull($to))  
...
function getJVIdForInvoice($invoice_id)
{
	if(is_numeric($invoice_id))
	{
		$sql="SELECT jv_id FROM edms_ac_jv WHERE auto_id = $invoice_id AND auto_rasid_type=10";
		$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
...
function getDebitCreditDetailsForJv($jv_id)
{
	if(is_numeric($jv_id))
	{
		$sql="SELECT jv_cd_id, type, amount, from_ledger_id, from_customer_id, to_ledger_id, to_customer_id FROM edms_ac_jv_cd WHERE jv_id = $jv_id";
...
function getJvCDByInvoiceAndLedgerId($invoice_id,$ledger_id)
{
	if(is_numeric($invoice_id))
	{
	...
	else if(substr($ledger_id, 0, 1) == 'C') // if the pament is done to a general account ledger
	{
		...
		$sql="SELECT jv_cd_id, type, SUM(edms_ac_jv_cd.amount) as amount, from_ledger_id, from_customer_id, to_ledger_id, to_customer_id FROM edms_ac_jv_cd,edms_ac_jv WHERE edms_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND auto_rasid_type = 10 AND auto_id = $invoice_id ";
