...
function checkForCreditNoteItemsNSInArray($item_id_array,$amount_array,$discount_array,$tax_group_id_array,$ns_type,$out_side_labour_provider_array,$our_rate_array)
{
	$total_amount=0;
	$has_items=false;
...
function insertNonStockToCreditNote($item_id,$amount,$discount,$credit_note_id,$tax_group_id,$ns_type=0,$out_side_labour_provider=0,$our_rate=0)
{
	
	if(checkForNumeric($item_id,$amount,$discount,$credit_note_id,$tax_group_id,$ns_type) && (($ns_type==1 && checkForNumeric($out_side_labour_provider,$our_rate) && $out_side_labour_provider>0 && $our_rate>=0) || $ns_type==0) && $tax_group_id>=0  && $item_id>0 && $amount>=0 && $discount>=0)
			{
				...
				$sql="INSERT INTO edms_ac_credit_note_nonstock (item_id,amount,discount,net_amount,credit_note_id,ns_type,created_by,last_updated_by,date_added,date_modified) VALUES ($item_id,$amount,$discount,$nett_amount,$credit_note_id,$ns_type,$admin_id,$admin_id,NOW(),NOW())";
				dbQuery($sql);
...
function insertNonStocksToCreditNote($item_id_array,$amount_array,$discount_array,$credit_note_id,$tax_group_array,$ns_type=0,$outside_labour_provider_array=NULL,$our_rate_array=NULL)
{
	...
	if(is_array($item_id_array) && count($item_id_array)>0)
	{
		for($i=0;$i<count($item_id_array);$i++)
		{
			...
			if(checkForNumeric($item_id,$amount,$discount,$credit_note_id,$ns_type) && (($ns_type==1 && $outside_labour_provider>0 && $our_rate>=0) || $ns_type==0) && $item_id>0 && $amount>=0 && $discount>=0  && $tax_group_id>=0)
			{
				...
				$credit_note_item_id=insertNonStockToCreditNote($item_id,$amount,$discount,$credit_note_id,$tax_group_id,$ns_type,$outside_labour_provider,$our_rate);
