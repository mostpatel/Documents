...
function generalVehicleReports($from=NULL,$to=NULL,$city_id=NULL,$area_id=NULL,$file_status=NULL,$agency_id=NULL,$broker_string=NULL)
{
	if(validateForNull($area_id))
	$area_id_array=explode(",",$area_id);
	else
...
$sql="SELECT fin_file.file_id , file_number, fin_file.date_added, fin_file.created_by, admin_username,city_id, fin_vehicle.vehicle_id, vehicle_reg_no, vehicle_chasis_no, vehicle_engine_no, rto_papers, passing, permit, insurance,hp,bill,vehicle_key,remarks,broker_name, fin_customer.customer_id
      FROM fin_file
	  INNER JOIN fin_customer
	  ON fin_customer.file_id=fin_file.file_id
	  INNER JOIN fin_broker
