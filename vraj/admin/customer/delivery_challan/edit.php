...
<td>Battery Make<span class="requiredField">* </span> : </td>
				<td>
                   ...
                   <select id="battery_make" name="battery_make" >
					...
					{
					?>
                    <option value="<?php echo $btm['battery_make_id']; ?>" <?php if($btm['battery_make_id']==$vehicle['battery_make_id']) {?> selected="selected" <?php } ?> ><?php echo $vehicle['battery_make']; ?></option>			
                    <?php	
						}
					 ?>
                    </select><span class="DateError customError">Please select a Ledger!</span>
