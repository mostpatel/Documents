...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertTax($_POST["name"],$_POST['display_name'],$_POST['percent'],$_POST['in_out']);
				
				if(is_numeric($result))
				{
				$_SESSION['ack']['msg']="Tax successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteTax($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Tax deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
				$_SESSION['ack']['msg']="Cannot delete Tax! Tax already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateTax($_POST["lid"],$_POST["name"],$_POST['display_name'],$_POST['percent'],$_POST['in_out']);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Tax updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/tax.js");
