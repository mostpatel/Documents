...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertInvoiceType($_POST["name"],$_POST['print_name'],$_POST['invoice_counter'],$_POST['invoice_prefix'],$_POST['type']);
				
				if(is_numeric($result))
				{
				$_SESSION['ack']['msg']="Invoice Type successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteInvoiceType($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Invoice Type deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Invoice Type! Invoice Type already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,$admin_rights)))
			{
				...
				$result=updateInvoiceType($_POST["lid"],$_POST["name"],$_POST['print_name'],$_POST['invoice_counter']);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Invoice Type updated Successfuly!";
