...
<h4 class="headingAlignment">Outwards Inventory JV Details</h4>
<?php 
...
<tr>
	<td >
    	<table width="100%" class="adminContentTable productPurchaseTable" id="productPurchaseTable">
            ...
            <?php $total_credit_amount = 0; if(count($credit_items)>0) { 
			 for($i=1;$i<=count($credit_items);$i++) { 
			$sales_item=$credit_items[$i-1];
			$total_credit_amount = $total_credit_amount + $sales_item['amount'];
			?>
...
<h4 class="headingAlignment">Outwards (Credit) Total : <span id="in_total"><?php echo $total_credit_amount ?> Rs</span></h4>
...
<a href="<?php echo WEB_ROOT."admin/accounts/transactions/credit_inventory_jv" ?>"><input type="button" class="btn btn-success" value="Back"/></a>
</td>
</tr>
