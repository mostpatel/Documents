...
if(is_array($payment[0]) && is_numeric($payment[0]['receipt_id']))
{	

if(count($_SESSION['back_links'])>0)	
array_pop($_SESSION['back_links']);

header("Location: ".WEB_ROOT."admin/accounts/transactions/multi_receipt/index.php?view=details&id=".$payment[0]['parent_id']);
exit;	
}
else if(!is_numeric($payment['receipt_id']))
		{
...
header("Location: ".WEB_ROOT."admin/accounts/transactions/multi_receipt/index.php?view=edit&id=".$payment[0]['parent_id']);
exit;	
}
else if(!is_numeric($payment['receipt_id']))
		{
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{
						...
						if($_POST['sales_id']>0)
						$result = insertReceiptForSalesId($_POST['sales_id'],$_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks'],$_POST['kasar_amount'],$_POST['kasar_type'],$_POST['oc_id'],$_POST['payment_mode_id'],$_POST['cheque_date'],$_POST['cheque_no'],$_POST['bank_name'],$_POST['branch_name']);
						else
						$result=insertReceipt($_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks'],$_POST['auto_rasid_type'],$_POST['sales_id'],$_POST['ref'],$_POST['ref_type'],$_POST['oc_id'],$_POST['payment_mode_id'],$_POST['cheque_date'],$_POST['cheque_no'],$_POST['bank_name'],$_POST['branch_name']); // $cheque_return is 0 when inserting a payment			
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			...
			$result=deleteReceipt($_GET["lid"]);
				...
				header("Location: ".WEB_ROOT."admin/accounts/transactions/payment/index.php?view=details&id=".$_GET['lid']);
				exit;
				}
			}
			else
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updateReceiptForSalesId($_POST['sales_id'],$_POST["lid"],$_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks'],$_POST['kasar_amount'],$_POST['kasar_type'],NULL,$_POST['payment_mode_id'],$_POST['cheque_date'],$_POST['cheque_no'],$_POST['bank_name'],$_POST['branch_name'],$ref,$ref_type);
				}
				else
				{
					$result=updateReceipt($_POST["lid"],$_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks'],$ref,$ref_type,NULL,$_POST['payment_mode_id'],$_POST['cheque_date'],$_POST['cheque_no'],$_POST['bank_name'],$_POST['branch_name']);
				}
				if($result=="success")
				{	
				...
				header("Location: ".WEB_ROOT."admin/accounts/transactions/receipt/index.php?view=details&id=".$sales_id);
				exit;
				}
				else{
...
if(isset($link))
$selectedLink=$link;
$jsArray=array("jquery.validate.js","jquery-ui/js/jquery-ui.min.js","addInsuranceProof.js","customerDatePicker.js","validators/addReceipt_Payment.js","cScript.ashx","transliteration.I.js",'dropdown.js');
