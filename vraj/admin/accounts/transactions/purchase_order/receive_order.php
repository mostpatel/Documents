...
<h4 class="headingAlignment"> Receive Purchase Order </h4>
<?php 
...
?>
<form onsubmit="return submitPayment();" id="addLocForm" action="<?php echo $_SERVER['PHP_SELF'].'?action=receive'; ?>" method="post" enctype="multipart/form-data" >
<input type="hidden" name="id" value="<?php echo $purchase_id; ?>" />
<input type="hidden" name="jv_type" value="2" />
<table id="insertInsuranceTable" class="insertTableStyling no_print">

<tr>
<td width="220px">Received Date<span class="requiredField">* </span> : </td>
				<td>
...
<tr>
<td width="220px">PO Number<span class="requiredField">* </span> : </td>
				<td>
					<input type="text" readonly name="purchase_order_ref" id="purchase_order_ref" value="<?php echo $purchase['purchase_order_ref']; ?>" />
                            </td>
</tr> 
...
<tr>
<td>Supplier<span class="requiredField">* </span> : </td>
				<td>
					<input readonly type="text" id="from_ledger" name="from_ledger_id" value="<?php  if(is_numeric($purchase['from_ledger_id'])) echo getCustomerLedgerNameFromLedgerNameLedgerId('L'.$purchase['from_ledger_id']); else if(is_numeric($purchase['from_customer_id'])) echo getCustomerLedgerNameFromLedgerNameLedgerId('C'.$purchase['from_customer_id']); ?>"/> 
    	...
    	<table width="100%" class="adminContentTable productPurchaseTable" id="productPurchaseTable">
    		<tr>
            	<th>Item Name / Code</th>
                 ...
                 <th>Rate</th>
                 <th>Amount</th>
                 <th style="display:none;">Disc.</th>
                 <th style="display:none;">Tax</th>
                 <th style="display:none;">Nett Amt.</th>
                 <th>Shelf</th>
                 <th></th>
            </tr>
            <tbody style="display:none" id="p0">
            	<tr>
                    <td><input name="item_id[]" type="text" class="inventory_item_autocomplete1" /><br><br>Ordered Qty : <input type="text" name="ordered_qty[]" class="item_quantity" style="width:35px;"   value="1" readonly /></td>
                    <td><input type="text" name="quantity[]" class="item_quantity" style="width:35px;" value="1" onchange="onchangeQuantity(this);" /><span style="color:#f00;font-size:12px;"></span></td>
                     ...
                     <td style="display:none;"><input type="text" name="disc[]" class="item_disc" style="width:25px;" value="0" onchange="onchangeDisc(this);" /> %</td>
                     <td style="display:none;"><select class="tax_group" name="tax_group_id[]" style="width:150px;" onchange="onchangeTaxGroup(this);">
                     ...
                     <td style="display:none;"><input type="text" class="item_net_amount uneditable-input" style="width:75px;" disabled="disabled" value="0" /></td>
                     <td style="display:none;"><select id="godown" name="godown_id[]" style="width:150px;">
			...
			$purchase_item=$purchase_items[$i-1]['purchase_order_item_details'];
			$item_tax_details = $purchase_items[$i-1]['tax_details'];
			$received_qty=getReceivedQtyForItemPurchaseOrder($purchase_item['item_id'],$purchase_id);
			$item = getInventoryItemById($purchase_item['item_id']);
			?>
            <tbody id="p<?php echo $i; ?>">
            	<tr >
                    <td><input name="item_id[]" type="text" class="inventory_item_autocomplete" value="<?php echo getUPCFromItemId($purchase_item['item_id']); ?>" readonly />
                    <br><br><?php echo getFullItemNameFromItemId($purchase_item['item_id'],1); ?><br><br>Ordered Qty : <input type="text" name="ordered_qty[]" class="item_quantity" style="width:35px;"   value="<?php echo $purchase_item['quantity']; ?>" readonly /></td>
                    <td><input type="text" name="quantity[]" class="item_quantity" style="width:35px;"  onchange="onchangeQuantity(this);" value="<?php if($received_qty) echo $received_qty; else echo $purchase_item['quantity']; ?>" /><span style="color:#f00;font-size:12px;"><?php echo getRemainingQuanityForItemForDate($purchase_item['item_id'],$purchase_item['godown_id']); ?></span></td>
                     <td><input type="text" name="rate[]" readonly class="item_rate" style="width:35px;" value="<?php echo $purchase_item['rate']; ?>" onchange="onchangeRate(this);" /></td>
                     ...
                     <td style="display:none;"><input type="text" name="disc[]" class="item_disc" style="width:25px;" value="<?php echo $purchase_item['discount']; ?>" onchange="onchangeDisc(this);" /> %</td>
                     <td style="display:none;"><select class="tax_group" name="tax_group_id[]" style="width:150px;" onchange="onchangeTaxGroup(this);">
                        
                      <?php 
					  
					  $models = listTaxGroups();
									foreach($models as $model)
									{
								 ?>
                     ...
                     <td style="display:none;"><input type="text" class="item_net_amount uneditable-input" style="width:75px;" disabled="disabled" value="<?php echo round($purchase_item['net_amount']+$purchase_item['tax_amount'],2); ?>" /></td>
                     <td><select id="godown" name="godown_id[]" style="width:150px;">
                      ...
                      <?php
					   ...
					   $models = listGodowns();
									...
									{
								 ?>
                                 <option value="<?php echo $model['godown_id'] ?>" <?php if($model['godown_id']==$item['opening_godown_id']){ ?> selected="selected" <?php } ?>  ><?php echo $model['godown_name']; ?></option>
                                 <?php } ?>
                            </select> </td>
            ...
            <tbody id="p<?php echo $i; ?>">
            	<tr >
                    ...
                    <td><input type="text" name="quantity[]" class="item_quantity" style="width:35px;" value="1" onchange="onchangeQuantity(this);" /><span style="color:#f00;font-size:12px;"></span></td>
                     ...
                     <td style="display:none;"><input type="text" class="item_net_amount uneditable-input" style="width:75px;" disabled="disabled" value="0" /></td>
                     <td style="display:none;"><select id="godown" name="godown_id[]" style="width:150px;" onchange="getRateQuantityAndTaxForPurchaseFromGodwonId(this.value,this);">
...
<td width="250px;"></td>
<td>
<input id="disableSubmit" type="submit" value="Receive Purchase Order"  class="btn btn-warning">
