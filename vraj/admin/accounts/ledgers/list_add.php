...
<td width="200px;">Head<span class="requiredField">* </span> : </td>
				<td>
					<select id="head" name="head_id" onchange="generateTaxClass(this.value);">
...
<?php if(TAX_CLASS==1) { ?>
<tr id="tax_class_tr" style="display:none;">
...
<td width="200px;" class="firstColumnStyling">CST NO : </td>
<td> <input type="text" name="cst_no"/> </tr>
</tr>

<tr>
<td> Service Tax Number : </td>
<td> <input type="text" name="service_tax_no"/> </tr>
</tr>

<tr>
<td> Opening Balance on <?php echo date('01/04/Y',strtotime(getCurrentDateForUser($admin_id=$_SESSION['edmsAdminSession']['admin_id']))); ?> : </td>
<td> <input type="text" name="opening_balance" id="opening_balance"/> <select name="opening_balance_cd" class="credit_debit"><option value="0">Debit</option><option value="1">Credit</option> </select> </tr>
</tr>
...
<script>
<?php $purchase_head_id = getPurchaseHeadId();
$sales_head_id = getSalesHeadId();
$tax_class_contant = TAX_CLASS; ?>
document.purchase_head_id = <?php echo $purchase_head_id; ?>;
document.sales_head_id = <?php echo $sales_head_id; ?>;
...
function generateTaxClass(head_id)
{
	if(document.purchase_head_id==head_id || document.sales_head_id==head_id)
	{
		document.getElementById('tax_class_tr').style.display="table-row";
	}
	else
	document.getElementById('tax_class_tr').style.display="none";
