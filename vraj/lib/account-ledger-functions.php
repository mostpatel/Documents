...
function updateLedger($id,$name,$postal_name,$address,$city_id,$area,$pincode,$head_id,$contact_no,$pan_no,$sales_no,$notes,$opening_balance,$opening_balance_cd,$tax_class_id,$cst_no,$service_tax_no){
	...
	try
	{
		...
		if(TAX_CLASS==0 || ($head_id!=$purchase_head_id && $head_id!=$sales_head_id))
		$tax_class_id="NULL";
		else if(!checkForNumeric($tax_class_id) || $tax_class_id<1)
		return "error";
...
function getLedgerIdFromLedgerName($name)
{
try
	{
...
function getNotAvailableSupplier()
{
	$supplier_id = getLedgerIdFromLedgerName('Not Available');
	if(!checkForNumeric($supplier_id))
	return insertLedger('Not Available','','',NULL,NULL,NULL,getSundryCreditorsId(),9999999999,'','','',0,0);
	else
	return $supplier_id;
}	
