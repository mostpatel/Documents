...
function checkForJvItemsInArray($item_id_array,$rate_array,$quantity_array,$godown_id_array,$trans_type=NULL,$trans_id_if_update=NULL)
{
	$total_amount=false;
	$has_items=false;
...
function ConvertUpcArrayInToIdArray($item_name_array,$is_purchase=0)
{
	$item_id_array = array();
	foreach($item_name_array as $item_name){
		
		$item_id=getItemIdFromUPC($item_name);
...
function insertInventoryJV($item_id_array,$rate_array,$quantity_array,$item_id_cd_array,$rate_cd_array,$quantity_cd_array,$trans_date,$remarks,$godown_id_array,$godown_id_cd_array,$jv_type_id=NULL,$inventory_jv_mode=0,$ledger_customer_id=NULL,$po_id="NULL")
{
	...
	if(ITEM_PROMPT=="otc")
	{
	$item_id_array = ConvertUpcArrayInToIdArray($item_id_array);
	$item_id_cd_array = ConvertUpcArrayInToIdArray($item_id_cd_array);
	}
	...
	$trans_type=0;
	$nett_amount_credit = checkForJvItemsInArray($item_id_array,$rate_array,$quantity_array,$godown_id_array,$trans_type);
	$nett_amount_debit = checkForJvItemsInArray($item_id_cd_array,$rate_cd_array,$quantity_cd_array,$godown_id_cd_array,$trans_type);
	
	if($nett_amount_credit=="barcode_transaction_error" || $nett_amount_debit=="barcode_transaction_error")
	return "barcode_transaction_error";
	
	if(!checkForNumeric($po_id))
	$po_id="NULL";
	else
	$duplicate = checkForDUplicatePurchaseOrderReceive($po_id);
	
	if($duplicate)
	{
	deleteInventoryJvForPurchaseOrder($po_id);
	}
	...
	$sql="INSERT INTO edms_inventory_jv(trans_date,remarks,oc_id,created_by,last_updated_by,date_added,date_modified,jv_type_id,inventory_jv_mode,ledger_id,customer_id,purchase_order_id) VALUES ('$trans_date','$remarks',$oc_id,$admin_id,$admin_id,NOW(),NOW(),$jv_type_id,$inventory_jv_mode,$ledger_id,$customer_id,$po_id)";
	...
	insertInventoryItemsToJV($item_id_array,$rate_array,$quantity_array,$inventory_jv_id,$godown_id_array,1); // credit
	
	if($nett_amount_debit && checkForNumeric($nett_amount_debit) && $nett_amount_debit>=0)
	insertInventoryItemsToJV($item_id_cd_array,$rate_cd_array,$quantity_cd_array,$inventory_jv_id,$godown_id_cd_array,0); // debit
	}
	else
	return "error";
...
function updateInventoryNonStockItemJV($inventory_jv_id,$item_id_array,$rate_array,$quantity_array,$item_id_cd_array,$rate_cd_array,$quantity_cd_array,$trans_date,$remarks,$godown_id_array,$godown_id_cd_array,$jv_type_id=NULL,$ledger_customer_id=NULL,$po_id="NULL")
{
	...
	$nett_amount_credit = checkForJvItemsInArray($item_id_array,$rate_array,$quantity_array,$godown_id_array,$trans_type,$inventory_jv_id);
	$nett_amount_debit = checkForJvItemsInArray($item_id_cd_array,$rate_cd_array,$quantity_cd_array,$godown_id_cd_array);
	...
	$sql="UPDATE edms_inventory_jv SET trans_date = '$trans_date' , remarks = '$remarks', last_updated_by = $admin_id, date_modified = NOW(), jv_type_id = $jv_type_id, ledger_id = $ledger_id,  customer_id = $customer_id, purchase_order_id = $po_id WHERE inventory_jv_id = $inventory_jv_id";	
	$result = dbQuery($sql);
...
function insertInventoryItemToJV($item_id,$rate,$quantity,$inventory_jv_id,$godown_id,$type=0,$barcode=NULL) // type 0=debit , 1=credit
{
	...
	if(checkForNumeric($item_id,$rate,$quantity,$inventory_jv_id,$godown_id,$type) && $type>=0 && $godown_id>0 && $item_id>0 && $rate>=0 && $quantity>0)
			{
				...
				if(checkForNumeric($godown_id) && $godown_id>0)
				{
					updateShelfForInventoryItem($item_id,$godown_id);
				}
...
function deleteInventoryJvForPurchaseOrder($inventory_jv_id)
{
	if(checkForNumeric($inventory_jv_id))
	{
		
		$sql="DELETE FROM edms_inventory_jv WHERE purchase_order_id = $inventory_jv_id";
		dbQuery($sql);
		return "success";
	}	
	return "error";
...
function insertInventoryItemsToJV($item_id_array,$rate_array,$quantity_array,$inventory_jv_id,$godown_id_array,$type=0)
{
	...
	if(is_array($item_id_array) && count($item_id_array)>0)
	{
		for($i=0;$i<count($item_id_array);$i++)
		{
			...
			{
				$sales_item_id=insertInventoryItemToJV($item_id,$rate,$quantity,$inventory_jv_id,$godown_id,$type,$barcode);
			}	
...
function checkForDUplicatePurchaseOrderReceive($po_id)
{
	if(checkForNumeric($po_id))
	{
		$sql="SELECT purchase_order_id FROM edms_inventory_jv WHERE purchase_order_id = $po_id";
		$result=dbQuery($sql);
...
function getReceivedQtyForItemPurchaseOrder($item_id,$purchase_order_id)
{
	if(checkForNumeric($item_id,$purchase_order_id))
	{
		$sql="SELECT SUM(quantity) as quantity FROM edms_inventory_item_jv, edms_inventory_jv WHERE edms_inventory_item_jv.inventory_jv_id = edms_inventory_jv.inventory_jv_id AND purchase_order_id = $purchase_order_id AND item_id = $item_id GROUP BY item_id";
		$result=dbQuery($sql);
