	...
	try
	{
		...
		$sql="SELECT item_id,item_name, alias, item_code, CONCAT(item_name, ' | ' , IF(alias!='',CONCAT(alias,' | '),'')  , IF(mrp!='',CONCAT(mrp,' | '),'') , IF(item_code!='',CONCAT(item_code,' | '),''), IF(mfg_item_code!='NA',CONCAT(mfg_item_code,' | '),''), IF(edms_inventory_item.manufacturer_id IS NULL,'',manufacturer_name) ) as full_item_name  , edms_item_type.item_type_id , IF(edms_inventory_item.item_type_id IS NULL,'NA',item_type) as item_type, min_quantity_purchase,  edms_inventory_item.item_unit_id, unit_name,edms_item_manufacturer.manufacturer_id, IF(edms_inventory_item.manufacturer_id IS NULL,'NA',manufacturer_name) as manufacturer_name, mfg_item_code, dealer_price, mrp, tax_group_id, opening_quantity,opening_rate,opening_godown_id,remarks,use_barcode,barcode_prefix,barcode_counter, supplier_id, expiration_date,  edms_inventory_item.created_by, edms_inventory_item.last_updated_by, edms_inventory_item.date_added, edms_inventory_item.date_modified, godown_name
		  ...
		  ON edms_inventory_item.item_type_id = edms_item_type.item_type_id
		   LEFT OUTER JOIN edms_godown
		  ON edms_inventory_item.opening_godown_id = edms_godown.godown_id
		  LEFT OUTER JOIN edms_item_unit
		  ON edms_inventory_item.item_unit_id = edms_item_unit.item_unit_id WHERE inc_inventory=1 AND edms_inventory_item.our_company_id = $our_company_id";
		$result=dbQuery($sql);	 
...
function getItemIdFromFullItemName($item_full_name,$is_purchase=0,$barcode_trans_id = NULL)
{
	...
	$item_full_name = utf8_decode($item_full_name);
	
	$item_full_name = mysql_real_escape_string($item_full_name);
	$item_full_name = trim($item_full_name);
	...
	try
	{
		...
		$sql="SELECT edms_inventory_item.item_id,item_name, alias, item_code, CONCAT(item_name, ' | ' , IF(alias!='',CONCAT(alias,' | '),'')  , IF(mrp!='',CONCAT(mrp,' | '),'') , IF(item_code!='',CONCAT(item_code,' | '),''), IF(mfg_item_code!='NA',CONCAT(mfg_item_code,' | '),''), IF(edms_inventory_item.manufacturer_id IS NULL,'',manufacturer_name) ) as full_item_name  , edms_item_type.item_type_id , IF(edms_inventory_item.item_type_id IS NULL,'NA',item_type) as item_type, min_quantity_purchase,  item_unit_id, edms_item_manufacturer.manufacturer_id, IF(edms_inventory_item.manufacturer_id IS NULL,'NA',manufacturer_name) as manufacturer_name, mfg_item_code, dealer_price, mrp, tax_group_id, opening_quantity,opening_rate,opening_godown_id,remarks,use_barcode,barcode_prefix,barcode_counter, supplier_id , expiration_date,  edms_inventory_item.created_by, edms_inventory_item.last_updated_by, edms_inventory_item.date_added, edms_inventory_item.date_modified, SUM(edms_ac_purchase_item.quantity) as total_purchase_amount, SUM(edms_ac_debit_note_item.quantity) as total_debit_note_amount,  SUM(edms_ac_sales_item.quantity) as total_sales_amount ,  SUM(edms_ac_credit_note_item.quantity) as total_credit_note_amount, SUM(edms_inventory_item_jv.quantity) as total_jv_amount 
		  ...
		  ON edms_inventory_item.item_id = edms_inventory_item_jv.item_id
		   WHERE inc_inventory=1 AND our_company_id = $our_company_id  GROUP BY edms_inventory_item.item_id HAVING (opening_quantity>0 OR total_purchase_amount >0 OR total_debit_note_amount >0 OR total_sales_amount>0 OR  total_credit_note_amount >0 OR total_jv_amount>0)";
...
function listNonStockItems(){
	...
	{
		$our_company_id=$_SESSION['edmsAdminSession']['oc_id'];
		$sql="SELECT item_id,item_name, alias, item_code, CONCAT(item_name, ' | ' , IF(alias!='',CONCAT(alias,' | '),'') , IF(mrp!='',CONCAT(mrp,' | '),'') , IF(item_code!='',CONCAT(item_code,' | '),''), IF(mfg_item_code!='NA',CONCAT(mfg_item_code,' | '),''), IF(edms_inventory_item.manufacturer_id IS NULL,'',manufacturer_name) ) as full_item_name  , edms_item_type.item_type_id , IF(edms_inventory_item.item_type_id IS NULL,'NA',item_type) as item_type, min_quantity_purchase,  item_unit_id, edms_item_manufacturer.manufacturer_id, IF(edms_inventory_item.manufacturer_id IS NULL,'NA',manufacturer_name) as manufacturer_name, mfg_item_code, dealer_price, mrp, tax_group_id, opening_quantity,opening_rate,opening_godown_id,remarks,use_barcode,barcode_prefix,barcode_counter, supplier_id,  edms_inventory_item.created_by, edms_inventory_item.last_updated_by, edms_inventory_item.date_added, edms_inventory_item.date_modified
...
function checkforDuplicateMFGItemCode($mfg_item_code,$manufacturer_id)
{
	...
	{ 
	$our_company_id=$_SESSION['edmsAdminSession']['oc_id'];
	$sql="SELECT mfg_item_code FROM edms_inventory_item WHERE mfg_item_code = '$mfg_item_code'  AND our_company_id = $our_company_id";
	$result = dbQuery($sql);
	if(dbNumRows($result)>0)
	return true;
	}
...
function insertInventoryItem($name,$alias,$item_code,$item_unit_id,$manufacturer_id,$mfg_item_code,$dealer_price,$mrp,$opening_quantity,$opening_rate,$remarks,$item_type_id,$min_quantity,$opening_godown_id,$tax_group_id,$use_barcode,$barcode_prefix,$barcode_counter,$barcode_array,$gen_barcode,$supplier_id=NULL,$expiration_date="01/01/1970",$sku_array=array()){
	...
	/*	if(!(checkForNumeric($oc_id)))
		...
		if(!validateForNull($opening_godown_id) || $opening_godown_id<0)
		{
		...
		if(!validateForNull($expiration_date))
		$expiration_date = "01/01/1970";
		
		if(isset($expiration_date) && validateForNull($expiration_date))
		{
		$expiration_date = str_replace('/', '-', $expiration_date);
		$expiration_date = date('Y-m-d',strtotime($expiration_date));
		}
		if(validateForNull($name,$item_code,$item_type_id,$tax_group_id,$opening_godown_id) && checkForNumeric($min_quantity,$item_type_id,$supplier_id) && !checkforDuplicateItemCode($item_code,false,$name) && !checkforDuplicateMFGItemCode($mfg_item_code,$manufacturer_id)  && $item_type_id>0 && ($use_barcode==0 || ($use_barcode==1 &&  checkforDuplicateBarcodePrefix($barcode_prefix) && validateForNull($barcode_prefix) && checkForNumeric($barcode_counter))))
			{
				
			$sql="INSERT INTO edms_inventory_item
					(item_name, alias, item_code,  item_unit_id, item_type_id, min_quantity_purchase, manufacturer_id, mfg_item_code, dealer_price, mrp,tax_group_id, opening_quantity,opening_rate,opening_godown_id, our_company_id, remarks, use_barcode, gen_barcode, barcode_prefix, barcode_counter,  created_by, last_updated_by, date_added, date_modified, supplier_id,expiration_date)
					VALUES
					('$name','$alias','$item_code',$item_unit_id,$item_type_id, $min_quantity,$manufacturer_id,'$mfg_item_code',$dealer_price,$mrp,$tax_group_id,$opening_quantity,$opening_rate, $opening_godown_id,$our_company_id,'$remarks',$use_barcode, $gen_barcode,'$barcode_prefix','$barcode_counter',$admin_id,$admin_id,NOW(),NOW(),$supplier_id,'$expiration_date')";
					
			
			dbQuery($sql);
			$item_id=dbInsertId();
			$sku_array[] = $item_code;
			insertSkusToItem($item_id,$sku_array);
...
function updateInventoryItem($id,$name,$alias,$item_code,$item_unit_id,$manufacturer_id,$mfg_item_code,$dealer_price,$mrp,$opening_quantity,$opening_rate,$remarks,$item_type_id,$min_quantity,$opening_godown_id,$tax_group_id,$use_barcode,$barcode_prefix,$barcode_counter,$barcode_array,$supplier_id=NULL,$expiration_date="01/01/1970",$sku_array=array()){
	...
	/*	if(!(checkForNumeric($oc_id)))
		...
		if(validateForNull($name,$item_code,$item_type_id,$tax_group_id,$opening_godown_id) && checkForNumeric($min_quantity,$id,$supplier_id) && !checkforDuplicateItemCode($item_code,$id,$name))
			{
			
			$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
			$sql="UPDATE edms_inventory_item
					SET item_name = '$name', alias = '$alias', item_code='$item_code', item_unit_id = $item_unit_id, manufacturer_id = $manufacturer_id, mfg_item_code= '$mfg_item_code' , dealer_price=$dealer_price, mrp=$mrp, tax_group_id= $tax_group_id,opening_quantity=$opening_quantity,opening_rate=$opening_rate, opening_godown_id = $opening_godown_id, remarks='$remarks' , item_type_id = $item_type_id, min_quantity_purchase = $min_quantity,last_updated_by=$admin_id, date_modified=NOW(), supplier_id = $supplier_id, expiration_date = '$expiration_date'
					WHERE item_id=$id";
				
			dbQuery($sql);
		    insertDictionaryWords($name);
			$sku_array[] = $item_code;
			deleteSkusForItemId($id);
			insertSkusToItem($id,$sku_array);
			if(USE_BARCODE && $old_item['use_barcode']==1)
			{
				if($old_item['gen_barcode']==0)
				{
...
function getInventoryItemById($id){
	...
	try
	{
		...
		{	
		$sql="SELECT item_id,item_name, alias, item_code, edms_item_type.item_type_id , IF(edms_inventory_item.item_type_id IS NULL,'NA',item_type) as item_type, min_quantity_purchase,  item_unit_id, edms_item_manufacturer.manufacturer_id, IF(edms_inventory_item.manufacturer_id IS NULL,'NA',manufacturer_name) as manufacturer_name, mfg_item_code, dealer_price, mrp, tax_group_id, opening_quantity,opening_rate,opening_godown_id, edms_inventory_item.our_company_id,remarks,use_barcode,barcode_prefix,barcode_counter,  edms_inventory_item.created_by, edms_inventory_item.last_updated_by, edms_inventory_item.date_added, edms_inventory_item.date_modified, gen_barcode, godown_name, supplier_id, expiration_date
		  FROM edms_inventory_item LEFT OUTER JOIN edms_item_manufacturer
		  ON edms_inventory_item.manufacturer_id=edms_item_manufacturer.manufacturer_id
		  LEFT OUTER JOIN edms_godown
		  ON edms_godown.godown_id=edms_inventory_item.opening_godown_id
		  LEFT OUTER JOIN edms_item_type
...
function insertSkusToItem($item_id,$sku_array)
{
	if(checkForNumeric($item_id) && is_array($sku_array) && count($sku_array)>0)
	{
		foreach($sku_array as $sku)
		{
			insertSkuToItem($item_id,$sku);
		}
	}
	
}

function deleteSkusToItem($item_id,$sku_array)
{
	if(checkForNumeric($item_id) && is_array($sku_array) && count($sku_array)>0)
	{
		foreach($sku_array as $sku)
		{
			deleteSkuToItem($item_id,$sku);
		}
	}
	
}
function insertSkuToItem($item_id,$sku)
{
	$sku=clean_data($sku);
	if(checkForNumeric($item_id) && validateForNull($sku) && !checkForDuplicateSku($sku))
	{
	   $sql="INSERT INTO edms_item_skus (sku,item_id) VALUES ('$sku',$item_id)";
	   dbQuery($sql);	
	   return "success";
	}
	else
...
function deleteSkuToItem($item_id,$sku)
{
	$sku=clean_data($sku);
	if(checkForNumeric($item_id) && validateForNull($sku))
	{
	   $sql="DELETE FROM edms_item_skus WHERE sku = '$sku' AND item_id = $item_id";
	   dbQuery($sql);	
	   return "success";
	}
	else
...
function getSkusForItemId($item_id)
{
	if(checkForNumeric($item_id))
	{
		$sql="SELECT sku_id, sku FROM edms_item_skus WHERE item_id = $item_id";
		$result=dbQuery($sql);
	    $result_array=dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $result_array;
...
function getItemIdFromSku($sku)
{
	...
	{
		$sql="SELECT item_id FROM edms_item_skus WHERE sku = '$sku'";
		$result=dbQuery($sql);
	    $result_array=dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $result_array[0][0];
...
function getItemIdFromUPC($sku)
{
	if(validateForNull($sku))
	{
		$sql="SELECT item_id FROM edms_inventory_item WHERE mfg_item_code = '$sku'";
		$result=dbQuery($sql);
	    $result_array=dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $result_array[0][0];
...
function getUPCFromItemId($item_id)
{
	if(checkForNumeric($item_id))
	{
		$sql="SELECT mfg_item_code FROM edms_inventory_item WHERE item_id = '$item_id'";
		$result=dbQuery($sql);
	    $result_array=dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $result_array[0][0];
...
function deleteSkusForItemId($item_id)
{
	if(checkForNumeric($item_id))
	{
		$sql="DELETE FROM edms_item_skus WHERE item_id = $item_id";
		dbQuery($sql);
	}
	
}				
function getSecondarySkusForItemId($item_id)
{
	    $sql="SELECT sku_id, sku FROM edms_item_skus WHERE item_id = $item_id AND sku != (select item_code FROM edms_inventory_item WHERE item_id = $item_id)";
		$result=dbQuery($sql);
	    $result_array=dbResultToArray($result);
		if(dbNumRows($result)>0)
		return $result_array;
...
function checkForDuplicateSku($sku,$item_id=false)
{
	if(validateForNull($sku))
	{
		$sql="SELECT sku FROM edms_item_skus WHERE sku = '$sku'";
		if(checkForNumeric($item_id) && $item_id>0)
		$sql=$sql." AND item_id!=$item_id";
		$result=dbQuery($sql);
	    $result_array=dbResultToArray($result);
		if(dbNumRows($result)>0)
...
function updateShelfForInventoryItem($item_id,$shelf_id)
{
	
	if(checkForNumeric($item_id,$shelf_id) && $item_id>0 && $shelf_id>0)
	{
		$sql="UPDATE edms_inventory_item SET opening_godown_id = $shelf_id WHERE item_id = $item_id";
		dbQuery($sql);
		return true;
	}
	return false;
...
function updateOpeningQuantityForItem($item_id,$opening_quantity)
{
	
	if(checkForNumeric($item_id,$opening_quantity) && $item_id>0 && $opening_quantity>0)
	{
		$sql="UPDATE edms_inventory_item SET opening_quantity = $opening_quantity WHERE item_id = $item_id";
		dbQuery($sql);
		return true;
	}
	return false;
...
function updateExpirationDateForItem($item_id,$expiration_date)
{
	
	if(checkForNumeric($item_id) && $item_id>0 && validateForDate($expiration_date))
	{
		...
		$sql="UPDATE edms_inventory_item SET expiration_date = '$expiration_date' WHERE item_id = $item_id";
		dbQuery($sql);
