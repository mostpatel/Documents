...
function addProofImageInsurance($customer_id,$image)
{
    $image_name_prefix= $customer_id;
	$image_name_prefix=str_replace("/","_",$image_name_prefix);
...
function UploadExcel($image, $uploadDir,$prefix=false)
{
    ...
    if (trim($image['tmp_name']) != '') {
		...
		if (($image["type"] ==   'application/vnd.ms-excel') || ($image["type"] == 'text/plain') || ($image["type"] == 'text/csv') || ($image["type"] == 'text/tsv') || ($image["type"] ==  'application/csv') || ($image["type"] ==  'text/comma-separated-values') || ($image["type"] ==   'application/excel') || ($image["type"] ==   'application/vnd.msexcel')  || ($image["type"] ==   'text/anytext')  || ($image["type"] ==    'application/octet-stream')  || ($image["type"] ==   'application/txt')  || ($image["type"] ==   'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')  ) 
		{
		
		  $path_parts = pathinfo($image['tmp_name']);
	     
		  move_uploaded_file($image['tmp_name'], IMG_ROOT.$uploadDir.$imagePath);
		  chmod(IMG_ROOT.$uploadDir.$imagePath, 0777);
