...
foreach($resultArray as $n)
{
	
	
	$objPHPExcel->getActiveSheet()->SetCellValue('A'.$i, utf8_encode($n['item_name']));
$objPHPExcel->getActiveSheet()->setCellValueExplicit('B'.$i,utf8_encode($n['sku']), PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet()->SetCellValueExplicit('C'.$i, utf8_encode($n['upc']), PHPExcel_Cell_DataType::TYPE_STRING);
$objPHPExcel->getActiveSheet()->SetCellValue('D'.$i, $n['qty_in_stock']);
$objPHPExcel->getActiveSheet()->SetCellValue('E'.$i,utf8_encode($n['shelf']));
$objPHPExcel->getActiveSheet()->SetCellValue('F'.$i, utf8_encode($n['manufacturer']));
$objPHPExcel->getActiveSheet()->SetCellValue('G'.$i, utf8_encode($n['supplier']));
$i++;
}
