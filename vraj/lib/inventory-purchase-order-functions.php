...
function updateInventoryNonStockItemPurchaseOrder($purchase_order_id,$item_id_array,$rate_array,$quantity_array,$discount_array,$item_id_ns_array,$amount_ns_array,$discount_ns_array,$trans_date,$delivery_date,$to_ledger,$from_ledger,$remarks,$godown_id_array,$tax_group_array,$tax_group_ns_array,$sales_ref="NA",$sales_ref_type=0)
{
	...
	$result=updatePurchaseOrder($purchase_order_id,$total_nett_amount,$trans_date,$trans_date,$to_ledger,$from_ledger,$remarks,$sales_ref,$sales_ref_type);
	if($result=="success")
	{
...
function insertInventoryNonStockItemPurchaseOrder($item_id_array,$rate_array,$quantity_array,$discount_array,$item_id_ns_array,$amount_ns_array,$discount_ns_array,$trans_date,$delivery_date,$to_ledger,$from_ledger,$remarks,$godown_id_array,$tax_group_array,$tax_group_ns_array,$purchase_order_ref="NA",$purchase_order_ref_type=0,$oc_id=NULL,$convert_item_name_array=1)
{
	if(!isset($convert_item_name_array) || $convert_item_name_array==1)
	{
	$item_id_array = ConvertItemNameArrayInToIdArray($item_id_array,1);
...
function checkReceivedQuantityToOrderedQty($order_qty,$receive_qty)
{
	if(is_array($order_qty) && is_array($receive_qty))
	{
		...
		for($i=0;$i<count($order_qty);$i++)
		{
			...
			if(checkForNumeric($o_qty,$r_qty))
			{
				
				if($r_qty>$o_qty)
				return false;
			}
			else if(checkForNumeric($r_qty))
			return false;
		}
		return true;
	}
