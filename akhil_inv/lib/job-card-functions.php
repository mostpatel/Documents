...
function finalizeJobCard($job_card_id,$invoice_no,$bay_out,$actual_delivery,$next_service_date,$send_sms,$next_service_kms=0)
{
	if(!checkForNumeric($next_service_kms))
	$next_service_kms-0;
	if(checkForNumeric($job_card_id,$invoice_no,$send_sms) && validateForNull($bay_out,$actual_delivery))
	{
		...
		$sql="UPDATE edms_job_card SET bay_out = '$bay_out' , actual_delivery = '$actual_delivery', next_service_date = '$next_service_date',  next_service_kms = $next_service_kms WHERE job_card_id = $job_card_id";
		dbQuery($sql);
...
function getJobCardById($id){
	...
	try
	{
		...
		{
		$returnArray = array();
		$sql="SELECT job_card_no, job_card_datetime,service_type_id, free_service_no, date_of_sale,kms_covered, estimated_repair_cost, bay_in, bay_out, delivery_promise, actual_delivery, next_service_date, next_service_kms, technician_id, vehicle_id, customer_id, created_by, last_updated_by, date_added, date_modified
		      FROM edms_job_card
			  WHERE job_card_id=$id";
