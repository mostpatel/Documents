...
function getTransItemUnitByInventoryJvItemId($purchase_item_id)
{
	if(checkForNumeric($purchase_item_id))
	{
		$sql="SELECT inventory_item_jv_id, rate,quantity,item_id, edms_ac_trans_item_unit.item_unit_id,unit_name FROM edms_ac_trans_item_unit INNER JOIN edms_item_unit ON edms_item_unit.item_unit_id = edms_ac_trans_item_unit.item_unit_id WHERE inventory_item_jv_id = $purchase_item_id";
		$result=dbQuery($sql);
		$resultArray =dbResultToArray($result);
		return $resultArray[0];
