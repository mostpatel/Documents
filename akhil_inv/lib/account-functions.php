...
function getAllTransactionsForLedgerIdMonthWise($id,$transaction_type_array=NULL,$from=NULL,$to=NULL,$tax_inc=0)
{
	...
	{	
		if(!defined('SALES_PURCHASE_INCLUDE') || SALES_PURCHASE_INCLUDE!=0)
		{
		$purchase=getTotalPurchaseForLedgerIdForMonth($id,$month,$year,$from,$to,$tax_inc);
	...
	if((validateForNull($transaction_type_array) && in_array(6,$transaction_type_array)) || !validateForNull($transaction_type_array))
	{
		...
		$sale=getTotalSaleForLedgerIdForMonth($id,$month,$year,$from,$to,$tax_inc);
...
if($head_type!=2)
{
	...
	$sql=$sql."SELECT receipt_id as id, 2 as type,";
	if($head_type==0)
	...
	$sql=$sql.",from_ledger_id as debit_ledger_id,to_ledger_id as credit_ledger_id, NULL as debit_customer_id,to_customer_id as credit_customer_id,oc_id,auto_rasid_type,auto_id, trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year,created_by,last_updated_by,date_added,date_modified,1 as other_ledger_id , remarks
			  FROM edms_ac_receipt WHERE edms_ac_receipt.oc_id = $oc_id AND ";	  
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
	...
	$sql=$sql."  to_customer_id=$customer_id GROUP BY parent_id,to_customer_id";	  
	if($head_type==0)
	$sql=$sql." GROUP BY parent_id,from_ledger_id";
	else if(checkForNumeric($id) && $head_type>0 && !checkForNumeric($customer_id))
	...
	$sql=$sql." UNION ALL ";
		...
		$sql=$sql."SELECT contra_id as id, 4 as type,amount,to_ledger_id as debit_ledger_id,from_ledger_id as credit_ledger_id,NULL as debit_customer_id, NULL as credit_customer_id,oc_id,0 as auto_rasid_type,0 as auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,1 as other_ledger_id , remarks
			  FROM edms_ac_contra WHERE edms_ac_contra.oc_id = $oc_id AND ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
	...
	$sql=$sql." (from_customer_id=$customer_id OR to_customer_id=$customer_id)";	  
		...
		if( ((validateForNull($transaction_type_array) && in_array(5,$transaction_type_array)) || !validateForNull($transaction_type_array)) && (!defined('SALES_PURCHASE_INCLUDE') || SALES_PURCHASE_INCLUDE!=0))
		{
		if($sql!="")
	$sql=$sql." UNION ALL ";
	...
	$sql=$sql." ,to_ledger_id as debit_ledger_id,from_ledger_id as credit_ledger_id, NULL as debit_customer_id ,from_customer_id as credit_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,1 as other_ledger_id , remarks
			  FROM edms_ac_purchase LEFT JOIN (SELECT f.purchase_id, SUM( tax_amount ) AS total_tax
...
WHERE edms_ac_purchase.oc_id = $oc_id AND ";
	...
	$sql=$sql."   from_customer_id=$customer_id";	  
		...
		if(((validateForNull($transaction_type_array) && in_array(6,$transaction_type_array)) || !validateForNull($transaction_type_array)) && (!defined('SALES_PURCHASE_INCLUDE') || SALES_PURCHASE_INCLUDE!=0))
		{
		if($sql!="")
	$sql=$sql." UNION ALL ";
	...
	$sql=$sql." ,to_ledger_id as debit_ledger_id , from_ledger_id as credit_ledger_id,to_customer_id as debit_customer_id,NULL as credit_customer_id,oc_id,auto_rasid_type,auto_id, trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year,created_by,last_updated_by,date_added,date_modified,1 as other_ledger_id, remarks
			  FROM edms_ac_sales LEFT JOIN (SELECT f.sales_id, SUM( tax_amount ) AS total_tax
FROM edms_ac_sales_tax f
GROUP BY f.sales_id
)h ON edms_ac_sales.sales_id = h.sales_id WHERE edms_ac_sales.oc_id=$oc_id AND ";	  
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
	...
	$sql=$sql."  to_customer_id=$customer_id";	  	  
		...
		if(((validateForNull($transaction_type_array) && in_array(7,$transaction_type_array)) || !validateForNull($transaction_type_array))  && (!defined('SALES_PURCHASE_INCLUDE') || SALES_PURCHASE_INCLUDE!=0))
		{
		if($sql!="")
	$sql=$sql." UNION ALL ";	
	...
	$sql=$sql." + IF(total_tax IS NOT NULL,total_tax,0) AS amount ";
	$sql=$sql." ,from_ledger_id as debit_ledger_id,to_ledger_id as credit_ledger_id, from_customer_id as debit_customer_id,NULL as credit_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,1 as other_ledger_id, remarks
			  FROM edms_ac_debit_note LEFT JOIN (SELECT f.debit_note_id, SUM( tax_amount ) AS total_tax
FROM edms_ac_debit_note_tax f
GROUP BY f.debit_note_id
)h ON edms_ac_debit_note.debit_note_id = h.debit_note_id
WHERE edms_ac_debit_note.oc_id = $oc_id AND ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
	...
	$sql=$sql."  from_customer_id=$customer_id";	 	  
		...
		if(((validateForNull($transaction_type_array) && in_array(8,$transaction_type_array)) || !validateForNull($transaction_type_array))  && (!defined('SALES_PURCHASE_INCLUDE') || SALES_PURCHASE_INCLUDE!=0))
		{
	if($sql!="")
	$sql=$sql." UNION ALL ";	
	...
	$sql=$sql.",from_ledger_id as debit_ledger_id,to_ledger_id as credit_ledger_id,to_customer_id as credit_customer_id,NULL as debit_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year,created_by,last_updated_by,date_added,date_modified,1 as other_ledger_id, remarks
			  FROM edms_ac_credit_note LEFT JOIN (SELECT f.credit_note_id, SUM( tax_amount ) AS total_tax
FROM edms_ac_credit_note_tax f
GROUP BY f.credit_note_id
)h ON edms_ac_credit_note.credit_note_id = h.credit_note_id WHERE edms_ac_credit_note.oc_id = $oc_id AND ";	   
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
...
function getAllTransactionsForLedgerId($id,$transaction_type_array=NULL,$from=NULL,$to=NULL)
{
	...
	$purchase_tax = getPurchasesTaxesForLedgerIdBetweenDates($id,$from,$to);
		...
		$returnArray=array_merge($payments,$receipts,$jvs,$contras,$purchases,$sales,$debit_notes,$credit_notes);
...
function getLatestTransactionDateForLedgerId($id)
{
	...
	$sql=$sql." to_customer_id=$from_customer GROUP BY to_customer_id";	
				...
				$sql=$sql." SELECT MAX(trans_date)
					  FROM edms_ac_jv, edms_ac_jv_cd WHERE oc_id = $oc_id AND  edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id AND ";
			...
			$sql=$sql." UNION ALL
					  SELECT MAX(trans_date)
					  FROM edms_ac_purchase WHERE oc_id = $oc_id AND ";	  
			if($head_type==3)		  
			$sql=$sql." to_ledger_id = $from_ledger GROUP BY to_ledger_id ";
			else if(isset($head_type) && checkForNumeric($head_type) && $head_type==1)
			$sql=$sql." from_ledger_id = $from_ledger GROUP BY from_ledger_id ";
			...
			$sql=$sql."  MAX(trans_date)
					  FROM edms_ac_sales WHERE oc_id = $oc_id AND ";	  
			if($head_type==4)		  
			$sql=$sql." from_ledger_id = $from_ledger GROUP BY from_ledger_id ";
			else if(isset($head_type) && checkForNumeric($head_type) && $head_type==1)
			$sql=$sql." to_ledger_id = $from_ledger GROUP BY to_ledger_id ";
			...
			$sql=$sql." UNION ALL
					  SELECT ";	  
			$sql=$sql."MAX(trans_date)
					  FROM edms_ac_debit_note WHERE oc_id = $oc_id AND ";	  
			if($head_type==3)		  
			$sql=$sql." to_ledger_id = $from_ledger GROUP BY to_ledger_id ";
			else if(isset($head_type) && checkForNumeric($head_type) && $head_type==1)
			$sql=$sql." from_ledger_id = $from_ledger GROUP BY from_ledger_id ";
			...
			$sql=$sql."MAX(trans_date)
					  FROM edms_ac_credit_note WHERE oc_id = $oc_id AND ";
			if($head_type==4)		  
			$sql=$sql." from_ledger_id = $from_ledger GROUP BY from_ledger_id ";
			else if(isset($head_type) && checkForNumeric($head_type) && $head_type==1)
			$sql=$sql." to_ledger_id = $from_ledger GROUP BY to_ledger_id ";
			...
			if(isset($head_type) && $head_type==0)
			{
			  ...
			  FROM edms_ac_contra WHERE oc_id = $oc_id AND ";
			if(isset($head_type) && checkForNumeric($head_type) && $head_type==0)  	  
			$sql=$sql." from_ledger_id=$from_ledger GROUP BY from_ledger_id"; 
			$sql=$sql." UNION ALL SELECT  MAX(trans_date)
...
function getOpeningBalanceForLedgerArrayForDate($array_id,$date) // return opening balance on date for id, id should start with L or C
{
	...
	$sql="SELECT CONCAT('L',ledger_id) as id, ledger_name as name, head_id, IF(head_id = ".$bank_head_id." OR head_id =  ".$cash_head_id. " OR head_id = $bank_od_head_id  , 0 , IF( head_id = ".$tax_head_id." , 2 , IF( head_id = ".$purchase_head_id." , 3 ,  IF(head_id = ".$sales_head_id." , 4 , 1)))) as head_type ,  IF(opening_cd=0, opening_balance , -opening_balance) as opening_balance,
	(SELECT IF(head_id = $cash_head_id OR head_id = $bank_head_id OR head_id = $bank_od_head_id , -SUM(amount) , SUM(amount) ) FROM edms_ac_payment WHERE oc_id = $oc_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (to_ledger_id = ledger_id OR from_ledger_id = ledger_id) GROUP BY CASE WHEN head_id = $cash_head_id OR head_id = $bank_head_id OR head_id = $bank_od_head_id  THEN to_ledger_id ELSE from_ledger_id END )  as payment_amount,
	(SELECT IF(head_id != $cash_head_id AND head_id != $bank_head_id AND head_id != $tax_head_id AND head_id != $purchase_head_id AND head_id != $sales_head_id AND head_id!= $bank_od_head_id, -SUM(amount) , SUM(amount) ) FROM edms_ac_receipt WHERE oc_id = $oc_id AND  ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (to_ledger_id = ledger_id OR from_ledger_id = ledger_id) GROUP BY CASE WHEN head_id = $cash_head_id OR head_id = $bank_head_id OR head_id = $bank_od_head_id  THEN from_ledger_id ELSE to_ledger_id END  ) as receipt_amount,
	(SELECT IF(head_id != $cash_head_id AND head_id != $bank_head_id AND head_id != $bank_od_head_id, -SUM(edms_ac_jv_cd.amount) , 0 ) FROM edms_ac_jv_cd, edms_ac_jv WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id) AND from_ledger_id = ledger_id GROUP BY from_ledger_id) as credit_jv_amount,
	(SELECT IF(head_id != $cash_head_id AND head_id != $bank_head_id AND head_id != $bank_od_head_id, SUM(edms_ac_jv_cd.amount) , 0 ) FROM edms_ac_jv_cd, edms_ac_jv WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id) AND to_ledger_id = ledger_id GROUP BY to_ledger_id)  as debit_jv_amount,
	(SELECT IF(head_id = $cash_head_id OR head_id = $bank_head_id OR head_id = $bank_od_head_id , -SUM(amount) , 0 ) FROM edms_ac_contra WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	// if sales and purchase are to be included
	if(!defined('SALES_PURCHASE_INCLUDE') || SALES_PURCHASE_INCLUDE!=0)
	{
	$sql=$sql.",
	(SELECT IF(head_id != $purchase_head_id, -SUM(amount) , SUM(amount) ) FROM  edms_ac_purchase WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT IF(head_id = $sales_head_id, -SUM(amount) , SUM(amount)) FROM  edms_ac_sales WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT IF(head_id = $purchase_head_id, -SUM(amount) , SUM(amount) ) FROM  edms_ac_debit_note WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT IF(head_id != $sales_head_id, -SUM(amount) , SUM(amount)) FROM  edms_ac_credit_note WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT  IF(head_id = $tax_head_id,SUM(-tax_amount),SUM(tax_amount))  FROM edms_ac_sales, edms_ac_sales_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_sales.sales_id = edms_ac_sales_tax.sales_id AND edms_ac_sales_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT  IF(head_id = $tax_head_id,SUM(-tax_amount),SUM(tax_amount))  FROM edms_ac_debit_note, edms_ac_debit_note_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_debit_note.debit_note_id = edms_ac_debit_note_tax.debit_note_id AND edms_ac_debit_note_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT  IF(head_id = $tax_head_id,SUM(tax_amount),SUM(-tax_amount))  FROM edms_ac_credit_note, edms_ac_credit_note_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_credit_note.credit_note_id = edms_ac_credit_note_tax.credit_note_id AND edms_ac_credit_note_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	(SELECT  IF(head_id = $tax_head_id,SUM(tax_amount),SUM(-tax_amount)) FROM edms_ac_purchase, edms_ac_purchase_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_purchase.purchase_id = edms_ac_purchase_tax.purchase_id AND edms_ac_purchase_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	$sql=$sql." FROM edms_ac_ledgers WHERE oc_id = $oc_id AND ledger_id IN ($ledger_string)";
...
function getOpeningBalanceForCustomerArrayForDate($array_id,$date) // return opening balance on date for id, id should start with L or C
{
	...
	$sql="SELECT CONCAT('C',customer_id) as id, customer_name as name, $sundry_debtors_head_id as head_id, 1, (SELECT IF(opening_cd=0,opening_balance,-opening_balance) as opening_balance FROM edms_customer_opening_balance WHERE edms_customer_opening_balance.customer_id = edms_customer.customer_id AND oc_id = $oc_id) as opening_balance ,
	(SELECT  SUM(amount)  FROM edms_ac_payment WHERE oc_id = $oc_id AND  ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (from_customer_id = customer_id) GROUP BY   from_customer_id ) as payment_amount,
	(SELECT  -SUM(amount)  FROM edms_ac_receipt WHERE oc_id = $oc_id AND  ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (to_customer_id = customer_id) GROUP BY to_customer_id  ) as receipt_amount,
	(SELECT  -SUM(edms_ac_jv_cd.amount) FROM edms_ac_jv_cd, edms_ac_jv WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	...
	{  
	$sql=$sql." ,
	(SELECT  -SUM(amount)  FROM  edms_ac_purchase WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	
	$sql=$sql."  from_customer_id  = customer_id GROUP BY  from_customer_id ) as purchase_amount,
	(SELECT  SUM(amount) FROM  edms_ac_sales WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (to_customer_id = customer_id ) GROUP BY to_customer_id ) as sales_amount,
	(SELECT  SUM(amount)  FROM  edms_ac_debit_note WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (from_customer_id  = customer_id) GROUP BY from_customer_id ) as debit_note_amount,
	(SELECT -SUM(amount)  FROM  edms_ac_credit_note WHERE oc_id = $oc_id AND   ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (to_customer_id = customer_id ) GROUP BY to_customer_id) as credit_note_amount,
	(SELECT  SUM(tax_amount) FROM edms_ac_sales, edms_ac_sales_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_sales.sales_id = edms_ac_sales_tax.sales_id AND edms_ac_sales_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." ( to_customer_id = customer_id) GROUP BY  to_customer_id) as sales_tax_amount,
	(SELECT  SUM(tax_amount) FROM edms_ac_debit_note, edms_ac_debit_note_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_debit_note.debit_note_id = edms_ac_debit_note_tax.debit_note_id AND edms_ac_debit_note_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (from_customer_id = customer_id) GROUP BY from_customer_id) as debit_note_tax_amount,
	(SELECT  SUM(-tax_amount) FROM edms_ac_credit_note, edms_ac_credit_note_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_credit_note.credit_note_id = edms_ac_credit_note_tax.credit_note_id AND edms_ac_credit_note_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
	$sql=$sql." (to_customer_id = customer_id) GROUP BY to_customer_id) as credit_note_tax_amount,
	(SELECT  SUM(-tax_amount) FROM edms_ac_purchase, edms_ac_purchase_tax,edms_tax WHERE oc_id = $oc_id AND edms_ac_purchase.purchase_id = edms_ac_purchase_tax.purchase_id AND edms_ac_purchase_tax.tax_id = edms_tax.tax_id AND ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."trans_date<='$to'
		  AND ";
...
function getAllTransactionForLedgerGroupId($group_id,$transaction_type_array=NULL,$from=NULL,$to=NULL) // $group_id =-1 means customers
{
	...
	else if($group_id==-3)
	{
		...
		$dealers = listCustomer($oc_agency_id);
		$no=0;
	
		foreach($dealers as $agencyDetails)
		{
			
			$customer_id =  $agencyDetails['customer_id'];
...
function getOpeningBalanceForLedgerForDate($id,$date) // return opening balance on date for id, id should start with L or C
{
	...
	else if(substr($id, 0, 1) == 'C')
	{
		...
		$from_customer=intval($from_customer);
		$from_ledger=false;
		$sql="SELECT IF(opening_balance_id IS NOT NULL,IF(opening_cd=0,opening_balance,-opening_balance),0) FROM edms_customer_opening_balance WHERE customer_id = $from_customer AND oc_id = $oc_id ";
		}	
	...
	$sql=$sql." to_customer_id=$from_customer GROUP BY to_customer_id";	
		...
		if($head_type==1 || $head_type==3 || $head_type==4 || !isset($head_type)) // if head is normal or purchase or sales ledger include jv
		{
				...
				$sql=$sql." SELECT  -sum(edms_ac_jv_cd.amount)
					  FROM edms_ac_jv , edms_ac_jv_cd WHERE oc_id = $oc_id AND  ";
			if(isset($to) && validateForNull($to))  
			$sql=$sql."trans_date<='$to'
				  AND ";
			$sql=$sql." edms_ac_jv.jv_id = edms_ac_jv_cd.jv_id AND ";	  
    ...
    {  
		...
		$sql=$sql." SUM(-tax_amount)
				  FROM edms_ac_purchase, edms_ac_purchase_tax,edms_tax WHERE oc_id = $oc_id AND
		...
		$sql=$sql."-";			  	  
		$sql=$sql." SUM(tax_amount)
				  FROM edms_ac_sales, edms_ac_sales_tax,edms_tax WHERE oc_id = $oc_id AND
		...
		$sql=$sql."-";		  
		$sql=$sql."SUM(tax_amount)
				  FROM edms_ac_debit_note, edms_ac_debit_note_tax,edms_tax WHERE oc_id = $oc_id AND
		...
		$sql=$sql."-";			  	  
		$sql=$sql." SUM(-tax_amount)
				  FROM edms_ac_credit_note, edms_ac_credit_note_tax,edms_tax WHERE oc_id = $oc_id AND
       ...
       {  
		if($head_type==3) // if purchase or normal or customer ledger
			{		
				$sql=$sql."
						  SELECT "; 
				$sql=$sql."SUM(net_amount)
						  FROM edms_ac_purchase_item,edms_ac_purchase WHERE oc_id = $oc_id AND edms_ac_purchase_item.purchase_id = edms_ac_purchase.purchase_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			  AND ";	 	  
				...
				$sql=$sql." UNION ALL
						  SELECT "; 
				$sql=$sql."SUM(net_amount)
						  FROM edms_ac_purchase_nonstock,edms_ac_purchase WHERE oc_id = $oc_id AND edms_ac_purchase_nonstock.purchase_id = edms_ac_purchase.purchase_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			  AND ";	 	  
				...
				$sql=$sql." UNION ALL
						  SELECT "; 
				$sql=$sql."-SUM(net_amount)
						  FROM edms_ac_debit_note, edms_ac_debit_note_item WHERE oc_id = $oc_id AND edms_ac_debit_note_item.debit_note_id = edms_ac_debit_note.debit_note_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			  AND ";	 	  
				...
				$sql=$sql." UNION ALL
						  SELECT "; 
				$sql=$sql."-SUM(net_amount)
						  FROM edms_ac_debit_note, edms_ac_debit_note_nonstock WHERE oc_id = $oc_id AND edms_ac_debit_note_nonstock.debit_note_id = edms_ac_debit_note.debit_note_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			...
			{	
				...
				$sql=$sql."-";		  
				$sql=$sql."SUM(net_amount)
						  FROM edms_ac_sales, edms_ac_sales_item WHERE oc_id = $oc_id AND edms_ac_sales_item.sales_id = edms_ac_sales.sales_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			  AND ";	 	  
				...
				$sql=$sql."-";		  
				$sql=$sql."SUM(net_amount)
						  FROM edms_ac_sales, edms_ac_sales_nonstock WHERE oc_id = $oc_id AND edms_ac_sales_nonstock.sales_id = edms_ac_sales.sales_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			  AND ";	 	  
				...
				$sql=$sql." UNION ALL
						  SELECT ";	  		  
				$sql=$sql."SUM(net_amount)
						  FROM edms_ac_credit_note, edms_ac_credit_note_item WHERE oc_id = $oc_id AND   edms_ac_credit_note_item.credit_note_id = edms_ac_credit_note.credit_note_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
		$sql=$sql."trans_date>='$from'
			  AND ";	 	  
				...
				$sql=$sql." UNION ALL
						  SELECT ";	  		  
				$sql=$sql."SUM(net_amount)
						  FROM edms_ac_credit_note, edms_ac_credit_note_nonstock WHERE oc_id = $oc_id AND   edms_ac_credit_note_nonstock.credit_note_id = edms_ac_credit_note.credit_note_id AND ";	  
				if(isset($to) && validateForNull($to))  
				$sql=$sql."trans_date<='$to'
					  AND ";
				if(isset($from) && validateForNull($from))  
...
function getOpeningBalanceForHeadIdForDate($head_id,$date) // head_id should be numeric and date is compalsory
{
	...
	$sql="SELECT ";
	if(($head_id==$cash_head_id || $head_id==$bank_head_id || $head_id==$bank_od_head_id))
	$sql=$sql."-";
	$sql=$sql."SUM(amount) as amount
			  FROM edms_ac_payment WHERE oc_id = $oc_id AND ";
	if(isset($to) && validateForNull($to))  
...
function getFirstPageBalanceSheet($to,$from)
{
	...
	if(!defined('TAX_MODE') || TAX_MODE==0)
	{
	...
	foreach($main_heads as $head)
	{
		...
		if($head_id==2  && (!defined('TAX_MODE') || TAX_MODE==0))
		{
...
function getProfitAndLossSheet($from,$to) // $from and $to of form dd/mm/yyyy
{
	...
	if($closing_stock>0)
	$sub_total=$sub_total-$closing_stock;
...
function getSecondPageBalanceSheet($head_id)
{
	...
	$i=0;
		...
		foreach($child_heads as $child_head)
		{
			...
			if($stock_in_hand_head == $child_head_id  && (!defined('TAX_MODE') || TAX_MODE==0))
			{
		...
		$ledgers_array = array_merge($ledgers,$customers);
		$ob=0;
		...
		$ledgers_array = array_merge($ledgers_opening_balance_array,$customers_opening_balance_array);
		...
		if($stock_in_hand_head == $head_id && (!defined('TAX_MODE') || TAX_MODE==0))
		{
				$closing_stock = getClosingStockForDate($to);
				$closing_stock_details=array();
				$closing_stock_details['head_id']="closePL";
