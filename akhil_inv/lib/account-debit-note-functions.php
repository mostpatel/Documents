...
{
	$sql="SELECT debit_note_id,debit_note_ref_type,debit_note_ref,amount,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date, delivery_date,remarks,created_by,last_updated_by,date_added,date_modified,vch_no
...
function addDebitNote($amount,$trans_date, $delivery_date,$to_ledger,$from_ledger,$remarks,$auto_rasid_type=0,$auto_id=0,$debit_note_ref="NA",$debit_note_ref_type=0,$oc_id=NULL,$vch_no=NULL) //$from_ledger should start with C for customer or L for ledger, from_ledger: debit and to_ledger: credit
{
	...
	$vch_counter = getTransCounterForOCID($oc_id,4);
	...
	if(checkForNumeric($amount,$to_ledger,$admin_id,$vch_no) && $to_ledger>0 && validateForNull($trans_date,$delivery_date))
	{
			...
			$sql="INSERT INTO edms_ac_debit_note (debit_note_ref_type,debit_note_ref,amount,from_ledger_id,from_customer_id,to_ledger_id,oc_id,auto_rasid_type,auto_id,trans_date, delivery_date,remarks,created_by,last_updated_by,date_added,date_modified,vch_no)
			VALUES ($debit_note_ref_type,'$debit_note_ref',$amount,$from_ledger,$from_customer,$to_ledger,$oc_id,$auto_rasid_type,$auto_id,'$trans_date','$delivery_date','$remarks',$admin_id,$admin_id,NOW(),NOW(),'$vch_no')";
			
			$result=dbQuery($sql);
			$debit_note_id=dbInsertId();
			$ac_starting_date = $accounts_settings['ac_starting_date'];
			if($vch_counter==$vch_no)
			incrementTransCounterForOCID($oc_id,4);
...
function removeDebitNote($id)
{
	...
	if(USE_BARCODE==1)
		{
			$used_barcode_tansactions = getUsedBarcodeForTransactionItemWise($id,6);
...
function updateDebitNote($id,$amount,$trans_date, $delivery_date,$to_ledger,$from_ledger,$remarks,$debit_note_ref="NA",$debit_note_ref_type=0,$oc_id=NULL,$vch_no=NULL)
{
...
if(isset($to) && validateForNull($to))
{
	...
	$sql="SELECT edms_ac_debit_note.debit_note_id,SUM(amount) ,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,vch_no
			  FROM edms_ac_debit_note WHERE edms_ac_debit_note.oc_id = $oc_id AND ";
	if(isset($from) && validateForNull($from))
...
if(isset($to) && validateForNull($to))
{
	...
	if(checkForNumeric($from_customer) || (isset($head_type) && checkForNumeric($from_ledger)  && $head_type!=2 && $head_type!=4))
	{
	$sql="SELECT edms_ac_debit_note.debit_note_id,SUM(net_amount),from_ledger_id,to_ledger_id,from_customer_id
			  FROM edms_ac_debit_note, edms_ac_debit_note_item WHERE edms_ac_debit_note_item.debit_note_id = edms_ac_debit_note.debit_note_id AND edms_ac_debit_note.oc_id = $oc_id AND  ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
	...
	$sql=" UNION ALL SELECT edms_ac_debit_note.debit_note_id,SUM(net_amount),from_ledger_id,to_ledger_id,from_customer_id
			  FROM edms_ac_debit_note, edms_ac_debit_note_nonstock WHERE edms_ac_debit_note_nonstock.debit_note_id = edms_ac_debit_note.debit_note_id AND edms_ac_debit_note.oc_id = $oc_id AND  ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
	if(isset($to) && validateForNull($to))  
...
if(isset($to) && validateForNull($to))
{
	...
	$sql=$sql." ,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,vch_no
			  FROM ";
			  
			  $sql=$sql."(SELECT edms_ac_debit_note.debit_note_id, edms_ac_debit_note_item.amount, edms_ac_debit_note_item.net_amount, ledger_id, from_ledger_id, to_ledger_id, from_customer_id, oc_id,auto_rasid_type,auto_id, trans_date, edms_ac_debit_note.created_by, edms_ac_debit_note.last_updated_by, edms_ac_debit_note.date_added, edms_ac_debit_note.date_modified,vch_no FROM edms_ac_debit_note , edms_ac_debit_note_item WHERE edms_ac_debit_note.debit_note_id = edms_ac_debit_note_item.debit_note_id AND edms_ac_debit_note.oc_id = $oc_id AND ";
			  if(isset($from) && validateForNull($from))
				$sql=$sql."trans_date>='$from' 
					  AND ";
				if(isset($to) && validateForNull($to))  
				...
				$sql=$sql." from_customer_id=$from_customer";
			  $sql=$sql." UNION ALL SELECT edms_ac_debit_note.debit_note_id, edms_ac_debit_note_nonstock.amount, edms_ac_debit_note_nonstock.net_amount, ledger_id, from_ledger_id, to_ledger_id, from_customer_id, oc_id,auto_rasid_type,auto_id, trans_date, edms_ac_debit_note.created_by, edms_ac_debit_note.last_updated_by, edms_ac_debit_note.date_added, edms_ac_debit_note.date_modified,vch_no FROM edms_ac_debit_note , edms_ac_debit_note_nonstock WHERE edms_ac_debit_note.debit_note_id = edms_ac_debit_note_nonstock.debit_note_id AND edms_ac_debit_note.oc_id = $oc_id AND ";
			  if(isset($from) && validateForNull($from))
				$sql=$sql."trans_date>='$from' 
					  AND ";
				if(isset($to) && validateForNull($to))  
			  ...
			  $sql=$sql." LEFT JOIN (SELECT f.debit_note_id, SUM( tax_amount ) AS total_tax
FROM edms_ac_debit_note_tax f
GROUP BY f.debit_note_id
)h ON i.debit_note_id = h.debit_note_id WHERE i.oc_id = $oc_id  AND DATE_FORMAT(trans_date,'%c')=$month_id AND DATE_FORMAT(trans_date,'%Y') = $year GROUP BY i.debit_note_id";	
	$result=dbQuery($sql);
...
if(isset($to) && validateForNull($to))
{
	...
	$sql=$sql.",SUM(amount) as total_amount,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,vch_no
			  FROM ";
			  ...
			  $sql=$sql." WHERE i.oc_id = $oc_id  AND DATE_FORMAT(trans_date,'%c')=$month_id AND DATE_FORMAT(trans_date,'%Y') = $year GROUP BY month_year";	  		  
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
...
if(isset($to) && validateForNull($to))
{
	...
	$sql=$sql." ,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified, remarks,vch_no
			  ...
			  $sql=$sql." edms_ac_debit_note AS i ";
			  $sql=$sql."  LEFT JOIN (SELECT f.debit_note_id, SUM( tax_amount ) AS total_tax
FROM edms_ac_debit_note_tax f
GROUP BY f.debit_note_id
)h ON i.debit_note_id = h.debit_note_id
WHERE i.oc_id = $oc_id ";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT SUM(net_amount) as total_amount, SUM(net_amount)/SUM(quantity) as avg_rate, SUM(quantity) as quantity, CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year,vch_no
