...
function insertBank($name,$branch){
	...
	try
	{
		...
		$sql="INSERT INTO
		      edms_bank (bank_name, created_by, last_updated_by, date_added, date_modified)
			  VALUES
...
function insertBankBranch($bank_id,$branch)
{
	try
	{
		...
		if(validateForNull($branch) && !$duplicate && checkForNumeric($bank_id))
		{
			...
			$admin_id=$_SESSION['adminSession']['admin_id'];
		$sql="INSERT INTO
		      edms_bank_branch (branch_name, bank_id)
			  VALUES
			  ('$branch',  $bank_id)";
		$result=dbQuery($sql);	
		return dbInsertId();	
...
{
	$sql="SELECT edms_bank.bank_id, bank_name
		  FROM 
		  edms_bank
		  WHERE edms_bank.bank_id=$id";
		$result=dbQuery($sql);	
...
function checkIfBankInUse($bankId)
{
	...
	$sql="SELECT bank_id 
		  FROM edms_agency_settle_cheque
		  WHERE bank_id=$bankId LIMIT 0, 1";
	$result1=dbQuery($sql);
...
function deleteBankBranch($id)
{
	...
	{
		$sql="DELETE FROM edms_bank_branch
		      WHERE branch_id=$id";
		$result=dbQuery($sql);	
		return "success";  
		}
...
{
	$sql="SELECT bank_id FROM edms_bank WHERE bank_name='$name'";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
	if(dbNumRows($result)>0)
	{
