...
<h4 class="headingAlignment no_print">Customer Outstanding Reports</h4>
<?php 
					...
					<select name="group_id" class="broker selectpicker"   id="group_id" >
						 ...
						 $brokers = listCustomerGroups();
                            ...
                            foreach($brokers as $broker)
                              {
                             ?>
                             <option value="<?php echo $broker['group_id'] ?>" <?php if(isset($_SESSION['cToBCReport']['group_id'])){ if($broker['group_id']==$_SESSION['cToBCReport']['group_id']) { ?> selected="selected" <?php }} ?>><?php echo $broker['group_name'] ?></option					>
                             <?php } 
 ...
 <?php if(isset($_SESSION['cToBCReport']['emi_array']))
{
		...
		$outstanding_greater=-9999999999999999999999999;
	 ?>    
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    <div class="showColumns">
     ...
     <input class="showCB" type="checkbox" id="3" checked="checked"   /><label class="showLabel" for="3">Customer DR</label> 
        <input class="showCB" type="checkbox" id="4" checked="checked"   /><label class="showLabel" for="4">Customer CR</label> 
        <input class="showCB" type="checkbox" id="5" checked="checked"  /><label class="showLabel" for="5">Name</label> 
         <input class="showCB" type="checkbox" id="6" checked="checked"  /><label class="showLabel" for="6">Address</label> 
          <input class="showCB" type="checkbox" id="7" checked="checked"  /><label class="showLabel" for="7">Contact No</label> 
    </div>
    ...
    <tbody>
		...
		foreach($emi_array as $emi)
		{
			...
			if(validateForNull($_SESSION['cToBCReport']['to']))
			{
			$upto = $_SESSION['cToBCReport']['to'];
			...
			$upto = getNextDate(getLatestTransactionDateForLedgerId('C'.$emi['customer_id']));
			
		$total_customer_outstanding = getOpeningBalanceForLedgerForDate('C'.$emi['customer_id'],$upto);
         ...
         <tr class="resultRow">
         	<td class="no_print"><input type="checkbox" class="selectTR" name="selectTR[]" value="<?php echo $emi['customer_id']; ?>"  /></td>
           ...
           <td ><?php if($total_customer_outstanding>=0) echo number_format($total_customer_outstanding,2,'.','');  ?>
          </td>
             <td ><?php if($total_customer_outstanding<0) echo number_format(-$total_customer_outstanding,2,'.',''); ?>
          </td>
            ...
            <td><?php   echo $emi['customer_name']; ?></td>
             ...
             <td class="no_print"> <a href="<?php echo WEB_ROOT.'admin/customer/index.php?view=details&id='.$emi['customer_id']; ?>"><button title="View this entry" type="button" class="btn viewBtn"><span class="view">V</span></button></a>
            </td>
