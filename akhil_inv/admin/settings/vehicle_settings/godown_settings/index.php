...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		...
		$result=insertGodown($_POST["name"], $_POST["address"], $_POST["city"], $_POST["contactNo"]);
		if($result=="success")
				{
				$_SESSION['ack']['msg']="Godown successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteGodown($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Godown deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Godown! Godown already in use!";
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updateGodown($_POST["lid"],$_POST["name"], $_POST["address"], $_POST["city"],  $_POST["contactNo"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Godown updated Successfuly!";
...
$jsArray=array("jquery.validate.js","validators/godown.js");
