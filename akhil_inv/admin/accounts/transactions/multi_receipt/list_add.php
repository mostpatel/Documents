...
{
	$to_customer_id = $_GET['cid'];
}

if(is_numeric($to_ledger_id) || is_numeric($to_customer_id))
{
			...
			{
				$ledger_name = getCustomerNameByCustomerId($to_customer_id);
			}
}
...
<a href="<?php echo WEB_ROOT; ?>admin/accounts/transactions/multi_receipt/index.php"><button class="btn btn-success"> Receipt</button></a>
    ...
    <?php } ?>
     ...
     <br><br>
       <?php if(is_numeric($to_customer_id)) { ?>
          <a href="<?php echo WEB_ROOT.'admin/customer/index.php?view=details&id='.$to_customer_id; ?>"><button title="View this entry" class="btn viewBtn btn-success">Back To Customer Profile</button></a>
          <?php } ?>
...
<input type="hidden" name="oc_id" id="oc_id" value="<?php echo $oc_id ?>" />
<?php if(isset($_GET['sales_id']) && is_numeric($_GET['sales_id'])) { ?>
...
<td>To (Credit)<span class="requiredField">* </span> : </td>
                ...
                <?php if(is_numeric($to_ledger_id) || is_numeric($to_customer_id)) { ?>
					<input type="hidden" class="to_ledger1" name="to_ledger_id[]" value="<?php if(is_numeric($to_customer_id)) echo getCustomerLedgerNameFromLedgerNameLedgerId('C'.$to_customer_id); else if(is_numeric($to_ledger_id)) echo getCustomerLedgerNameFromLedgerNameLedgerId('L'.$to_ledger_id);  ?>"  /> 
                 <?php }else{ ?>   
...
<td>To (Credit)<span class="requiredField">* </span> : </td>
				<td>
                <?php if(is_numeric($to_ledger_id) || is_numeric($to_customer_id)) { ?>
					<input type="hidden" class="to_ledger" name="to_ledger_id[]" value="<?php if(is_numeric($to_customer_id)) echo getCustomerLedgerNameFromLedgerNameLedgerId('C'.$to_customer_id); else if(is_numeric($to_ledger_id)) echo getCustomerLedgerNameFromLedgerNameLedgerId('L'.$to_ledger_id);  ?>"  /> 
                 <?php }else{ ?>   
