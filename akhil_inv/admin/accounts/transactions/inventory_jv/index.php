...
if(isset($_GET['view']))
{
	...
	else if($_GET['view']=='details')
	{
        ...
        if(is_array($sale) && checkForNumeric($sale['inventory_jv_id']))
		{
			if($sale['inventory_jv_mode']==1)
			{
				...
				header("Location: ".WEB_ROOT."admin/accounts/transactions/debit_inventory_jv/index.php?view=details&id=".$sales_id);
				exit;
			}
			else if($sale['inventory_jv_mode']==2)
			{
				
					if(count($_SESSION['back_links'])>0)	
               		array_pop($_SESSION['back_links']);
				
				header("Location: ".WEB_ROOT."admin/accounts/transactions/credit_inventory_jv/index.php?view=details&id=".$sales_id);
				exit;
				
			}
			
		}
		else if(!is_numeric($sale['inventory_jv_id']))
		{
	...
	else if($_GET['view']=='edit')
	{
        ...
        if(is_array($sale) && checkForNumeric($sale['inventory_jv_id']))
		{
			if($sale['inventory_jv_mode']==1)
			{
				...
				header("Location: ".WEB_ROOT."admin/accounts/transactions/debit_inventory_jv/index.php?view=edit&id=".$sales_id);
				exit;
			}
			else if($sale['inventory_jv_mode']==2)
			{
				...
				header("Location: ".WEB_ROOT."admin/accounts/transactions/credit_inventory_jv/index.php?view=edit&id=".$sales_id);
				exit;
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{
						...
						$result=insertInventoryJV($_POST["item_id"],$_POST['rate'],$_POST['quantity'],$_POST["ns_item_id"],$_POST['ns_rate'],$_POST['ns_quantity'],$_POST['payment_date'],$_POST['remarks'],$_POST['godown_id'],$_POST['ns_godown_id'],NULL,0,NULL,$_POST['unit_id'],$_POST['ns_unit_id']); // $cheque_return is 0 when inserting a payment			
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				...
				header("Location: ".WEB_ROOT."admin/accounts/transactions/inventory_jv/index.php?view=details&id=".$_GET['lid']);
				exit;
				}
			}
			else
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updateInventoryNonStockItemJV($_POST["id"],$_POST["item_id"],$_POST['rate'],$_POST['quantity'],$_POST["ns_item_id"],$_POST['ns_rate'],$_POST['ns_quantity'],$_POST['payment_date'],$_POST['remarks'],$_POST['godown_id'],$_POST['ns_godown_id'],NULL,NULL,$_POST['unit_id'],$_POST['ns_unit_id']);
				if($result=="success")
				{	
