	...
	<td >
		...
		$trans_item_unit_details = getTransItemUnitByInventoryJvItemId($sales_item['inventory_item_jv_id']);
            ...
            <tbody id="p<?php echo $i; ?>">
            	<tr >
                    <td><input name="item_id[]" type="text" class="inventory_item_autocomplete" value="<?php echo getFullItemNameFromItemId($sales_item['item_id'],0,$sales_item['barcode_transaction_id']); ?>"  <?php $in_use=0; if(is_numeric($sales_item['barcode_transaction_id'])) { echo CheckIfLatestTransactionForBarcode($sales_item['barcode'],$sales_id,8); if(!CheckIfLatestTransactionForBarcode($sales_item['barcode'],$sales_id,8)) { $in_use=1; ?> readonly title="Delete Further Transaction to Edit!" <?php  }  } ?>/></td>
