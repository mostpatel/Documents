...
<h4 class="headingAlignment">Inwards Inventory JV Details</h4>
<?php 
...
<td>Type :</td>
<td>
	<?php echo getJVTypeNameById($sale['jv_type_id']) ?>
</td>
</tr>
...
<?php if(is_array($debit_items) && count($debit_items)) { ?>
<h4 class="headingAlignment">Inwards</h4>
	...
	<td >
    	<table width="100%" class="adminContentTable productPurchaseTable" id="nonStockSaleTable">
    		<tr>
             ...
             <?php $total_debit_amount = 0; if(count($debit_items)>0) { 
			 for($i=1;$i<=count($debit_items);$i++) { 
			$sales_item=$debit_items[$i-1];
			$total_debit_amount = $total_debit_amount + $sales_item['amount'];
			
			?>
            <tbody id="p<?php echo $i; ?>">
            	<tr >
                    <td><?php echo getItemNameFromItemId($sales_item['item_id']); $barcode = $sales_item['barcode']; if(validateForNull($barcode))  echo "<br> <img  src='".WEB_ROOT."lib/barcode.php?text=".$barcode."' />"."<br>".$barcode; ?></td>
                    <td align="center"><?php echo getGodownNameFromGodownId($sales_item['godown_id']); ?> </td>
                    <td align="center"><?php echo number_format($sales_item['quantity'],2); ?></td>
                     <td align="center"><?php echo number_format($sales_item['rate'],2); ?> Rs</td>
                     <td align="center"><?php echo number_format($sales_item['amount'],2); ?> Rs</td>
...
<h4 class="headingAlignment">Inwards (Debit) Total : <span id="ns_total"><?php echo $total_debit_amount; ?> Rs</span></h4>
<?php } ?>
...
<a href="<?php echo WEB_ROOT."admin/accounts/transactions/debit_inventory_jv" ?>"><input type="button" class="btn btn-success" value="Back"/></a>
</td>
</tr>
