...
$tax_form=getTransTaxFormByPurchaseId($purchase_id);
$purchase_items = getInventoryItemForPurchaseId($purchase_id);	// tax details inside the array
$ns_items = getNonStockItemForPurchaseId($purchase_id);
}
else
exit;
...
<h4 class="headingAlignment"> Edit Purchase </h4>
<?php 
...
<td width="220px">Purchase Date<span class="requiredField">* </span> : </td>
				<td>
					<input type="text" name="payment_date" id="payment_date" class="datepicker1" placeholder="click to select date!" value="<?php echo date('d/m/Y',strtotime($purchase['trans_date'])); ?>" autofocus /><span class="DateError customError">Please select a date!</span>
                            </td>
</tr>
...
<td>By (Debit)<span class="requiredField">* </span> : </td>
				<td>
                    ...
                    <?php
					$bank_cash_ledgers=listPurchaseLedgers(false,$payment['oc_id']);
    	...
    	<table width="100%" class="adminContentTable productPurchaseTable" id="productPurchaseTable">
            ...
            <tbody style="display:none" id="p0">
            	<tr>
                    ...
                    <td><input type="text" name="quantity[]" class="item_quantity" style="width:35px;" value="1" onchange="onchangeQuantity(this);" /><span style="color:#f00;font-size:12px;"></span><select name="unit_id[]" style="width:35px;" class="item_unit">
                      ...
                      <?php 
							...
							{ ?>
                                <select class="sales_ledger_item_wise" name="sales_ledger_id[]" onchange="changeSalesPurchaseLedger(this);" style="margin-bottom:5px;width:170px;">
                                    ...
                                    <?php
                                    $bank_cash_ledgers=listPurchaseLedgers(false,$purchase['oc_id']);
			...
			$trans_item_unit_details = getTransItemUnitByPurchaseItemId($purchase_item['purchase_item_id']);
            ...
            <tbody id="p<?php echo $i; ?>">
            	<tr >
                    ...
                    <td><input type="text" name="quantity[]" class="item_quantity" style="width:35px;"  onchange="onchangeQuantity(this);" value="<?php if(!is_numeric($trans_item_unit_details['quantity'])) echo $sales_item['quantity']; else echo $trans_item_unit_details['quantity']; ?>" /><span style="color:#f00;font-size:12px;display:block;"><?php echo getRemainingQuanityForItemForDate($purchase_item['item_id'],$purchase_item['godown_id']); ?></span><select name="unit_id[]" style="width:65px;" class="item_unit"><?php $units = getUnitsForItemId($purchase_item['item_id']); foreach($units as $unit) { ?>
                      ...
                      <?php
							...
							{ ?>
                                <select class="sales_ledger_item_wise" name="sales_ledger_id[]" onchange="changeSalesPurchaseLedger(this);" style="margin-bottom:5px;width:170px;">
                                    ...
                                    {
                                    ?>
                                    <option value="<?php echo $bank_cash_ledger['ledger_id']; ?>" <?php if(is_numeric($purchase_item['ledger_id']) && $bank_cash_ledger['ledger_id']==$purchase_item['ledger_id']) { ?> selected="selected" <?php } ?>><?php echo $bank_cash_ledger['ledger_name']; ?></option>			
                                    <?php	
                                        }
                                     ?>
                                </select>
                                <select style="margin-bottom:5px;width:170px;" name="tax_class_id[]" class="tax_class_id" onchange="changeTaxClass(this);"> <?php $bank_cash_ledgers=getTaxClassByLedgerId($purchase_item['ledger_id']);   foreach($bank_cash_ledgers as $bank_cash_ledger)
                                    {
                                    ?>
                                    <option value="<?php echo $bank_cash_ledger['tax_class_id']; ?>" <?php if(is_numeric($purchase_item['tax_class_id']) && $bank_cash_ledger['tax_class_id']==$purchase_item['tax_class_id']) { ?> selected="selected" <?php } ?>><?php echo $bank_cash_ledger['tax_class']; ?></option>			
                                    <?php	
                                        } ?>
                                </select>
                                <select class="tax_group" name="tax_group_id[]" style="width:170px;" onchange="onchangeTaxGroup(this);">
                                	<?php $bank_cash_ledgers=getTaxGroupsForTaxClassId($purchase_item['tax_class_id']);   foreach($bank_cash_ledgers as $bank_cash_ledger)
                                    {
                                    ?>
                                    <option id="tax<?php if($bank_cash_ledger['in_out']!=3) echo getTotalTaxPercentForTaxGroup($bank_cash_ledger['tax_group_id']); else echo 0; ?>"  value="<?php echo $bank_cash_ledger['tax_group_id']; ?>" <?php if(is_numeric($purchase_item['tax_group_id']) && $bank_cash_ledger['tax_group_id']==$purchase_item['tax_group_id']) { ?> selected="selected" <?php } ?>><?php echo $bank_cash_ledger['tax_group_name']; ?></option>			
                                    <?php	
                                        } ?>
                                </select>
...
<td>Bill No: </td>
				<td>
					<input type="text" id="bill_no" name="bill_no"  value="<?php echo $purchase['bill_no']; ?>" /> 
                  
                            </td>

<td>Bill Date : </td>
				<td>
					<input type="text" id="bill_date" name="bill_date" class="datepicker4" value="<?php if( $purchase['bill_date']!="1970-01-01" ) echo date('d/m/Y',strtotime($purchase['bill_date'])); ?>" /> 
                  
                            </td>
</tr>
<tr>
<td width="250px;">Purchase Type<span class="requiredField">* </span> : </td>
				<td>
...
$( ".inventory_item_autocomplete" ).autocomplete({
	...
	select=true; $(event.target).val(ui.item.label);
