...
var tax_select = $(item_option_el).parent().next().next().next().next().next().next().children()[0];
...
var item_name = $(godown_select).parent().prev().children()[1].value;
...
// Before adding new we must remove previously loaded elements
 ...
 xmlhttp1.open('GET', "getRateQuantityAndTaxForItem.php?id="+item_name+"&state="+godown_id, true );    
  xmlhttp1.send(null);
