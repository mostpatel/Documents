...
require_once("contact-person-functions.php");
...
function listCustomer($our_company_id=false)
{
	...
	$sql="SELECT edms_customer.customer_id,customer_name,customer_address, customer_address2,customer_pincode,city_id,area_id,pan_no,tin_no,cst_no,service_tax_no,email,notes,edms_customer.oc_id, our_company_id,created_by,last_updated_by,date_added,date_modified,IF(opening_balance_id IS NOT NULL,edms_customer_opening_balance.opening_balance,0) as opening_balance,IF(opening_balance_id IS NOT NULL,edms_customer_opening_balance.opening_cd,0) as opening_cd, tin_no, pan_no, service_tax_no, cst_no ,customer_through,customer_no
	FROM edms_customer 
...
function insertCustomer($name,$address,$city_id,$area,$pincode,$contact_no,$human_proof_type_id,$proofno,$proofImg,$pan_no="NA",$tin_no="NA",$notes="NA",$opening_balance=0,$opening_cd=0,$cst_no="NA",$service_tax_no="NA",$email="",$customer_no="NA",$date_of_joining=NULL,$cp_name="NA",$cp_contact_no=NULL,$cp_dob=NULL,$prefix=NULL,$contact_prefix=NULL,$address2="",$cp_anniversary=NULL,$customer_through=1){	
	try
	{
...
$our_company_id = $oc_id;
	//	$area_id=insertArea($area,$city_id);
		...
		$name = ucwords(strtolower($name));
			...
			if(validateForNull($name,$address,$contact_no,$date_of_joining,$cp_name,$cp_contact_no) && checkForNumeric($city_id,$pincode,$our_company_id,$area_id,$customer_through)  && !empty($contact_no) && ((!checkForDuplicateCustomer($name,$id) && DUPLICATE_CUSTOMER_NAME==1) || DUPLICATE_CUSTOMER_NAME==0))
			{
				
					$date_of_joining = str_replace('/', '-', $date_of_joining);
					$date_of_joining = date('Y-m-d',strtotime($date_of_joining));
				$date_of_joining = getTodaysDate();
				$area_name = getAreaNameByID($area_id);
				
				$area_name = substr($area_name,0,2);
				
				$address=trim($address);
			    $customer_counter=getChallanCounterForOCID($oc_id);
				$customer_no="GA".$area_name.date('dmY',strtotime($date_of_joining)).$customer_counter;
				$sql="INSERT INTO 	edms_customer(customer_name,customer_address, customer_address2,customer_pincode,city_id,area_id,pan_no,tin_no,cst_no,service_tax_no,email,notes,oc_id, our_company_id,created_by,last_updated_by,date_added,date_modified,customer_no,date_of_joining,prefix_id,customer_through)				VALUES 
						('$name', '$address', '$address2' ,$pincode, $city_id, $area_id,'$pan_no', '$tin_no', '$cst_no', '$service_tax_no', '$email' , '$notes', $oc_id, $our_company_id, $admin_id, $admin_id, NOW(), NOW(),'$customer_no','$date_of_joining',$prefix,$customer_through)";
				$result=dbQuery($sql);
				...
				insertContactPerson($cp_name,$cp_contact_no,0,$cp_dob,$email,$customer_id,$contact_prefix,1,$cp_anniversary);
				insertShippingLocation($name,$address,$city_id,$area_id,$cp_name,$cp_contact_no,NULL,NULL,NULL,NULL,$customer_id,1,$address2);
...
function updateCustomer($id,$name,$address,$city_id,$area,$pincode,$contact_no,$human_proof_type_id,$proofno,$proofImg,$pan_no="NA",$tin_no="NA",$notes="NA",$opening_balance=0,$opening_cd=0,$cst_no="NA",$service_tax_no="NA",$email = '',$customer_no="NA",$date_of_joining=NULL,$cp_name="NA",$cp_contact_no=NULL,$cp_dob=NULL,$prefix="NULL",$contact_prefix="NULL",$address2="",$cp_anniversary=NULL,$customer_through=1){
	...
	try
	{
		...
		$cp_contact_no = clean_data($cp_contact_no);
		$cp_dob = clean_data($cp_dob);
		$cp_anniversary = clean_data($cp_anniversary);
		if(!validateForNull($email))
		$email='';
		...
		//	$area_id=insertArea($area,$city_id);
			$area_id = $area;
			if(validateForNull($name,$address,$date_of_joining,$cp_name) && checkForNumeric($city_id,$area_id,$cp_contact_no,$customer_through) && $contact_no!=null && !empty($contact_no) && ((!checkForDuplicateCustomer($name,$id) && DUPLICATE_CUSTOMER_NAME==1) || DUPLICATE_CUSTOMER_NAME==0) )
			{
				$address=trim($address);
				...
				$sql="UPDATE edms_customer
				     SET customer_name = '$name', customer_address = '$address', customer_address2 = '$address2', customer_pincode = $pincode , city_id = $city_id, area_id = $area_id,  pan_no = '$pan_no', tin_no = '$tin_no', cst_no = '$cst_no', service_tax_no = '$service_tax_no', email = '$email', notes='$notes', last_updated_by = $admin_id, date_modified = NOW(),  prefix_id = $prefix, customer_through = $customer_through
					 WHERE customer_id=$id";
				...
				deleteContactPersonDetailsForCustomerId($id);
				
				insertContactPerson($cp_name,$cp_contact_no,0,$cp_dob,$email,$id,$contact_prefix,1,$cp_anniversary);
				deleteShippingLocationForcustomer_id($id);
				insertShippingLocation($name,$address,$city_id,$area_id,$cp_name,$cp_contact_no,NULL,NULL,NULL,NULL,$id,1,$address2);
...
function getCustomerDetailsByCustomerId($customer_id)
{
	...
	if(checkForNumeric($customer_id))
	{
		$sql="SELECT edms_customer.customer_id,customer_name,customer_address, customer_address2,customer_pincode,city_id, area_id, edms_customer.oc_id, current_balance, current_balance_cd, pan_no, tin_no,cst_no,service_tax_no,email, notes,IF(opening_balance_id IS NOT NULL,edms_customer_opening_balance.opening_balance,0) as opening_balance,IF(opening_balance_id IS NOT NULL,edms_customer_opening_balance.opening_cd,0) as opening_cd, customer_no, date_of_joining, prefix_id, customer_through FROM edms_customer LEFT JOIN edms_customer_opening_balance ON edms_customer.customer_id = edms_customer_opening_balance.customer_id AND edms_customer_opening_balance.oc_id = $our_company_id
