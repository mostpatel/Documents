...
{
	$sql="SELECT payment_id,payment_ref_type,payment_ref,amount,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,vch_no
...
function getPaymentsForVehicleForCustomerId($customer_id)
{
	if(checkForNumeric($customer_id))
	{
		...
		$sql="SELECT payment_id,payment_ref,amount,from_ledger_id,to_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,created_by,last_updated_by,date_added,date_modified,remarks,vch_no
			  FROM edms_ac_payment
...
{
	$sql="SELECT payment_id,payment_ref,amount,to_ledger_id,from_ledger_id,from_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,created_by,last_updated_by,date_added,date_modified,remarks,vch_no
...
// auto_rasid_type = 2 for financer payment
{
	...
	$vch_counter = getTransCounterForOCID($oc_id,5);
	if(!checkForNumeric($vch_no))
	{
	$vch_no = $vch_counter;	
	}
	if(checkForNumeric($amount,$to_ledger,$admin_id,$vch_no) && $to_ledger>0 && validateForNull($trans_date))
	{
			
			$sql="INSERT INTO edms_ac_payment (payment_ref_type,payment_ref,amount,from_ledger_id,from_customer_id,to_ledger_id,oc_id,auto_rasid_type,auto_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,vch_no)
			VALUES ($payment_ref_type,'$payment_ref',$amount,$from_ledger,$from_customer,$to_ledger,$oc_id,$auto_rasid_type,$auto_id,'$trans_date','$remarks',$admin_id,$admin_id,NOW(),NOW(),'$vch_no')";
			
			$result=dbQuery($sql);
			$payment_id = dbInsertId();
			$ac_starting_date = $accounts_settings['ac_starting_date'];
			if($vch_counter==$vch_no)
			incrementTransCounterForOCID($oc_id,5);
...
function updatePayment($id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,$payment_ref="NA",$payment_ref_type=0,$oc_id=NULL,$payment_mode_id=-1,$chq_date="01/01/1970",$chq_no="000000",$bank_name="NA",$branch_name="NA",$vch_no=NULL)
{
