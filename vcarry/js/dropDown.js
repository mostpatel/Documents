...
function createDropDownVehiclePurchaseLedger(vehicle_id)
{	
	var xmlhttp1;
if (window.XMLHttpRequest)
  {// code for IE7+, Firefox, Chrome, Opera, Safari
...
document.getElementById("customer_purchase").options.add(optn);

} 
    }
  }
  
  xmlhttp1.open('GET', "getRelatedPurchaseLedger.php?id="+vehicle_id, true );    
  xmlhttp1.send(null);
...
function createDropDownAreaCustomer(city_id)
{
	...
	createDropDown("getAreaFromCity.php?id="+city_id,"city_area1",null);
	}	

function createDropDownVehicleTypeDriver(vehicle_type_id)
{
	
	createDropDown("getDriversForVehicleType.php?id="+vehicle_type_id,"driver_id",null);
	}		
...
function createDropDownPaymentRef(url,id)  
{
...
function createDropDownPaymentRefMulti(url,el)  
{
...
function createDropDownItemItemUnit(url,id)  
{
