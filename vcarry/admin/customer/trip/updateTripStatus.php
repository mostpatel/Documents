...
<h4 class="headingAlignment"> Update Driver Details </h4>
<?php 
...
<td>Vehicle Type :</td>
            <td > 
           <form action="index.php?action=updateTripStatus" method="post">
            <input type="hidden" name="trip_id" value="<?php echo $trip_id; ?>">
            <select name="status_id" id="status_id" >
           <?php  $accepted_drivers = getAllTripStatus(); 
		   foreach($accepted_drivers as $driver)
		   {
		   ?>
           <option value="<?php echo $driver['status_id']; ?>" <?php if($driver['status_id']==$trip['trip_status']) { ?> selected <?php } ?>><?php echo $driver['status']; ?></option>
           <?php } ?>
            </td>
            </tr>
...
<td width="250px;"></td>
<td>
<input type="submit"  class="btn editBtn" value="Confirm Status" />
<a href="<?php echo '../index.php?view=details&id='.$customer_id; ?>"><button type="button" class="btn btn-warning" >Back to customer</button></a>
</form>
</td>
</tr>
</table>
