...
{      
$sql = "SELECT file_agreement_no FROM edms_file WHERE our_company_id=$oc_id AND agency_id=$agency_id AND file_agreement_no='$value' AND file_status!=3";
if(isset($file_id) && is_numeric($file_id))
$sql=$sql." AND file_id!=$file_id ";	
}
else if($agency_id=="NULL" && is_numeric($our_company_id))
{
	$sql = "SELECT file_agreement_no FROM edms_file WHERE our_company_id=$oc_id AND oc_id=$our_company_id AND file_agreement_no ='$value' AND file_status!=3"; 
     if(isset($file_id) && is_numeric($file_id))
	$sql=$sql." AND file_id!=$file_id ";	  
	}	
