...
$payment_id = $_GET['state'];
else
exit;
$finanacer_payment=getPaymentById($payment_id); 
$financer_receipts = getUnPaidReceiptsForFinancerId($financer_id);
$receipt_ids_array=getReceiptsForFinancerPaymentId($payment_id); ?>
...
<td width="220px">Payment Date<span class="requiredField">* </span> : </td>
				<td>
					<input type="text" name="payment_date" id="payment_date" class="datepicker1" placeholder="click to select date!" value="<?php echo date('d/m/Y',strtotime($finanacer_payment['trans_date'])); ?>"/><span class="DateError customError">Please select a date!</span>
                            </td>
</tr>
...
<td width="220px">Amount<span class="requiredField">* </span> : </td>
				<td>
					<input type="text" name="amount" id="total_amount" placeholder="Only Digits!" value="<?php echo $finanacer_payment['amount']; ?>" readonly="readonly" />
                            </td>
</tr>
...
<td>To (Credit)<span class="requiredField">* </span> : </td>
				<td>
					...
					{
					?>
                    <option value="<?php echo $bank_cash_ledger['ledger_id']; ?>" <?php if($bank_cash_ledger['ledger_id']==$finanacer_payment['to_ledger_id']) { ?> selected="selected" <?php } ?>><?php echo $bank_cash_ledger['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
                    </select>
...
<td>
<textarea name="remarks" id="remarks"><?php echo $finanacer_payment['remarks']; ?></textarea>
</td>
</tr>
...
<a href="<?php echo WEB_ROOT."admin/accounts/" ?>"><input type="button" class="btn btn-success" value="Back"/></a>
     ...
     <?php if((isset($financer_receipts) && is_array($financer_receipts)) || (isset($receipt_ids_array) && is_array($receipt_ids_array)))
	{
    ...
    <tbody>
		...
		foreach($receipt_ids_array as $financer_receipt)
		{
			$receipt_id = $financer_receipt['receipt_id'];
         ...
         <td class="no_print"><input type="checkbox" class="selectTR" name="selectTR[]" value="<?php echo $receipt_id; ?>" checked="checked" /></td>
        	<td><?php echo ++$i; ?></td>
