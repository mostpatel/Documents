...
$last_Added_sales_id = getLastAddedSale();
 ?>
<?php if(isset($_GET['cid']) && is_numeric($_GET['cid']) && isset($_GET['trip_id']) && is_numeric($_GET['trip_id']))
{
...
alert('Invoice Already Created!');
  window.location.href = '<?php echo  WEB_ROOT ?>admin/customer/index.php?view=details&id=<?php echo $customer_id; ?>';
</script>	
<?php	
}
...
<a href="<?php echo WEB_ROOT; ?>admin/accounts/transactions/multi_receipt/index.php"><button class="btn btn-success"> Receipt</button></a>
    ...
    <?php } ?>
     ...
     <br><br>
      <td class="no_print"> <a href="<?php echo WEB_ROOT.'admin/accounts/transactions/sales_inventory/index.php?view=invoice&id='.$last_Added_sales_id; ?>"><button title="View this entry" class="btn viewBtn btn-success">Print Last Invoice</button></a>
<div class="insideCoreContent adminContentWrapper wrapper">

<h4 class="headingAlignment"> <?php echo SALES_NAME; ?> </h4>
<?php 
...
<input type="hidden" name="auto_rasid_type" value="4"  />
<input type="hidden" name="auto_id" value="<?php echo $trip_id; ?>"  />
<?php } else { ?>
<input type="hidden" name="auto_rasid_type" value="2"  />
...
<tr>
	<td >
    	<table width="100%" class="adminContentTable productPurchaseTable ns_inventory_table" id="nonStockSaleTable">
            ...
            <tbody id="ns<?php echo $i; ?>">
            	<tr >
                    <td><input type="text" name="ns_item_id[]" class="inventory_ns_item_autocomplete"  placeholder="Select Only From  Suggestions" <?php if($i==1 && is_numeric($trip_id)) { ?> value="Trip Charges | ITE428 | " readonly <?php } ?>
                  />
                      <?php if(defined('INVOICE_ITEM_DESC') && INVOICE_ITEM_DESC==1) { ?>
                       <br />
                         <span style="padding-top:10px;display:block"> Desc : <input name="ns_item_desc[]" type="text" style="width:160px;"  <?php if($i==1 && is_numeric($trip_id)) { ?> value="<?php echo $from_location['area_name']." - ".$to_location['area_name']; ?>" readonly <?php } ?> /></span>
                         <?php  } else if(defined('INVOICE_TAX_YEARS') && INVOICE_TAX_YEARS==1) {  ?>
                         Year : <select name="ns_item_desc[]">
                       <?php $product_Desc=listProductDesc(); foreach($product_Desc as $productdesc) { ?>
                         <option value="<?php echo $productdesc['product_desc']; ?>"><?php echo $productdesc['product_desc'] ?></option>
                     ...
                     <td><input type="text" name="ns_rate[]" class="item_rate" style="width:35px;" <?php if($i==1 && is_numeric($trip_id)) { ?> value="<?php echo $trip['fare']; ?>" <?php } else { ?> value="0" <?php } ?> onchange="onchangeRateNS(this);" /></td>
