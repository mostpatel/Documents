<?php
		...
		$reportArray=getFirstPageBalanceSheet($to,$from);
		...
		$profitAndLossCurrentPeriod=$pl_sheet_current_period['nett_profit'];
		$profitAndLossOpeningPeriod=$pl_sheet_opening_period['nett_profit'];
		 ?>
			...
			foreach($transaction_array as $payment)
			{
				$child_heads=$payment['child_heads'];
				 ...
				 <tr class="resultRow mainHeadRow">
					...
					<td class="mainHeadTd"><a href="index.php?view=second&id=<?php echo $payment['head_id']; ?>"><b><?php echo $payment['head_name']; ?></b></a></td>			
              ...
              <!--      <?php if(isset($payment['child_heads']) && is_array($payment['child_heads']) && count($payment['child_heads'])>0 )
                ...
                <?php if($payment['head_id']>0) { ?>
                 <tr class="resultRow">
                    <td></td>
                    <td></td> 
					<td><a href="index.php?view=second&id=<?php echo $payment['head_id']; ?>"><b><?php echo $payment['head_name']; ?></b></a></td>			
              ...
              <!--      <?php if(isset($payment['child_heads']) && is_array($payment['child_heads']) && count($payment['child_heads'])>0 ) 
				 ...
				 <tr class="resultRow">
					...
					<td><?php if($child_head['opening_balance']<0) echo $child_head['opening_balance']; else echo $child_head['opening_balance']; ?></td>
