...
<td width="260px;">From Date : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cash_bank_book']['from'])) echo $_SESSION['cash_bank_book']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To Date : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['cash_bank_book']['to'])) echo $_SESSION['cash_bank_book']['to']; ?>"/>	
                 </td>
</tr>
...
<td>Ledger<span class="requiredField">* </span> : </td>
				<td>
                   ...
                   <select id="to_ledger" name="ledger_id" >
					...
					{
					?>
                    <option value="<?php echo 'L'.$ledger['ledger_id']; ?>" <?php if( 'L'.$ledger['ledger_id']==$_SESSION['cash_bank_book']['ledger_id']) { ?>  selected="selected"<?php } ?>><?php echo $ledger['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
...
<td>Transactions : </td>
				<td>
					<select name="transaction_type[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="1" <?php if(in_array(1,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?> >Payments</option>
                         <option value="2" <?php if(in_array(2,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Receipts</option>
                         <option value="3" <?php if(in_array(3,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Journal Entries</option>
                         <option value="4" <?php if(in_array(4,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Contra Entries</option>
                         <option value="5" <?php if(in_array(5,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Purchase Entries</option>
                         <option value="6" <?php if(in_array(6,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Sales Entries</option>
                         <option value="7" <?php if(in_array(7,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Debit Note Entries</option>
                         <option value="8" <?php if(in_array(8,$_SESSION['cash_bank_book']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Credit Note Entries</option>
                    </select>
                            </td>
</tr>
...
<hr class="firstTableFinishing" />
	...
	<div class="no_print">
 <?php if(isset($_SESSION['cash_bank_book']['entries_array']))
{
	$id=$_SESSION['cash_bank_book']['ledger_id'];
	$transaction_array=$_SESSION['cash_bank_book']['entries_array'];
	...
	if(isset($_SESSION['cash_bank_book']['from']) && validateForNull($_SESSION['cash_bank_book']['from']))
	{	
	$from=$_SESSION['cash_bank_book']['from'];
	}
	else
	{
    ...
    <tbody>
		...
		foreach($transaction_array as $payment)
		{
				 ...
				 <tr class="resultRow">
				   ...
				   <td><?php if($head_type==1) $openingBalance=$openingBalance+$payment['amount']; else if($head_type==0) $openingBalance=$openingBalance-$payment['amount']; if($openingBalance>=0) echo $openingBalance." Dr"; else echo -$openingBalance." Cr";  ?>
					...
					<td class="no_print"> <a href="<?php if(($payment['auto_rasid_type']==0 || $payment['auto_rasid_type']>100) && is_numeric($from_customer_id)) { echo WEB_ROOT.'admin/customer/payment_for_customer/index.php?view=details&id='.$payment['id']; }
					 ...
					 echo WEB_ROOT.'admin/customer/vehicle_payment/index.php?view=details&id='.$payment['id'];					 ?>"><button title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
         ...
         <?php  }
				...
				{	
					...
					if($payment['auto_rasid_type']==3)
					{
						
						$job_card= getJobCardById($payment['auto_id']);
						$invoice_no=getFinalizeDetailsForJobCard($payment['auto_id']);
					
						$job_card = $job_card['job_card_details'];
							$vehicle = getVehicleById($job_card['vehicle_id']);
					}
					else if($payment['auto_rasid_type']==5)
					{
						$sales=getSaleById($payment['auto_id']);
					}
					 ?>
					 <tr class="resultRow">
						<td class="no_print"><input type="checkbox" class="selectTR" name="selectTR"  /></td>
						<td><?php echo ++$i; ?></td>
						 ...
						 <td><?php switch($payment['auto_rasid_type']){
							 ...
							 case 2: echo "Financer";
							 		break;	
							case 3: echo "Job Card (".$job_card['job_card_no'].")<br/>".$invoice_no."<br/>".$vehicle['vehicle_reg_no'];
							 		break;	
							case 4: echo "Vehicle";
							 		break;	
							case 5: echo "Sales"."<br/>".$invoice_no;
							 		break;		
							case $payment['auto_rasid_type']>100: echo getReceiptTypeNameById($payment['auto_rasid_type']);
							 		break;								
							 } ?>
		...
		$from_ledger_id=$payment['credit_ledger_id'];
         ...
         <tr class="resultRow">
           ...
           <td><?php if($to_ledger_id==$ledger_id) $openingBalance=$openingBalance+$payment['amount'];  if($from_ledger_id==$ledger_id) $openingBalance=$openingBalance-$payment['amount']; if($openingBalance>=0) echo $openingBalance." Dr"; else echo -$openingBalance." Cr";  ?>
            </td>
				...
				$from_customer_id=$payment['credit_customer_id'];
				 ...
				 <tr class="resultRow">
					...
					<td><?php switch($payment['auto_rasid_type']){
					 ...
					 <td><?php if($head_type==3) echo $payment['amount']; ?>
					</td>
					<td><?php if($head_type!=3) echo $payment['amount']; ?>
					</td>
				   <td><?php if($head_type==3) $openingBalance=$openingBalance+$payment['amount']; else if($head_type!=3) $openingBalance=$openingBalance-$payment['amount']; if($openingBalance>=0) echo $openingBalance." Dr"; else echo -$openingBalance." Cr";  ?>
					</td>
				...
				{	
					...
					$to_customer_id=$payment['debit_customer_id'];
					 ...
					 <tr class="resultRow">
						...
						<td><?php if($head_type!=4) echo $payment['amount']; ?>
						</td>
						<td><?php if($head_type==4) echo $payment['amount']; ?>
						</td>
					   <td><?php if($head_type!=4) $openingBalance=$openingBalance+$payment['amount']; else if($head_type==4) $openingBalance=$openingBalance-$payment['amount']; if($openingBalance>=0) echo $openingBalance." Dr"; else echo -$openingBalance." Cr";  ?>
						</td>
