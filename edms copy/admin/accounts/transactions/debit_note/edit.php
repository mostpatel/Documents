...
{
$tax_form_id=getTaxFormIdForTransId($purchase_id,4);

if(is_numeric($tax_form_id))
$tax_form=getTransTaxFormByDebitNoteId($purchase_id);
	...
	<td >
    	<table width="100%" class="adminContentTable productPurchaseTable" id="productPurchaseTable">
			...
			$item_tax_details = $purchase_items[$i-1]['tax_details'];
			 $trans_item_unit_details = getTransItemUnitByDebitNoteItemId($purchase_item['debit_note_item_id']);
			?>
            <tbody id="p<?php echo $i; ?>">
            	<tr >
                     <td><input name="item_id[]" type="text" class="inventory_item_autocomplete" value="<?php echo getFullItemNameFromItemId($purchase_item['item_id'],0,$purchase_item['barcode_transaction_id']); ?>"   <?php $in_use=0; if(is_numeric($purchase_item['barcode_transaction_id'])) { if(!CheckIfLatestTransactionForBarcode($purchase_item['barcode'],$purchase_id,8)) { $in_use=1; ?> readonly title="Delete Further Transaction to Edit!" <?php  }  } ?> /></td>
                    <td><input type="text" name="quantity[]" class="item_quantity" style="width:35px;"  onchange="onchangeQuantity(this);" value="<?php if(!is_numeric($trans_item_unit_details['quantity'])) echo $sales_item['quantity']; else echo $trans_item_unit_details['quantity']; ?>" <?php if($in_use==1) { ?> readonly title="Delete Further Transaction to Edit!" <?php } ?> /><span style="color:#f00;font-size:12px;display:block;"><?php echo getRemainingQuanityForItemForDate($purchase_item['item_id'],$purchase_item['godown_id']); ?></span><select name="unit_id[]" style="width:65px;" class="item_unit"><?php $units = getUnitsForItemId($purchase_item['item_id']); foreach($units as $unit) { ?>
