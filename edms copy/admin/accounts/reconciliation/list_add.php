...
<td width="260px;">From Date : </td>
				<td>
				 <input autocomplete="off" type="text"  name="start_date" id="start_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['reconciliationEntries']['from'])) echo $_SESSION['reconciliationEntries']['from']; ?>" />	
                 </td>
</tr>
...
<td>Up To Date : </td>
				<td>
				 <input autocomplete="off" type="text"  name="end_date" id="end_date" placeholder="Click to select Date!" class="datepicker2" value="<?php if(isset($_SESSION['reconciliationEntries']['to'])) echo $_SESSION['reconciliationEntries']['to']; ?>"/>	
                 </td>
</tr>
...
<td>Ledger<span class="requiredField">* </span> : </td>
				<td>
                   ...
                   <select id="to_ledger" name="ledger_id" >
                    ...
                    <?php
					$ledgers=listAccountingLedgersForOC($oc_id);
					foreach($ledgers as $ledger)
					{
					?>
                    <option value="L<?php echo $ledger['ledger_id']; ?>" <?php if( 'L'.$ledger['ledger_id']==$_SESSION['reconciliationEntries']['ledger_id']) { ?>  selected="selected"<?php } ?>><?php echo $ledger['ledger_name']; ?></option>			
                    <?php	
						}
					 ?>
...
<td>Transactions : </td>
				<td>
					<select name="transaction_type[]" class="city_area selectpicker" multiple="multiple"  id="city_area1" >
                    	 <option value="1" <?php if(in_array(1,$_SESSION['reconciliationEntries']['transaction_array'])) { ?>  selected="selected"<?php } ?> >Payments</option>
                         <option value="2" <?php if(in_array(2,$_SESSION['reconciliationEntries']['transaction_array'])) { ?>  selected="selected"<?php } ?>>Receipts</option>
                    </select>
                            </td>
</tr>
...
<hr class="firstTableFinishing" />
	...
	<div class="no_print">
 <?php if(isset($_SESSION['reconciliationEntries']['entries_array']))
{
	$id=$_SESSION['reconciliationEntries']['ledger_id'];
	$transaction_array=$_SESSION['reconciliationEntries']['entries_array'];
	...
	if(isset($_SESSION['reconciliationEntries']['from']) && validateForNull($_SESSION['reconciliationEntries']['from']))
	{	
	$from=$_SESSION['reconciliationEntries']['from'];
	}
	else
	{
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    ...
    <thead>
        ...
        <th class="heading no_sort">Reconciliation Date</th>
    ...
    <tbody>
		...
		foreach($transaction_array as $payment)
		{
			...
			if($payment['type']==1)
			{
				...
				$reconciliation_date = getReconciliationDateForPaymentId($payment['id']);
				 ?>
				 <tr class="resultRow">
					<td><input type="hidden" name="trans_id[]" value="P<?php echo $payment['id']; ?>"  /></td>
					<td><?php echo ++$i; ?></td>
				   ...
				   <td><input type="text" name="reconciliation[]" placeholder="dd/mm/yyyy" value="<?php if(validateForNull($reconciliation_date)) echo date('d/m/Y',strtotime($reconciliation_date)); ?>"  />
					</td>
					<td class="no_print"> <a tabindex="-1" href="<?php if(($payment['auto_rasid_type']==0 || $payment['auto_rasid_type']>100) && is_numeric($from_customer_id)) { echo WEB_ROOT.'admin/customer/payment_for_customer/index.php?view=details&id='.$payment['id']; }
					 ...
					 echo WEB_ROOT.'admin/customer/vehicle_payment/index.php?view=details&id='.$payment['id'];					 ?>"><button tabindex="-1" title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
         ...
         <?php  }
				...
				{	
					...
					$reconciliation_date = getReconciliationDateForReceiptId($payment['id']);
			
					
					 ?>
					 <tr class="resultRow">
						<td><input type="hidden" name="trans_id[]" value="R<?php echo $payment['id']; ?>" /></td>
						<td>	<?php echo ++$i; ?></td>
						...
						<td><?php if($head_type==1) echo number_format($payment['amount'],2); ?>
                         ...
                         <td><?php echo $payment['remarks']; ?></td>
					    <td><input type="text" name="reconciliation[]" value="<?php if(validateForNull($reconciliation_date)) echo date('d/m/Y',strtotime($reconciliation_date)); ?>" placeholder="dd/mm/yyyy"  />
						</td>
					   
						<td class="no_print"> <a tabindex="-1" href="<?php
						   
						 if(($payment['auto_rasid_type']==0 || $payment['auto_rasid_type']>100) && is_numeric($from_customer_id)) { echo WEB_ROOT.'admin/customer/receipt/index.php?view=details&id='.$payment['id']; }else if(($payment['auto_rasid_type']==0 || $payment['auto_rasid_type']>100)) { echo WEB_ROOT.'admin/accounts/transactions/receipt/index.php?view=details&id='.$payment['id']; } else if($payment['auto_rasid_type']==2) echo WEB_ROOT.'admin/customer/financer_receipt/index.php?view=details&id='.$payment['id'].'&state='.$emi_id; else if($payment['auto_rasid_type']==3) echo WEB_ROOT.'admin/customer/vehicle/jobCard/receipt/index.php?view=details&id='.$payment['id']; else if($payment['auto_rasid_type']==4) echo WEB_ROOT.'admin/customer/vehicle_receipt/index.php?view=details&id='.$payment['id']; else if($payment['auto_rasid_type']==5) echo WEB_ROOT."admin/accounts/transactions/receipt/index.php?view=details&id=".$payment['id']; ?>"><button tabindex="-1" title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
		...
		?>
         <tr class="resultRow">
            ...
            <td><?php if($jv_type==0) // if it is a accounts ledger and a debit jv
             ...
             <td><?php echo $payment['remarks']; ?></td>
             <td><?php if($jv_type==0) $openingBalance=$openingBalance+number_format($payment['amount'],2); else if($jv_type==1) $openingBalance=$openingBalance-number_format($payment['amount'],2); if($openingBalance>=0) echo number_format($openingBalance,2)." Dr"; else echo number_format(-$openingBalance,2)." Cr";  ?>
            </td>
		...
		$from_ledger_id=$payment['credit_ledger_id'];
         ...
         <tr class="resultRow">
            ...
            <td><?php if($from_ledger_id==$ledger_id) echo number_format($payment['amount'],2); ?>
             ...
             <td><?php echo $payment['remarks']; ?></td>
           <td><?php if($to_ledger_id==$ledger_id) $openingBalance=$openingBalance+number_format($payment['amount'],2);  if($from_ledger_id==$ledger_id) $openingBalance=$openingBalance-number_format($payment['amount'],2); if($openingBalance>=0) echo number_format($openingBalance,2)." Dr"; else echo number_format(-$openingBalance,2)." Cr";  ?>
            </td>
				...
				$from_customer_id=$payment['credit_customer_id'];
				 ...
				 <tr class="resultRow">
					...
					<td><?php if($head_type!=3) echo number_format($payment['amount'],2); ?>
                     ...
                     <td><?php echo $payment['remarks']; ?></td>
				   <td><?php if($head_type==3) $openingBalance=$openingBalance+number_format($payment['amount'],2); else if($head_type!=3) $openingBalance=$openingBalance-number_format($payment['amount'],2); if($openingBalance>=0) echo number_format($openingBalance,2)." Dr"; else echo number_format(-$openingBalance,2)." Cr";  ?>
					</td>
					...
					$to_customer_id=$payment['debit_customer_id'];
					 ...
					 <tr class="resultRow">
						...
						<td><?php if($head_type==4) echo number_format($payment['amount'],2); ?>
                         ...
                         <td><?php echo $payment['remarks']; ?></td>
					   <td><?php if($head_type!=4) $openingBalance=$openingBalance+number_format($payment['amount'],2); else if($head_type==4) $openingBalance=$openingBalance-number_format($payment['amount'],2); if($openingBalance>=0) echo number_format($openingBalance,2)." Dr"; else echo number_format(-$openingBalance,2)." Cr";  ?>
						</td>
					...
					$to_customer_id=$payment['to_customer_id'];
					 ...
					 <tr class="resultRow">
						...
						<td><?php if($head_type==4) echo number_format($payment['amount'],2); ?>
                         ...
                         <td><?php echo $payment['remarks']; ?></td>
					   <td><?php if($head_type!=4) $openingBalance=$openingBalance+number_format($payment['amount'],2); else if($head_type==4) $openingBalance=$openingBalance-number_format($payment['amount'],2); if($openingBalance>=0) echo number_format($openingBalance)." Dr"; else echo number_format(-$openingBalance)." Cr";  ?>
						</td>
				...
				$to_customer_id=$payment['to_customer_id'];
				 ...
				 <tr class="resultRow">
					...
					<td><?php if($head_type!=4) echo number_format($payment['amount'],2); ?>
                     ...
                     <td><?php echo $payment['remarks']; ?></td>
				   <td><?php if($head_type==4) $openingBalance=$openingBalance+number_format($payment['amount'],2); else if($head_type!=4) $openingBalance=$openingBalance-number_format($payment['amount'],2); if($openingBalance>=0) echo number_format($openingBalance)." Dr"; else echo number_format(-$openingBalance,2)." Cr";  ?>
					</td>
					<td class="no_print"> <a href="<?php if($payment['auto_rasid_type']==0) { echo WEB_ROOT.'admin/accounts/transactions/credit_note/index.php?view=details&id='.$payment['id']; } else  echo WEB_ROOT.'admin/accounts/transactions/credit_note/index.php?view=details&id='.$payment['id']; ?>"><button title="View this entry" class="btn viewBtn"><span class="view">V</span></button></a>
					</td>
    ...
    <input type="submit" class="btn btn-warning" style="margin-top:20px;" />
    </form>
    </div>
