...
{	
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,$admin_rights)))
			{
						...
						$result=insertReceiptForVehicleId($_POST['vehicle_id'],$_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks']); // $cheque_return is 0 when inserting a payment			
				...
				if($result=="success")
				{
					...
					header("Location: ".WEB_ROOT."admin/customer/vehicle_receipt/index.php?view=allReceipts&id=".$_POST["customer_id"]);
					exit;
				}
				...
				$_SESSION['ack']['type']=4; // 4 for error
				header("Location: ".WEB_ROOT."admin/customer/vehicle_receipt/index.php?view=index&id=".$_POST["customer_id"]);
				}
			}
			else
			{	
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteReceiptForCustomerIdFinancer($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Receipt deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				header("Location: ".WEB_ROOT."admin/customer/vehicle_receipt/index.php?view=allReceipts&id=".$_GET["customer_id"]);
				}
			}
			else
			{	
		...
		if(isset($_SESSION['edmsAdminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=updateReceiptForVehicleId($_POST['vehicle_id'],$_POST["lid"],$_POST["amount"],$_POST['payment_date'],$_POST['to_ledger_id'],$_POST['from_ledger_id'],$_POST['remarks']);
