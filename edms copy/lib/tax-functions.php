...
function insertTax($name,$display_name,$tax_percent,$in_out){
	...
	try
	{
		...
		$our_company_id=$_SESSION['edmsAdminSession']['oc_id'];
		$duplicate=checkForDuplicateTax($name,$tax_percent,$in_out,$our_company_id);
		...
		$sql="INSERT INTO
		      edms_tax (tax_name,display_name, tax_percent, in_out, tax_ledger_id,company_id)
			  VALUES
			  ('$name', '$display_name', $tax_percent , $in_out, $tax_ledger_id,$our_company_id)";
		$result=dbQuery($sql);	
...
function updateTax($id,$name,$display_name,$tax_percent,$in_out){
	...
	try
	{
		...
		$display_name=$name;
		$tax = getTaxByID($id);
		$our_company_id=$tax['oc_id'];
		$duplicate=checkForDuplicateTax($name,$tax_percent,$in_out,$our_company_id,$id);
...
function checkForDuplicateTax($name,$tax_percent,$in_out,$oc_id,$id=false)
{
	try{
		$sql="SELECT tax_id 
			  FROM 
			  edms_tax 
			  WHERE tax_name='$name'
			  AND tax_percent=$tax_percent AND in_out = $in_out AND company_id = $oc_id ";
		if($id==false)
		$sql=$sql."";
		else
...
{
	$sql="SELECT tax_id,tax_percent, tax_name, display_name, in_out, tax_ledger_id, company_id
			  FROM 
			  edms_tax 
			  WHERE tax_id=$id";
		$result=dbQuery($sql);	
...
function insertTaxGroupName($name,$display_name,$in_out,$tax_class_id)
{
	...
	if(!validateForNull($display_name))
	$display_name=$name;
	if(validateForNull($name) && strlen($name)<255 && checkForNumeric($in_out,$our_company_id) && !checkForDuplicateTaxGroup($name,$in_out,$our_company_id) && ((checkForNumeric($tax_class_id) && $tax_class_id>0) || TAX_CLASS==0) )
	{
...
function checkForDuplicateTaxGroup($name,$in_out,$oc_id,$id=false)
{
	$sql="SELECT tax_group_id 
			  FROM 
			  edms_tax_grp 
			  WHERE tax_group_name='$name' AND in_out = $in_out AND our_company_id=$oc_id ";
...
function editTaxGroupName($id,$name,$display_name,$in_out,$tax_class_id)
{
	...
	if(!validateForNull($display_name))
	$display_name=$name;
	$tax_group=getTaxGroupByID($id);
	$oc_id = $tax_group['our_company_id'];
	if(validateForNull($name) && strlen($name)<255 && !checkForDuplicateTaxGroup($name,$in_out,$oc_id,$id) && checkForNumeric($id) && ((checkForNumeric($tax_class_id) && $tax_class_id>0) || TAX_CLASS==0))
	{
		$name=trim($name);
		$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
...
{
	$sql="SELECT edms_tax_grp.tax_group_id,GROUP_CONCAT(tax_id) as taxes_id, in_out,CONCAT(IF(in_out=2,'IN PURCHASE',IF(in_out=1,'OUT',IF(in_out=3,'IN SALE','IN'))), ' ', tax_group_name) as tax_group_name, tax_group_name as unprefixed_name, tax_class_id, our_company_id
	      FROM edms_tax_grp
		  LEFT JOIN edms_rel_tax_grp_tax
		  ON edms_tax_grp.tax_group_id=edms_rel_tax_grp_tax.tax_group_id
...
function listTaxFormsIssuable($from=NULL,$to=NULL,$form_no=NULL,$form_date=NULL)
{
	if(isset($from) && validateForNull($from))
    {
	$from = str_replace('/', '-', $from);
	...
	$sql="SELECT edms_ac_purchase.purchase_id as id, edms_ac_trans_tax_form.tax_form_id,form_no,form_date,form_name, 1 as type FROM edms_ac_trans_tax_form INNER JOIN edms_tax_forms ON edms_tax_forms.tax_form_id = edms_ac_trans_tax_form.tax_form_id INNER JOIN edms_ac_purchase.purchase_id = edms_ac_trans_tax_form.purchase_id WHERE edms_ac_trans_tax_form.purchase_id IS NOT NULL";
	if(isset($from) && validateForNull($from))
	$sql=$sql." AND trans_date >='$from' 
		  ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."AND trans_date<='$to' ";	
	$sql=$sql."SELECT edms_ac_credit_note.credit_note_id as id, edms_ac_trans_tax_form.tax_form_id,form_no,form_date,form_name ,3 as type FROM edms_ac_trans_tax_form INNER JOIN edms_tax_forms ON edms_tax_forms.tax_form_id = edms_ac_trans_tax_form.tax_form_id INNER JOIN edms_ac_credit_note.credit_note_id = edms_ac_trans_tax_form.credit_note_id WHERE edms_ac_trans_tax_form.credit_note_id IS NOT NULL ";
	if(isset($from) && validateForNull($from))
	$sql=$sql." AND trans_date >='$from' 
		  ";
	if(isset($to) && validateForNull($to))  
...
function listTaxFormsReceivable($from=NULL,$to=NULL,$form_no=NULL,$form_date=NULL)
{
	if(isset($from) && validateForNull($from))
    {
	$from = str_replace('/', '-', $from);
	...
	$sql="SELECT edms_ac_sales.sales_id, edms_ac_trans_tax_form.tax_form_id,form_no,form_date,form_name FROM edms_ac_trans_tax_form INNER JOIN edms_tax_forms ON edms_tax_forms.tax_form_id = edms_ac_trans_tax_form.tax_form_id INNER JOIN edms_ac_sales.sales_id = edms_ac_trans_tax_form.sales_id WHERE edms_ac_trans_tax_form.sales_id IS NOT NULL";
	if(isset($from) && validateForNull($from))
	$sql=$sql." AND trans_date >='$from' 
		  ";
	if(isset($to) && validateForNull($to))  
	$sql=$sql."AND trans_date<='$to' ";	
	$sql=$sql."SELECT edms_ac_debit_note.debit_note_id, edms_ac_trans_tax_form.tax_form_id,form_no,form_date,form_name FROM edms_ac_trans_tax_form INNER JOIN edms_tax_forms ON edms_tax_forms.tax_form_id = edms_ac_trans_tax_form.tax_form_id INNER JOIN edms_ac_debit_note.debit_note_id = edms_ac_trans_tax_form.debit_note_id WHERE edms_ac_trans_tax_form.debit_note_id IS NOT NULL ";
	if(isset($from) && validateForNull($from))
	$sql=$sql." AND trans_date >='$from' 
		  ";
	if(isset($to) && validateForNull($to))  
