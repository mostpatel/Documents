...
{
	$sql="SELECT contra_id,amount,from_ledger_id,to_ledger_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,vch_no
			  FROM edms_ac_contra";
	$result=dbQuery($sql);
...
function addContra($amount,$trans_date,$to_ledger,$from_ledger,$remarks,$vch_no=NULL) //$from_ledger should start with C for customer or L for ledger 
{
...
$oc_id=$_SESSION['edmsAdminSession']['oc_id'];
	...
	$vch_counter = getTransCounterForOCID($oc_id,8);
	if(!checkForNumeric($vch_no))
	{
	$vch_no = $vch_counter;	
	}
	if(checkForNumeric($amount,$to_ledger,$from_ledger,$admin_id,$vch_no) && validateForNull($trans_date))
	{
		
			
			$sql="INSERT INTO edms_ac_contra (amount,from_ledger_id,to_ledger_id,oc_id,trans_date,remarks,created_by,last_updated_by,date_added,date_modified,vch_no)
			VALUES ($amount,$from_ledger,$to_ledger,$oc_id,'$trans_date','$remarks',$admin_id,$admin_id,NOW(),NOW(),'$vch_no')";
	
			$result=dbQuery($sql);
			
			$ac_starting_date = $accounts_settings['ac_starting_date'];
			if($vch_counter==$vch_no)
			incrementTransCounterForOCID($oc_id,8);
...
function updateContra($id,$amount,$trans_date,$to_ledger,$from_ledger,$remarks,$vch_no=NULL)
{
	$admin_id=$_SESSION['edmsAdminSession']['admin_id'];
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT contra_id,SUM(amount),from_ledger_id,to_ledger_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,vch_no
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT contra_id,SUM(amount) as total_amount,from_ledger_id,to_ledger_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,vch_no
			  FROM edms_ac_contra WHERE ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT contra_id,amount,from_ledger_id,to_ledger_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified,vch_no
			  FROM edms_ac_contra WHERE ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
...
if(isset($to) && validateForNull($to))
{
	...
	{
	$sql="SELECT contra_id,amount,from_ledger_id,to_ledger_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified, remarks,vch_no
			  FROM edms_ac_contra WHERE ";
	if(isset($from) && validateForNull($from))
	$sql=$sql."trans_date>='$from' 
		  AND ";
