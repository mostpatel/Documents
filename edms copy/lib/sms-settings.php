...
/*	$type=1;
	...
	{
	$message = 'Dear '. $customer_name.", Your outstanding amount with Ambica Automobiles is ".$amount." Rs. Please Pay the amount as soon as possible. Ignore if paid.";
	}
	else if($type==7)
	{
	$message = 'Dear '. $customer_name.", unpaid amount of your Invoice No.  ".$invoice_no."  Dated ".$invoice_date." with Ambica Automobiles is ".$amount." Rs. Please Pay the amount as soon as possible. Ignore if paid.";
	}
	
	if($type=="delivery_status")
	{
		 $params = array('APIKEY' => 'HddAZZQosGE' ,'MsgID' => $msg_id, 'SenderID' => 'AMBICA');
		}
	else
	{
    /* $_GET Parameters to Send */
    $params = array('APIKEY' => 'HddAZZQosGE' ,'MobileNo' => '91'.$customer_mobile_no, 'SenderID' => 'AMBICA', 'Message' => $message , 'ServiceName' => 'TEMPLATE_BASED');
	}
