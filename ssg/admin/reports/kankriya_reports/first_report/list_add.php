<?php $payment_should_be_done_after_date = getTodaysDateTimeBeforeMonthsAndDays(2,15); ?>
...
<h4 class="headingAlignment no_print">TVR Reports</h4>
<?php 
...
<div class="printBtnDiv no_print"><button class="printBtn btn"><i class="icon-print"></i> Print</button></div>     
    <div class="showColumns">
        ...
        <input class="showCB" type="checkbox" id="9" checked="checked"  /><label class="showLabel" for="9">Name</label> 
          ...
          <input class="showCB" type="checkbox" id="11" checked="checked"  /><label class="showLabel" for="11">Gaurantor Name</label> 
          <input class="showCB" type="checkbox" id="12" checked="checked"  /><label class="showLabel" for="12">Gaurantor Contact No</label> 
           <input class="showCB" type="checkbox" id="13" checked="checked"  /><label class="showLabel" for="13">Broker</label> 
    </div>
    ...
    <thead>
        ...
        <th class="heading no_print">Print</th>
            ...
            <th class="heading">Contact No</th>
			 <th class="heading">Gau Name</th>
            <th class="heading">Gau Contact No</th>
    ...
    <tbody>
		...
		foreach($emi_array as $emi)
		{
		 ...
		 ?>
         <tr class="resultRow">
         	<td class="no_print"><input type="checkbox" class="selectTR" name="selectTR"  /><?php $last_payment_month=date('m',strtotime($emi['payment_date'])); if(getCurrentMonth()==$last_payment_month) echo "(D)"; ?></td>
        	<td><?php echo ++$i; ?></td>
			...
			echo $prefix.".".preg_replace('/[^0-9]+/', '', $emi['file_no']); ?></span> <?php  echo  $emi['file_no']; if($seieze) echo "(S)"; ?>
             ...
             <td><?php  echo $emi['file_agreement_number']; ?>
           ...
           <td>
            ...
            <td><?php   echo $emi['customer']['customer_name']; ?></td>
             <td><?php   $contactArray = $emi['customer']['contact_no']; 
			 	...
			 	?></td>
               <td><?php  if(isset( $emi['guarantor']['guarantor_name'])) echo $emi['guarantor']['guarantor_name']; else echo "-"; ?></td>
             <td><?php if(isset( $emi['guarantor']['guarantor_name'])) {  $contactArray = $emi['guarantor']['contact_no']; 
                 ...
                 <td><?php   echo $emi['broker_name']; ?></td>
                  <td><?php   $last_payment_date=date('d/m/Y',strtotime($reminder_date)); if($last_payment_date!='01/01/1970') echo $last_payment_date; else echo "NA"; ?>
            </td>
...
<?php  ?> 
<div style="position:relative;width:100%;" class="to_print">
<?php 
    	...
    	<tr>
            <td>
            	<table>
                	<tr>
                    	<td>Reg No :</td>
                        <td><?php if($emi['reg_no']!=null && $emi['reg_no']!="") echo $emi['reg_no']; else echo "NA"; ?></td>
                    </tr>
                    <tr>
                    	<td>Party :</td>
                        <td><?php if(validateForNull($emi['customer']['secondary_customer_name'])) echo $emi['customer']['secondary_customer_name']; else echo $emi['customer']['customer_name']; ?></td>
                    </tr>
                    <tr>
                    	<td>Address :</td>
                    ...
                    <tr>
                    	<td>Bucket :</td>
                        <td> <p><?php  $total_bucket=0; if(isset($emi['bucket_details']) && $emi['bucket_details']!=0 && is_array($emi['bucket_details']) && count($emi['bucket_details'])>1 && $emi['file_status']!=4) { foreach($emi['bucket_details'] as $e=>$corr_bucket) { $total_bucket=$total_bucket+$corr_bucket; echo $e." X ".$corr_bucket." <br>";} }else if($emi['file_status']!=4){ foreach($emi['bucket_details'] as $e=>$corr_bucket) { $total_bucket=$total_bucket+$corr_bucket; echo $e;}  } else if($emi['file_status']==4 && $emi['loan_scheme']!="error"){ if(is_array($emi['loan_scheme'])) {  foreach($emi['loan_scheme'] as $scheme){ echo $scheme['emi']." X ".$scheme['duration']."<br>"; } } }; ?>
          </p>(<?php echo $emi['balance']; ?>)	 </td>
                    </tr>
                    <tr>
                    	<td>Engine No :</td>
            ...
            <td>
            	<table>
                	<tr>
                    	<td>Ref No :</td>
                        <td><?php  echo  $emi['file_no']; if($seieze) echo "(S)"; ?></td>
                    </tr>
                    <tr>
                    	<td>Guarantor :</td>
