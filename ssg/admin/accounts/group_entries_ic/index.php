...
if(isset($_GET['action']))
{
	if($_GET['action']=='generateReport')
	{
		...
		$reportArray=getAllTransactionForLedgerGroupId($id,$transaction_array,$from,$to);
		$_SESSION['ledgerGroupICEntriess']['entries_array']=$reportArray;
		$_SESSION['ledgerGroupICEntriess']['from']=$from;
		$_SESSION['ledgerGroupICEntriess']['to']=$to;
		$_SESSION['ledgerGroupICEntriess']['transaction_array']=$transaction_array;
		$_SESSION['ledgerGroupICEntriess']['group_id']=$id;
		$_SESSION['ledgerGroupICEntriess']['debit_ic']=$debit_interest;
		$_SESSION['ledgerGroupICEntriess']['credit_ic']=$credit_interest;
		$_SESSION['ledgerGroupICEntriess']['tds']=$tds_rate;
		$_SESSION['ledgerGroupICEntriess']['tds_on_amount']=$tds_amount;
		header("Location: index.php");		
		exit;
	}			
	}
