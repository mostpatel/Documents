...
function getWelcomeById($id){
	...
	try
	{
		...
		if(checkForNumeric($id))
		{
			$sql="SELECT welcome_id, welcome_date, customer_name, customer_address,  guarantor_name, guarantor_address , vehicle_model,  welcome_type, reg_ad, received, received_date,file_id, created_by, last_updated_by, date_added, date_modified FROM fin_loan_welcome WHERE welcome_id=$id";
			$result=dbQuery($sql);
			$resultArray=dbResultToArray($result);
			if(dbNumRows($result)>0)
