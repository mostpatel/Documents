...
function updateOurCompany($id,$name,$address,$pincode,$city_id,$company_prefix,$subheading,$contact_no,$rasid_counter)
{
	try{
		$admin_id=$_SESSION['adminSession']['admin_id'];
		$name = strtoupper($name);
...
{
	$sql="SELECT our_company_id, sub_heading, our_company_name, our_company_address, our_company_pincode, sub_heading, fin_city.city_id, city_name, our_company_prefix, rasid_counter
	      FROM fin_our_company,fin_city
		  WHERE our_company_id=$id
