...
function getCustomerDetailsByFileId($file_id)
{
	if(checkForNumeric($file_id))
	{
		$sql="SELECT customer_id,customer_name,customer_address,customer_pincode,city_id,area_id,file_id,paid_by, opening_balance, opening_cd, current_balance, current_balance_cd, secondary_customer_name , secondary_customer_address 
		      FROM fin_customer
			  WHERE file_id=$file_id";
		$result=dbQuery($sql);
		$resultArray=dbResultToArray($result);
