...
function listNoticesForFileID($file_id){
	...
	try
	{
		...
		{
			$sql="SELECT notice_id, notice_date, customer_name, customer_address, guarantor_name, guarantor_address, bucket, bucket_amount, file_id, created_by, last_modified_by, date_added, date_modified FROM fin_loan_notice WHERE file_id=$file_id ORDER BY notice_date DESC";
			$result=dbQuery($sql);
...
function insertNotice($file_id,$notice_date,$customer_name,$customer_address,$guarantor_name, $guarantor_address ,$bucket=null,$bucket_amount=null,$note=""){
...
$guarantor_address='<pre>'.$guarantor_address.'</pre>';
		...
		$sql="INSERT INTO fin_loan_notice(notice_date,  customer_name, customer_address, guarantor_name, guarantor_address, bucket, bucket_amount, file_id, note, created_by, last_modified_by, date_added, date_modified) VALUES ('$notice_date', '$customer_name', '$customer_address', '$guarantor_name', '$guarantor_address', '$bucket', '$bucket_amount', $file_id , '$note', $admin_id, $admin_id, NOW(), NOW())";
...
function getNoticeById($id){
	...
	try
	{
		...
		if(checkForNumeric($id))
		{
			$sql="SELECT notice_id, notice_date, customer_name, customer_address, guarantor_name, guarantor_address, bucket, bucket_amount, file_id, note, created_by, last_modified_by, date_added, date_modified FROM fin_loan_notice WHERE notice_id=$id";
			$result=dbQuery($sql);
			$resultArray=dbResultToArray($result);
			if(dbNumRows($result)>0)
