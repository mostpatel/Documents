...
function insertLedger($name,$postal_name,$address,$city_id,$area,$pincode,$head_id,$contact_no,$pan_no,$sales_no,$notes,$opening_balance,$opening_balance_cd,$agency_id=null,$oc_id=null){
	...
	try
	{
		...
		if($name!=null && $name!=''  && checkForNumeric($head_id) && $head_id>0 && strlen($pincode)==6 && !checkforDuplicateLedger($name))
			{
...
function checkforDuplicateLedger($ledger_name)
{
	...
	if(!(checkForNumeric($oc_id)))
	 ...
	 $sql="SELECT ledger_id FROM fin_ac_ledgers WHERE ledger_name='$ledger_name' AND oc_id = $oc_id";
	 $result = dbQuery($sql);
	 $resultArray = dbResultToArray($result);
	 if(dbNumRows($result)>0)
	 return $resultArray[0][0];
