...
if(isset($to) && validateForNull($to))
{
	...
	if((checkForNumeric($from_customer)  || checkForNumeric($from_ledger)) && checkForNumeric($month_id,$year))
	{
	$sql="SELECT edms_ac_jv.jv_id,edms_ac_jv_cd.amount,(SELECT from_ledger_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 1) as from_ledger_id  , (SELECT from_customer_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 1) as from_customer_id,(SELECT to_ledger_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 0) as to_ledger_id  , (SELECT to_customer_id FROM edms_ac_jv_cd as inner_ac_jv_cd WHERE inner_ac_jv_cd.jv_id = edms_ac_jv.jv_id AND type = 0) as to_customer_id,oc_id,auto_rasid_type,auto_id,trans_date,CONCAT_WS(' ',DATE_FORMAT(trans_date,'%b'),DATE_FORMAT(trans_date,'%Y')) AS month_year,DATE_FORMAT(trans_date,'%c') as month_id, DATE_FORMAT(trans_date,'%Y') as year, created_by,last_updated_by,date_added,date_modified
