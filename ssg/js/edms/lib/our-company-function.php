...
function getTaxInvoiceCounterForOCID($id)
{
	$sql="SELECT our_company_prefix,tax_invoice_counter FROM
		   edms_our_company
		   WHERE our_company_id=$id";
	$result=dbQuery($sql);
	$resultArray=dbResultToArray($result);
...
function incrementTaxInvoiceNoForOCID($id)
{
	$r=getInvoiceCounterForOCID($id);
	$r++;
	$sql="UPDATE edms_our_company
	      SET tax_invoice_counter=$r
		  WHERE our_company_id=$id";
	dbQuery($sql);	  
...
function resetInvoiceCountersOC()
{
		$sql="UPDATE edms_our_company SET rasid_reset_date=NOW(), invoice_counter=1,tax_invoice_counter=1";
		dbQuery($sql);
		return "success";
		}		
