...
// JavaScript Document
				 ...
				 jQuery(".person_name").validate({
                     expression: "if (VAL.match(/^[0-9a-zA-Z.(),:- ]+$/)) return true; else return false;",
				...
				else if(document.alphanum_fileno==2)
				{
				  jQuery("#fileNumber").validate({
...
function calculateEmi()
{
	...
	if(!isNaN(amount) && !isNaN(duration) && !isNaN(roi) && amount!=null && amount!="" && roi!=null && roi!="" && duration!=null && duration!="")
	{
		...
		else
		{
			...
			document.getElementById('total_collection').innerHTML = emi*duration;
	}
	}
	else
	{
	...
	emi=total_collection/duration;	
		document.getElementById('total_collection').innerHTML = total_collection;
...
function calculateROI()
{
	...
	var emi_per_year=12/duration_unit;
	var roi_type=0;
