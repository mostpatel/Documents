...
if(isset($_GET['action']))
{
		...
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=insertCustomerNote($_POST["note"], $_POST["customer_id"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Note successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteQuantity($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Quantity deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Quantity! Quantity already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateQuantity($_POST["lid"],$_POST["quantity"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Quantity updated Successfuly!";
