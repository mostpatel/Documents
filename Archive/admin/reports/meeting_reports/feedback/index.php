...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$customerData=viewMeetingCustomers($_POST['from_date'],$_POST['to_date']);
			
				$_SESSION['fCustomerReport']['customer_array']=$customerData;
				$_SESSION['fCustomerReport']['from_date']=$_POST['from_date'];
				$_SESSION['fCustomerReport']['to_date']=$_POST['to_date'];
		...
		if($_GET['action']=='sendDirectorsms')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				
		sendDirectorSMS($_POST['selectTR']);
		header("Location: ".$_SERVER['PHP_SELF']);
				exit;
			}
			}
