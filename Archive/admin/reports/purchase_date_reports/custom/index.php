...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			...
			$purchaseDates=viewPurchaseDates($_POST['from_date'],$_POST['to_date']);
			
				$_SESSION['cPurchaseDateReport']['purchaseDate_array']=$purchaseDates;
				$_SESSION['cPurchaseDateReport']['from_date']=$_POST['from_date'];
				$_SESSION['cPurchaseDateReport']['to_date']=$_POST['to_date'];
	...
	else if($_GET['action']=='fromHomeUpcomingFollowUps')
	{
		...
		$purchaseDates=viewFollowUps($from, $to);
