...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertVehicleCC($_POST["name"],$_POST["vehicle_type_id"] );
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Vehicle CC successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteVehicleCC($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Vehicle CC deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Vehicle CC! Vehicle CC already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateVehicleCC($_POST["lid"],$_POST["cc"], $_POST["vehicle_type_id"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Vehicle CC updated Successfuly!";
