...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertPrefix($_POST["prefix"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Prefix successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deletePrefix($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Prefix deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Prefix! Prefix already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updatePrefix($_POST["lid"],$_POST["prefix"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Prefix updated Successfuly!";
