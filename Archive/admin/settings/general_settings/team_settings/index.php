...
if(isset($_GET['action']))
{
		...
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=insertTeam($_POST["team_name"], $_POST["admin_id"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Team successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteATeam($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Team deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Team! Team already in use!";
	...
	
	{
		...
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateATeam($_POST["lid"], $_POST["team_name"], $_POST["admin_id"] );
				
				if($result=="success")
				{
					
				$_SESSION['ack']['msg']="Team updated Successfuly!";
