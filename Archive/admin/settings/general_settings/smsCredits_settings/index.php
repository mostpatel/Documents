...
if(isset($_GET['action']))
{
	if($_GET['action']=='add')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=insertRelation($_POST["relation"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Relation successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteRelation($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Relation deleted Successfuly!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateRelation($_POST["lid"],$_POST["relation"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Relation updated Successfuly!";
