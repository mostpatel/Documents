...
require_once "../../../../lib/event-functions.php";
...
if(isset($_GET['action']))
{
		...
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=insertEvent($_POST["event_title"], $_FILES['event_img'], $_POST["event_description"]);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Event successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteEvent($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Event deleted Successfuly!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateEvent($_POST["lid"],$_POST["name"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Event updated Successfuly!";
