...
if(isset($_GET['action']))
{
		...
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(2,$admin_rights) || in_array(7,					$admin_rights)))
			{
				...
				$result=insertCategory($_POST["name"], $_FILES['cat_img'], $_POST["cat_description"], $_POST['super_cat_id']);
				
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Category successfully added!";
	...
	if($_GET['action']=='delete')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(4,$admin_rights) || in_array(7,					$admin_rights)))
			{	
				$result=deleteCategory($_GET["lid"]);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Category deleted Successfuly!";
				$_SESSION['ack']['type']=3; // 3 for delete
				}
				else
				{
					$_SESSION['ack']['msg']="Cannot delete Category!Category already in use!";
	...
	if($_GET['action']=='edit')
	{
		if(isset($_SESSION['EMSadminSession']['admin_rights']) && (in_array(3,$admin_rights) || in_array(7,					$admin_rights)))
			{
				$result=updateCategory($_POST["lid"],$_POST["name"], $_FILES['cat_img'], $_POST["cat_description"], $_POST['super_cat_id']);
				if($result=="success")
				{
				$_SESSION['ack']['msg']="Category updated Successfuly!";
