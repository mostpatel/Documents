...
<td>% for D/c : </td>
				<td>
				 <input autocomplete="off" type="text" class=""  name="dc" id="dc" placeholder=""  value="<?php if(isset($_SESSION['cKankriyaBrokerReport']['dc'])) echo $_SESSION['cKankriyaBrokerReport']['dc']; ?>" />	
                 </td>
</tr>
...
<td>% for L/R : </td>
				<td>
				 <input autocomplete="off" type="text" class=""  name="lr" id="lr" placeholder=""  value="<?php if(isset($_SESSION['cKankriyaBrokerReport']['lr'])) echo $_SESSION['cKankriyaBrokerReport']['lr']; ?>" />	
                 </td>
</tr>
...
<td>% Agent Participation : </td>
				<td>
				 <input autocomplete="off" type="text" class=""  name="participation" id="participation" placeholder=""  value="<?php if(isset($_SESSION['cKankriyaBrokerReport']['participation'])) echo $_SESSION['cKankriyaBrokerReport']['participation']; ?>" />	
                 </td>
</tr>
    ...
    <tbody>
		...
		foreach($emi_array as $emi)
		{
         ...
         <tr class="resultRow <?php if($seieze) echo "dangerRow";  ?>">
			...
			$penalty_amt=0;
			$participation_amt=0;
				$int_paid =  round($emi_paid*$net_loan_interest_to_pay_per_emi,0,PHP_ROUND_HALF_UP); echo $int_paid; $int_total = $int_total + $int_paid;
			  ?></b>
            </td>
          
            <td   style=""><b><?php  
			 
					if($file_charges==NULL) echo "NOT ADDED"; else { if(is_numeric($file_charges) && $file_charges>0 && $file_charges<=100) {  $net_file_charge_percent = $file_charges - $dc;  $net_file_charge_amt = ($net_file_charge_percent*$emi['loan_amount'])/100; echo $net_file_charge_amt; $dc_total = $dc_total + $net_file_charge_amt; } }		
			 	?></b></td>
            
               <td  style=""><b><?php   
			  if($penalty_type==1) { $lr_give = 100 - $lr;  $penalty_amt = round(($lr_give/100)*$penalty_amount,2); echo $penalty_amt; $lr_total = $lr_total + $penalty_amt; } else if($penalty_type==0) {  $penalty_amt = -round($penalty_amount,2); echo $penalty_amt;	$lr_total = $lr_total + $penalty_amt;}		
			 	?></b></td>
