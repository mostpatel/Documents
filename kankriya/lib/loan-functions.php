...
function getFileIdsAndLoanIdsForAgencyId($agency_id,$to=false,$vehicle_company_id=NULL,$vehicle_model_year=NULL,$customer_group_id=NULL,$vehicle_type=NULL)
{
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id 
				FROM fin_file 
				INNER JOIN fin_loan ON fin_file.file_id= fin_loan.file_id   
				LEFT JOIN fin_vehicle ON fin_file.file_id = fin_vehicle.file_id WHERE  agency_id=$agency_id AND file_status!=3 AND ";
		if($to)
		$sql=$sql."(file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL )";
		else
		$sql=$sql." 1=1";
...
function getFileIdsAndLoanIdsForOCId($agency_id,$to=false,$vehicle_company_id=NULL,$vehicle_model_year=NULL,$customer_group_id=NULL,$vehicle_type=NULL)
{
	
	if(checkForNumeric($agency_id))
	{
		$sql="SELECT fin_file.file_id, fin_loan.loan_id FROM fin_file INNER JOIN  fin_loan ON fin_file.file_id= fin_loan.file_id  LEFT JOIN fin_vehicle ON fin_file.file_id = fin_vehicle.file_id WHERE  oc_id=$agency_id AND file_status!=3 AND ";
		if($to)
		$sql=$sql."(file_status!=4 OR (SELECT file_id FROM fin_file_closed WHERE fin_file_closed.file_id = fin_file.file_id AND file_close_date > '$to') IS NOT NULL )";
		else
		$sql=$sql." 1=1";
