...
function getCashLedgerIdForOC($oc_id)
{
	...
	if(dbNumRows($result)>0 && checkForNumeric($resultArray[0][0]))
	return $resultArray[0][0];
	else
	{
...
function getPenaltyLedgerIdForOC($oc_id)
{
if(checkForNumeric($oc_id))
{	
	...
	$sql="SELECT ledger_id FROM fin_ac_ledgers WHERE head_id=$penalty_head_id  AND ledger_name='".$ledger_name."' AND ";
	if(!$ca_id)
	$sql=$sql." oc_id=$oc_id";
	else if(checkForNumeric($ca_id))
	{
	...
	else
	{
		...
		$result=createPenaltyLedgerForOC($oc_id);
		if($result)
		{
		return getPenaltyLedgerIdForOC($oc_id);
		}
		}
	}
	return false;	
...
function getPenaltyLedgerIdForAgency($agency_id)
{
	if(checkForNumeric($agency_id))
	{
	...
	{
		$result=createPenaltyLedgerForOC($agency_id);
		if($result)
		{
		return getPenaltyLedgerIdForAgency($agency_id);
		}
		}
	}
	return false;	
...
function getIncomeLedgerIdForOC($oc_id)
{
	if(checkForNumeric($oc_id))
	{
	if(defined('FINANCE_INCOME_HEAD'))
	$income_head_id = FINANCE_INCOME_HEAD;
	else		
	$income_head_id=getIndirectIncomeId();
	
		if(defined('FINANCE_INCOME_NAME'))
	{
	...
	$sql="SELECT ledger_id FROM fin_ac_ledgers WHERE head_id=$income_head_id  AND ledger_name='$ledger_name' AND ";
		if(!$ca_id)
	$sql=$sql." oc_id=$oc_id";
	else if(checkForNumeric($ca_id))
	{
...
function getPrincipalAmountToBeCreditedForEMIPaymentIdReducing($payment_id) //FOR PAYMENT AMOUNT after the books starting date
{
	
	if(checkForNumeric($payment_id))
	{
		$amount_percent_emi_wise = getAmountPercentWiseForPaymentId($payment_id);
		$loan_id=getLoanIdFromEmiPaymentId($payment_id);
		$payment=getPaymentDetailsForEmiPaymentId($payment_id);
		$file_id=getFileIdFromLoanId($loan_id);
		$agency_company_type_array=getAgencyOrCompanyIdFromFileId($file_id);
		...
		if(strtotime($payment['payment_date'])>=strtotime($booksStartingDate))	
		{
			foreach($amount_percent_emi_wise as $amount_percent_emi)
			{
				$princ_interest=getIntPrincBalanceTableForLoanAndEmiDate($loan_id,$amount_percent_emi['actual_emi_date']);
				$princ = $princ_interest['principal'];
				$princ = $princ*$amount_percent_emi['amount_percent'];
				$principal_amount = $principal_amount + $princ;
			}
			return round($principal_amount,2);
		}	
		else return 0;
...
function getAllTransactionForLedgerGroupId($group_id,$transaction_type_array=NULL,$from=NULL,$to=NULL) // $group_id =-1 means customers
{
	 ...
	 if(COMPANY_RESTRICTION==1)
	{
		...
		$sql=$sql." AND ";
			...
			$sql=$sql." oc_id IN ($admin_companies_string) ";
...
function getOpeningBalanceForHeadIdForDate($head_id,$date) // head_id should be numeric and date is compalsory
{
	...
	$sql="SELECT ";
	if(($head_id==$cash_head_id || $head_id==$bank_head_id || $head_id == $od_bank_heaD_id))
	$sql=$sql."-";
	$sql=$sql."SUM(amount) as amount
			  FROM fin_ac_payment WHERE ";
	if(isset($to) && validateForNull($to))  
...
function getNumberOfTransactionsForLedgerForDate($id,$from,$to) // return opening balance on date for id, id should start with L or C
{
	...
	if(!isset($head_type) || $head_type==1)
	{
		...
		$sql=$sql." UNION ALL ";
				$sql=$sql." SELECT  count(fin_ac_jv.jv_id)
					  FROM fin_ac_jv , fin_ac_jv_cd WHERE  ";
			if(isset($from) && validateForNull($from))  
	$sql=$sql."trans_date>='$from'
		  AND ";	  
	...
	else if($head_type==0)
	{
		...
		$sql=$sql." UNION ALL SELECT count(contra_id)
			  FROM fin_ac_contra WHERE ";
	if(isset($from) && validateForNull($from))  
	$sql=$sql."trans_date>='$from'
		  AND ";	  
...
function checkForProperLoanEntriesAgency($agency_id)
{
					$admin_id=$_SESSION['adminSession']['admin_id'];
	            	$our_company_id = $_SESSION['adminSession']['oc_id'];
					$account_settings=getAccountsSettingsForAgency($agency_id);
					$cash_ledger_id=getCashLedgerIdForAgency($agency_id);
	...
	$sql="SELECT fin_loan.file_id,fin_loan.loan_id,file_number FROM fin_loan INNER JOIN fin_file ON fin_loan.file_id = fin_file.file_id AND file_status!=3 WHERE (loan_id NOT IN (SELECT auto_id FROM fin_ac_payment WHERE auto_rasid_type=1) OR loan_amount != (SELECT SUM(amount) FROM fin_ac_payment WHERE auto_id = fin_loan.loan_id AND auto_rasid_type=1 GROUP BY auto_id)) AND loan_approval_date>='$books_starting_date' AND agency_id = $agency_id AND our_company_id = $our_company_id";
	if($account_settings['mercantile']==2)
				$sql=$sql." UNION SELECT fin_loan.file_id,fin_loan.loan_id,file_number FROM fin_loan INNER JOIN fin_file ON fin_loan.file_id = fin_file.file_id AND file_status!=3 WHERE (loan_id NOT IN (SELECT auto_id FROM fin_ac_jv WHERE auto_rasid_type=1)) AND loan_approval_date>='$books_starting_date' AND agency = $agency_id AND our_company_id = $our_company_id
				";
...
function checkForProperLoanEntriesOC($oc_id)
{
				...
				$sql="SELECT fin_loan.file_id,fin_loan.loan_id,file_number FROM fin_loan INNER JOIN fin_file ON fin_loan.file_id = fin_file.file_id AND file_status!=3 WHERE (loan_id NOT IN (SELECT auto_id FROM fin_ac_payment WHERE auto_rasid_type=1) OR loan_amount != (SELECT SUM(amount) FROM fin_ac_payment WHERE auto_id = fin_loan.loan_id AND auto_rasid_type=1 GROUP BY auto_id)) AND loan_approval_date>='$books_starting_date' AND oc_id = $oc_id AND our_company_id = $our_company_id";
				if($account_settings['mercantile']==2)
				$sql=$sql." UNION SELECT fin_loan.file_id,fin_loan.loan_id,file_number FROM fin_loan INNER JOIN fin_file ON fin_loan.file_id = fin_file.file_id AND file_status!=3 WHERE (loan_id NOT IN (SELECT auto_id FROM fin_ac_jv WHERE auto_rasid_type=1)) AND loan_approval_date>='$books_starting_date' AND oc_id = $oc_id AND our_company_id = $our_company_id
				";
...
function checkForProperLoanEntriesCombinedAgency($oc_id)
{
...
function checkForProperReceiptEntriesAgency($agency_id)
{
...
function checkForProperReceiptEntriesOC($oc_id)
{
...
function checkForProperReceiptEntriesCombinedAgency($oc_id)
{
