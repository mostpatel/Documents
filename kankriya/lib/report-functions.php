...
if(isset($to) && validateForNull($to))
{
	...
	$sql="SELECT fin_file.file_id, fin_loan.loan_id, file_number, file_agreement_no, emi, file_status, oc_id, agency_id,
		  ...
		  GROUP_CONCAT(actual_emi_date) as loan_emi_date_array,
		  max(actual_emi_date) as last_emi_date,
		  loan_approval_date,area_id, broker_name, vehicle_reg_no, vehicle_engine_no, vehicle_chasis_no, model_id, vehicle_company_id, vehicle_model
	 	  FROM 
	      fin_file 
		  INNER JOIN  fin_loan ON fin_loan.file_id=fin_file.file_id
		  INNER JOIN  fin_loan_emi ON fin_loan.loan_id=fin_loan_emi.loan_id
...
$today=date('Y-m-d');
	...
	if(dbNumRows($result)>0)
	{
		for($xy=0;$xy<count($resultArray);$xy++)
		{
			...
			if((($setWin_gt==-1 && $setWin_lt==-1) || ( ($setWin_gt==-1 || $window>=$setWin_gt) && ($setWin_lt==-1 || $window<=$setWin_lt) )) && ( ($setbal_gt==-1 && $setbal_lt==-1) || ( ($setbal_gt==-1 || $balance>=$setbal_gt) && ($setbal_lt==-1 || $balance<=$setbal_lt) )) && validateForNull($emi_date) && $emi_date!="NA" && ((!validateForNull($from) || (strtotime($latest_payment_date)<=strtotime($from)) && (!validateForNull($to) || (strtotime($latest_payment_date)>strtotime($to))) && !validateForNull($date_type)) || $date_type==1))
			{	
...
if(isset($to) && validateForNull($to))
	...
	$sql="SELECT emi_payment_id, file_number, file_agreement_no, fin_file.agency_id,fin_file.oc_id, SUM(payment_amount) as payment_amt,fin_loan_emi_payment.paid_by, payment_mode, payment_date, rasid_no, GROUP_CONCAT(fin_loan_emi.loan_emi_id) as loan_emi_array, fin_loan_emi.loan_id, fin_file.file_id, 1 as type, remarks, fin_loan_emi_payment.created_by, fin_loan_emi_payment.last_updated_by
...
if(isset($to) && validateForNull($to))
	...
	$sql="SELECT cheque_return_id, bank_id, branch_id, cheque_no,cheque_date, fin_file.file_id,loan_emi_id, cheque_amount, rasid_no, fin_loan_emi_payment_cheque_return.date_added, payment_date, file_number, customer_name
	      FROM fin_loan_emi_payment_cheque_return, fin_file, fin_customer
		  WHERE ";
	if($agency_id_string!="" && validateForNull($agency_id_string) && $oc_id_string=="")
	{
	$sql=$sql." agency_id IN ($agency_id_string) AND ";
...
$sql="SELECT fin_file.file_id , file_number, fin_file.date_added, fin_file.created_by, admin_username,city_id, fin_vehicle.vehicle_id, vehicle_reg_no, vehicle_chasis_no, vehicle_engine_no, rto_papers, passing, permit, insurance,hp,bill,vehicle_key,fin_customer_cheque_details.remarks,broker_name, fin_customer.customer_id, cheque_details_id,required_cheques, cheques_received
      FROM fin_file
	  INNER JOIN fin_customer
	  ON fin_customer.file_id=fin_file.file_id
	  INNER JOIN fin_broker
