...
function transferDocsFromVehicleToVehicleDocs()
{
	$sql="SELECT * FROM fin_vehicle WHERE rto_papers>0 OR passing>0 OR permit>0 OR insurance>0 OR hp>0 OR bill>0 OR vehicle_key>0 OR rto_agent_id IS NOT NULL";
	$result = dbQuery($sql);
	$resultArray = dbResultToArray($result);
	...
	foreach($resultArray as $re)
	{
		...
		insertVehicleDocuments($re['vehicle_id'],$re['rto_papers'],$re['passing'],$re['permit'],$re['insurance'],$re['hp'],$re['bill'],$re['remarks'],$re['vehicle_key'],$re['rto_agent_id'],$re['work_given_date'],array(),$re['work_completion_date']);
		
	}
	
}

function insertVehicleDocsForNonInsertedFile()
{
	$sql="SELECT vehicle_id FROM fin_vehicle WHERE vehicle_id NOT IN (SELECT vehicle_id FROM fin_vehicle_docs)";
	$result = dbQuery($sql);
	$resultArray = dbResultToArray($result);
	
	foreach($resultArray as $re)
	{
		$vehicle_id = $re['vehicle_id'];
		
		insertVehicleDocuments($vehicle_id,1,1,1,1,1,1,'',1,NULL,"",array(),"");
	}
}

// SELECT * FROM `fin_loan_emi_payment` WHERE `payment_date` >= "2015-04-01" AND `rasid_identifier` = 0 AND `emi_payment_id` NOT IN  (SELECT auto_id FROM fin_ac_receipt WHERE auto_rasid_type=2)	
?>